...
class crm_phonecall(base_state, osv.osv):
    ...
    _columns = {'date_action_last': fields.datetime('Last Action', readonly=1),
     'date_action_next': fields.datetime('Next Action', readonly=1),
     'create_date': fields.datetime('Creation Date', readonly=True),
     'section_id': fields.many2one('crm.case.section', 'Sales Team', select=True, help='Sales team to which Case belongs to.'),
     ...
     'state': fields.selection([('draft', 'Draft'),
               ...
               ('pending', 'Not Held'),
               ('cancel', 'Cancelled'),
               ('done', 'Held')], string='Status', size=16, readonly=True, track_visibility='onchange', help="The status is set to 'Todo', when a case is created.                                If the case is in progress the status is set to 'Open'.                                When the call is over, the status is set to 'Held'.                                If the call needs to be done then the status is set to 'Not Held'."),
     ...
     'categ_id': fields.many2one('crm.case.categ', 'Category', domain="['|',('section_id','=',section_id),('section_id','=',False),                        ('object_id.model', '=', 'crm.phonecall')]"),
    ...
    _defaults = {'date': fields.datetime.now,
    ...
    def schedule_another_phonecall(self, cr, uid, ids, schedule_time, call_summary, user_id = False, section_id = False, categ_id = False, action = 'schedule', context = None):
        """
    ...
    def _call_create_partner(self, cr, uid, phonecall, context = None):
        ...
        partner_id = partner.create(cr, uid, {'name': phonecall.name,
    ...
    def on_change_opportunity(self, cr, uid, ids, opportunity_id, context = None):
        ...
        if opportunity_id:
            ...
            values = {'section_id': opportunity.section_id and opportunity.section_id.id or False,
    ...
    def _call_create_partner_address(self, cr, uid, phonecall, partner_id, context = None):
        ...
        return address.create(cr, uid, {'parent_id': partner_id,
         'name': phonecall.name,
    ...
    def convert_opportunity(self, cr, uid, ids, opportunity_summary = False, partner_id = False, planned_revenue = 0.0, probability = 0.0, context = None):
        ...
        for call in self.browse(cr, uid, ids, context=context):
            ...
            opportunity_id = opportunity.create(cr, uid, {'name': opportunity_summary or call.name,
    ...
    def action_make_meeting(self, cr, uid, ids, context = None):
        ...
        res['context'] = {'default_phonecall_id': phonecall.id,

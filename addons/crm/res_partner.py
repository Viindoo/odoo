...
class res_partner(osv.osv):
    ...
    def _opportunity_meeting_count(self, cr, uid, ids, field_name, arg, context = None):
        res = dict(map(lambda x: (x, {'opportunity_count': 0,
          'meeting_count': 0}), ids))
        try:
            for partner in self.browse(cr, uid, ids, context):
                res[partner.id] = {'opportunity_count': len(partner.opportunity_ids),
    ...
    _columns = {'section_id': fields.many2one('crm.case.section', 'Sales Team'),
     'opportunity_ids': fields.one2many('crm.lead', 'partner_id', 'Leads and Opportunities', domain=[('state', 'in', ('draft', 'open', 'pending'))]),
     'meeting_ids': fields.many2many('crm.meeting', 'crm_meeting_partner_rel', 'partner_id', 'meeting_id', 'Meetings'),
    ...
    def copy(self, cr, uid, record_id, default = None, context = None):
        if default is None:
            default = {}
        default.update({'opportunity_ids': [],
         'meeting_ids': [],
         'phonecall_ids': []})
    ...
    def make_opportunity(self, cr, uid, ids, opportunity_summary, planned_revenue = 0.0, probability = 0.0, partner_id = None, context = None):
        ...
        for partner in self.browse(cr, uid, ids, context=context):
            ...
            opportunity_id = lead_obj.create(cr, uid, {'name': opportunity_summary,
             'planned_revenue': planned_revenue,
             'probability': probability,
             'partner_id': partner_id,
             'categ_ids': categ_ids and categ_ids[0:1] or [],
             'state': 'draft',
             'type': 'opportunity'}, context=context)

...
CRM_LEAD_FIELDS_TO_MERGE = ['name',
 ...
 'email_cc',
 'partner_name']
CRM_LEAD_PENDING_STATES = (crm.AVAILABLE_STATES[2][0], crm.AVAILABLE_STATES[3][0], crm.AVAILABLE_STATES[4][0])
...
class crm_lead(base_stage, format_address, osv.osv):
    ...
    _track = {'state': {'crm.mt_lead_create': lambda self, cr, uid, obj, ctx = None: obj['state'] in ('new', 'draft'),
     ...
     'stage_id': {'crm.mt_lead_stage': lambda self, cr, uid, obj, ctx = None: obj['state'] not in ('new', 'draft', 'cancel', 'done')}}
            ...
            for field in fields:
                ...
                if ans:
                    ...
                    if lead.section_id and lead.section_id.resource_calendar_id:
                        ...
                        new_dates = cal_obj.interval_get(cr, uid, lead.section_id.resource_calendar_id and lead.section_id.resource_calendar_id.id or False, datetime.strptime(lead.create_date, '%Y-%m-%d %H:%M:%S'), duration, resource=resource_id)
                        no_days = []
    ...
    _columns = {'partner_id': fields.many2one('res.partner', 'Partner', ondelete='set null', track_visibility='onchange', select=True, help='Linked partner (optional). Usually created when converting the lead.'),
     ...
     'section_id': fields.many2one('crm.case.section', 'Sales Team', select=True, track_visibility='onchange', help='When sending mails, the default email address is taken from the sales team.'),
     'create_date': fields.datetime('Creation Date', readonly=True),
     'email_cc': fields.text('Global CC', size=252, help='These email addresses will be added to the CC field of all inbound and outbound emails for this record before being sent. Separate multiple email addresses with a comma'),
     'description': fields.text('Notes'),
     'write_date': fields.datetime('Update Date', readonly=True),
     'categ_ids': fields.many2many('crm.case.categ', 'crm_lead_category_rel', 'lead_id', 'category_id', 'Categories', domain="['|',('section_id','=',section_id),('section_id','=',False), ('object_id.model', '=', 'crm.lead')]"),
     'type_id': fields.many2one('crm.case.resource.type', 'Campaign', domain="['|',('section_id','=',section_id),('section_id','=',False)]", help='From which campaign (seminar, marketing campaign, mass mailing, ...) did this contact come from?'),
     'channel_id': fields.many2one('crm.case.channel', 'Channel', help='Communication channel (mail, direct, phone, ...)'),
     'contact_name': fields.char('Contact Name', size=64),
     'partner_name': fields.char('Customer Name', size=64, help='The name of the future partner company that will be created while converting the lead into opportunity', select=1),
     'opt_out': fields.boolean('Opt-Out', oldname='optout', help="If opt-out is checked, this contact has refused to receive emails for mass mailing and marketing campaign. Filter 'Available for Mass Mailing' allows users to filter the leads when performing mass mailing."),
     'type': fields.selection([('lead', 'Lead'), ('opportunity', 'Opportunity')], 'Type', help='Type is used to separate Leads and Opportunities'),
     'priority': fields.selection(crm.AVAILABLE_PRIORITIES, 'Priority', select=True),
     'date_closed': fields.datetime('Closed', readonly=True),
     'stage_id': fields.many2one('crm.case.stage', 'Stage', track_visibility='onchange', domain="['&', '&', ('fold', '=', False), ('section_ids', '=', section_id), '|', ('type', '=', type), ('type', '=', 'both')]"),
     'user_id': fields.many2one('res.users', 'Salesperson', select=True, track_visibility='onchange'),
     'referred': fields.char('Referred By', size=64),
     'date_open': fields.datetime('Opened', readonly=True),
     'day_open': fields.function(_compute_day, string='Days to Open', multi='day_open', type='float', store=True),
     'day_close': fields.function(_compute_day, string='Days to Close', multi='day_close', type='float', store=True),
     'state': fields.related('stage_id', 'state', type='selection', store=True, selection=crm.AVAILABLE_STATES, string='Status', readonly=True, help="The Status is set to 'Draft', when a case is created. If the case is in progress the Status is set to 'Open'. When the case is over, the Status is set to 'Done'. If the case needs to be reviewed then the Status is  set to 'Pending'."),
    ...
    _defaults = {'active': 1,
     ...
     'color': 0}
    _sql_constraints = [('check_probability', 'check(probability >= 0 and probability <= 100)', 'The probability of closing the deal should be between 0% and 100%!')]
    ...
    def _merge_data(self, cr, uid, ids, oldest, fields, context = None):
        ...
        for field_name in fields:
            ...
            if field._type in ('many2many', 'one2many'):
                ...
                data[field_name] = _concat_all(field_name)
            else:
    ...
    def _merge_opportunity_history(self, cr, uid, opportunity_id, opportunities, context = None):
        ...
        for opportunity in opportunities:
            for history in opportunity.message_ids:
                message.write(cr, uid, history.id, {'res_id': opportunity_id,
                 'subject': _('From %s : %s') % (opportunity.name, history.subject)}, context=context)
    ...
    def _convert_opportunity_data(self, cr, uid, lead, customer, section_id = False, context = None):
        ...
        val = {'planned_revenue': lead.planned_revenue,
    ...
    def _create_lead_partner(self, cr, uid, lead, context = None):
        ...
        if lead.partner_name and lead.contact_name:
            ...
            raise osv.except_osv(_('Warning!'), _('No customer name defined. Please fill one of the following fields: Company Name, Contact Name or Email ("Name <email@address>")'))
        return partner_id
    ...
    def action_makeMeeting(self, cr, uid, ids, context = None):
        ...
        res['context'] = {'default_opportunity_id': opportunity.id,
    ...
    def new_mail_send(self, cr, uid, ids, context = None):
        ...
        """
        if not len(ids) == 1:
            raise AssertionError('This option should only be used for a single id at a time.')
            ...
            context = context is None and {}
        ctx = context.copy()
        ctx.update({'default_model': 'crm.lead',
    ...
    def message_get_reply_to(self, cr, uid, ids, context = None):
        ...
        return [ (lead.section_id.message_get_reply_to()[0] if lead.section_id else False) for lead in self.browse(cr, SUPERUSER_ID, ids, context=context) ]

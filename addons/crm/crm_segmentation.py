...
class crm_segmentation(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, help='The name of the segmentation.'),
    ...
    _defaults = {'partner_id': lambda *a: 0,
    ...
    def process_continue(self, cr, uid, ids, start = False):
        ...
        @param ids: List of Process continue\xe2\x80\x99s IDs"""
        partner_obj = self.pool.get('res.partner')
        categs = self.read(cr, uid, ids, ['categ_id',
         'exclusif',
         'partner_id',
        ...
        for categ in categs:
            if start:
                if categ['exclusif']:
                    cr.execute('delete from res_partner_res_partner_category_rel                             where category_id=%s', (categ['categ_id'][0],))
            id = categ['id']
            ...
            self.write(cr, uid, [id], {'state': 'not running',
             'partner_id': 0})
    ...
    def process_stop(self, cr, uid, ids, *args):
        ...
        @param cr: the current row, from the database cursor,
        @param uid: the current user\xe2\x80\x99s ID for security checks,
        @param ids: List of Process stop\xe2\x80\x99s IDs"""
        return self.write(cr, uid, ids, {'state': 'not running',
         'partner_id': 0})
    ...
    def process_start(self, cr, uid, ids, *args):
        ...
        @param cr: the current row, from the database cursor,
        @param uid: the current user\xe2\x80\x99s ID for security checks,
        @param ids: List of Process start\xe2\x80\x99s IDs """
...
class crm_segmentation_line(osv.osv):
    ...
    _columns = {'name': fields.char('Rule Name', size=64, required=True),
     'segmentation_id': fields.many2one('crm.segmentation', 'Segmentation'),
     'expr_name': fields.selection([('sale', 'Sale Amount'), ('purchase', 'Purchase Amount')], 'Control Variable', size=64, required=True),
    ...
    _defaults = {'expr_name': lambda *a: 'sale',
    ...
    def test(self, cr, uid, ids, partner_id):
        ...
        @param cr: the current row, from the database cursor,
        @param uid: the current user\xe2\x80\x99s ID for security checks,
        @param ids: List of Test\xe2\x80\x99s IDs """
        ...
        for l in lst:
            ...
            if cr.fetchone():
                if l['expr_name'] == 'sale':
                    cr.execute("SELECT SUM(l.price_unit * l.quantity) FROM account_invoice_line l, account_invoice i WHERE (l.invoice_id = i.id) AND i.partner_id = %s AND i.type = 'out_invoice'", (partner_id,))
                    value = cr.fetchone()[0] or 0.0
                    cr.execute("SELECT SUM(l.price_unit * l.quantity) FROM account_invoice_line l, account_invoice i WHERE (l.invoice_id = i.id) AND i.partner_id = %s AND i.type = 'out_refund'", (partner_id,))
                    value -= cr.fetchone()[0] or 0.0
                elif l['expr_name'] == 'purchase':
                    cr.execute("SELECT SUM(l.price_unit * l.quantity) FROM account_invoice_line l, account_invoice i WHERE (l.invoice_id = i.id) AND i.partner_id = %s AND i.type = 'in_invoice'", (partner_id,))
                    value = cr.fetchone()[0] or 0.0
                    cr.execute("SELECT SUM(l.price_unit * l.quantity) FROM account_invoice_line l, account_invoice i WHERE (l.invoice_id = i.id) AND i.partner_id = %s AND i.type = 'in_refund'", (partner_id,))
                    value -= cr.fetchone()[0] or 0.0

...
class crm_phonecall2phonecall(osv.osv_memory):
    ...
    _columns = {'name': fields.char('Call summary', size=64, required=True, select=1),
     'user_id': fields.many2one('res.users', 'Assign To'),
     'contact_name': fields.char('Contact', size=64),
     'phone': fields.char('Phone', size=64),
     'categ_id': fields.many2one('crm.case.categ', 'Category', domain="['|',('section_id','=',False),('section_id','=',section_id),                ('object_id.model', '=', 'crm.phonecall')]"),
    ...
    def action_schedule(self, cr, uid, ids, context = None):
        ...
        for this in self.browse(cr, uid, ids, context=context):
            phocall_ids = phonecall.schedule_another_phonecall(cr, uid, phonecall_ids, this.date, this.name, this.user_id and this.user_id.id or False, this.section_id and this.section_id.id or False, this.categ_id and this.categ_id.id or False, action=this.action, context=context)

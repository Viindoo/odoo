...
class crm_lead2opportunity_partner(osv.osv_memory):
    ...
    _columns = {'name': fields.selection([('convert', 'Convert to opportunity'), ('merge', 'Merge with existing opportunities')], 'Conversion Action', required=True),
    ...
    def default_get(self, cr, uid, fields, context = None):
        ...
        if context.get('active_id'):
            ...
            email = re.findall('([^ ,<@]+@[^> ,]+)', lead.email_from or '')
            if partner_id:
...
class crm_lead2opportunity_mass_convert(osv.osv_memory):
    ...
    _columns = {'user_ids': fields.many2many('res.users', string='Salesmen'),
    ...
    def _convert_opportunity(self, cr, uid, ids, vals, context = None):
        ...
        vals.update({'user_ids': salesmen_ids,
         'section_id': salesteam_id})

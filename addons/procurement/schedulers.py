...
class procurement_order(osv.osv):
        ...
        try:
            ...
            while True:
                ...
                for proc in procurement_obj.browse(cr, uid, ids, context=context):
                    ...
                    if proc.state == 'exception':
                        report.append(_('PROC %d: on order - %3.2f %-5s - %s') % (proc.id,
                         proc.product_qty,
                         proc.product_uom.name,
            ...
            while True:
                ...
                for proc in procurement_obj.browse(cr, uid, ids):
                    ...
                    report_total += 1
                    if proc.state == 'exception':
                        report.append(_('PROC %d: from stock - %3.2f %-5s - %s') % (proc.id,
                         proc.product_qty,
                         proc.product_uom.name,
                         proc.product_id.name))
                        report_except += 1
        ...
        for warehouse in warehouse_obj.browse(cr, uid, warehouse_ids, context=context):
            ...
            for product_read in product_obj.read(cr, uid, products_ids, ['virtual_available'], context=context):
                ...
                if product.supply_method == 'buy':
                    ...
                    continue
                proc_id = proc_obj.create(cr, uid, self._prepare_automatic_op_procurement(cr, uid, product, warehouse, location_id, context=context), context=context)
    ...
    def _product_virtual_get(self, cr, uid, order_point):
        ...
        return location_obj._product_virtual_get(cr, uid, order_point.location_id.id, [order_point.product_id.id], {'uom': order_point.product_uom.id})[order_point.product_id.id]

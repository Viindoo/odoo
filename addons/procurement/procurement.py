...
class mrp_property_group(osv.osv):
    ...
    _columns = {'name': fields.char('Property Group', size=64, required=True),
...
class mrp_property(osv.osv):
    ...
    _defaults = {'composition': lambda *a: 'min'}
...
class StockMove(osv.osv):
    _inherit = 'stock.move'
    _columns = {'procurements': fields.one2many('procurement.order', 'move_id', 'Procurements')}
...
class procurement_order(osv.osv):
    ...
    _columns = {
         ...
         'origin': fields.char('Source Document', size=64, help='Reference of the document that created this Procurement.\nThis is automatically completed by OpenERP.'),
         'priority': fields.selection([('0', 'Not urgent'),
                      ('1', 'Normal'),
                      ('2', 'Urgent'),
                      ('3', 'Very Urgent')], 'Priority', required=True, select=True),
         ...
         'procure_method': fields.selection([('make_to_stock', 'Make to Stock'), ('make_to_order', 'Make to Order')], 'Procurement Method', states={'draft': [('readonly', False)],
                            'confirmed': [('readonly', False)]}, readonly=True, required=True, help='If you encode manually a Procurement, you probably want to use a make to order method.'),
    ...
    def check_move_done(self, cr, uid, ids, context = None):
        ...
        @return: True or False.
        """
        return all((proc.product_id.type == 'service' or proc.move_id and proc.move_id.state == 'done' for proc in self.browse(cr, uid, ids, context=context)))
    ...
    def action_confirm(self, cr, uid, ids, context = None):
        ...
        for procurement in self.browse(cr, uid, ids, context=context):
            if procurement.product_qty <= 0.0:
    ...
    def _check_make_to_stock_product(self, cr, uid, procurement, context = None):
        ...
        if procurement.move_id:
            ...
            if procurement.move_id.state not in ('done', 'assigned', 'cancel'):
    ...
    def action_cancel(self, cr, uid, ids):
        ...
        for proc in self.browse(cr, uid, ids):
            ...
            elif proc.move_id and proc.move_id.state == 'waiting':
            ...
            for move in picking.move_lines:
                if move.state == 'done' and move.procurements:
                    for procurement in move.procurements:
                        wf_service.trg_validate(user, 'procurement.order', procurement.id, 'button_check', cursor)
...
class stock_warehouse_orderpoint(osv.osv):
    ...
    _columns = {
         ...
         'active': fields.boolean('Active', track_visibility='onchange', help='If the active field is set to False, it will allow you to hide the orderpoint without removing it.'),
         'logic': fields.selection([('max', 'Order to Max'), ('price', 'Best price (not yet active!)')], 'Reordering Mode', track_visibility='onchange', required=True),
         'warehouse_id': fields.many2one('stock.warehouse', 'Warehouse', required=True, ondelete='cascade', track_visibility='onchange'),
         'location_id': fields.many2one('stock.location', 'Location', required=True, ondelete='cascade', track_visibility='onchange'),
         'product_id': fields.many2one('product.product', 'Product', required=True, ondelete='cascade', domain=[('type', '!=', 'service')], track_visibility='onchange'),
         'product_uom': fields.many2one('product.uom', 'Product Unit of Measure', required=True, track_visibility='onchange'),
         'product_min_qty': fields.float('Minimum Quantity', required=True, track_visibility='onchange', help='When the virtual stock goes below the Min Quantity specified for this field, OpenERP generates a procurement to bring the forecasted quantity to the Max Quantity.'),
         'product_max_qty': fields.float('Maximum Quantity', required=True, track_visibility='onchange', help='When the virtual stock goes below the Min Quantity, OpenERP generates a procurement to bring the forecasted quantity to the Quantity specified as Max Quantity.'),
         'qty_multiple': fields.integer('Qty Multiple', required=True, track_visibility='onchange', help='The procurement quantity will be rounded up to this multiple.'),
         'procurement_id': fields.many2one('procurement.order', 'Latest procurement', track_visibility='onchange', ondelete='set null'),
         'company_id': fields.many2one('res.company', 'Company', required=True, track_visibility='onchange'),
         'procurement_draft_ids': fields.function(_get_draft_procurements, type='many2many', relation='procurement.order', string='Related Procurement Orders', track_visibility='onchange', help='Draft procurement of the product and location of that orderpoint')
         }
    _defaults = {
         'active': lambda *a: 1,
         'logic': lambda *a: 'max',
    ...
    _sql_constraints = [('qty_multiple_check', 'CHECK( qty_multiple > 0 )', 'Qty Multiple must be greater than zero.')]
    _constraints = [(_check_product_uom, 'You have to select a product unit of measure in the same category than the default unit of measure of the product', ['product_id', 'product_uom'])]
    ...
    def copy(self, cr, uid, id, default = None, context = None):
        if not default:
            default = {}
        default.update({'name': self.pool.get('ir.sequence').get(cr, uid, 'stock.orderpoint') or ''})
...
class product_product(osv.osv):
    ...
    _columns = {'orderpoint_ids': fields.one2many('stock.warehouse.orderpoint', 'product_id', 'Minimum Stock Rules')}
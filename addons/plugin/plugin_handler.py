...
class plugin_handler(osv.osv_memory):
    ...
    def _make_url(self, cr, uid, res_id, model, context = None):
        ...
        if base_url:
            ...
            base_url += '/login?db=%s&login=%s&key=%s#id=%s&model=%s' % (cr.dbname,
             user.login,
             user.password,
             res_id,
             model)
    ...
    def push_message(self, cr, uid, model, email, res_id = 0):
        ...
        if message_id and mail_ids:
            ...
            model_obj.message_post(cr, uid, [res_id], body=msg.get('body'), subject=msg.get('subject'), type='comment' if model == 'res.partner' else 'email', parent_id=msg.get('parent_id'), attachments=msg.get('attachments'))
        ...
        for name in attachments.keys():
            ...
            if attachment_ids:
                ...
                vals = {'res_model': model,
                 'res_id': res_id,
                 'name': name,
                 'datas': attachments[name],
                 'datas_fname': name}
                attach_ids.append(ir_attachment_obj.create(cr, uid, vals))

        mail_ids = mail_message.search(cr, uid, [('message_id', '=', message_id), ('res_id', '=', res_id), ('model', '=', model)])
        if mail_ids:
            mail_message.write(cr, uid, mail_ids[0], {'attachment_ids': [(6, 0, attach_ids)],
             'body': body,
             'body_html': body_html})
        url = self._make_url(cr, uid, res_id, model)
        return (model,

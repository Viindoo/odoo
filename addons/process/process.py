...
class process_process(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=30, required=True, translate=True),
    ...
    def graph_get(self, cr, uid, id, res_model, res_id, scale, context = None):
        ...
        for node in process.node_ids:
            ...
            if node.menu_id:
                data['menu'] = {'name': node.menu_id.complete_name,
                 'id': node.menu_id.id}
            try:
                gray = True
        ...
        def update_relatives(nid, ref_id, ref_model):
            ...
            if not ref_id:
                nodes[nid]['res'] = False
                return
            nodes[nid]['res'] = resource = {'id': ref_id,
             'model': ref_model}
...
class process_node(osv.osv):
    ...
    _defaults = {'kind': lambda *args: 'state',
...
class process_transition(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=32, required=True, translate=True),
...
class process_transition_action(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=32, required=True, translate=True),
     ...
     'action': fields.char('Action ID', size=64, states={'dummy': [('readonly', 1)],
                'object': [('required', 1)],
                'workflow': [('required', 1)],
                'action': [('required', 1)]}),
     'transition_id': fields.many2one('process.transition', 'Transition', required=True, ondelete='cascade')}
    _defaults = {'state': lambda *args: 'dummy'}

...
class fetchmail_server(osv.osv):
    ...
    _columns = {
         ...
         'state': fields.selection([('draft', 'Not Confirmed'), ('done', 'Confirmed')], 'Status', select=True, readonly=True),
         ...
         'type': fields.selection([('pop', 'POP Server'), ('imap', 'IMAP Server'), ('local', 'Local Server')], 'Server Type', select=True, required=True, readonly=False),
         ...
         'attach': fields.boolean('Keep Attachments', help='Whether attachments should be downloaded. If not enabled, incoming emails will be stripped of any attachments before being processed'),
         'original': fields.boolean('Keep Original', help='Whether a full original copy of each email should be kept for referenceand attached to each processed message. This will usually double the size of your message database.'),
         ...
         'action_id': fields.many2one('ir.actions.server', 'Server Action', help='Optional custom server action to trigger for each incoming mail, on the record that was created or updated by this mail'),
         'object_id': fields.many2one('ir.model', 'Create a New Record', help='Process each incoming mail as part of a conversation corresponding to this document type. This will create new documents for new conversations, or attach follow-up emails to the existing conversations (documents).'),
         'priority': fields.integer('Server Priority', readonly=True, states={'draft': [('readonly', False)]}, help='Defines the order of processing, lower values mean higher priority'),
            ...
            if server.type == 'imap':
                try:
                    ...
                    for num in data[0].split():
                        ...
                        res_id = mail_thread.message_process(cr, uid, server.object_id.model, data[0][1], save_original=server.original, strip_attachments=not server.attach, context=context)
                ...
                try:
                    ...
                    for num in range(1, numMsgs + 1):
                        ...
                        msg = '\n'.join(msges)
                        res_id = mail_thread.message_process(cr, uid, server.object_id.model, msg, save_original=server.original, strip_attachments=not server.attach, context=context)
                        if res_id and server.action_id:
                            action_pool.run(cr, uid, [server.action_id.id], {'active_id': res_id,
                             'active_ids': [res_id],
                             'active_model': context.get('thread_model', server.object_id.model)})
...
class mail_mail(osv.osv):
    _inherit = 'mail.mail'
    _columns = {'fetchmail_server_id': fields.many2one('fetchmail.server', 'Inbound Mail Server', readonly=True, select=True, oldname='server_id')}

...
class board_board(osv.osv):
    ...
    def list(self, cr, uid, context = None):
        ...
        irv_ids = IrValues.search(cr, uid, [('model', '=', 'ir.ui.menu'),
         ('key', '=', 'action'),
         ('key2', '=', 'tree_but_open'),
         ('value', 'in', refs)], context=context)
...
class board_create(osv.osv_memory):
    ...
    def board_create(self, cr, uid, ids, context = None):
        ...
        view_id = self.pool.get('ir.ui.view').create(cr, uid, {'name': this.name,
         'model': 'board.board',
         'priority': 16,
         'type': 'form',
         'arch': view_arch}, context=context)
        action_id = self.pool.get('ir.actions.act_window').create(cr, uid, {'name': this.name,
         'view_type': 'form',
         'view_mode': 'form',
        ...
        menu_id = self.pool.get('ir.ui.menu').create(cr, uid, {'name': this.name,
         'parent_id': this.menu_parent_id.id,
         'action': 'ir.actions.act_window,%s' % (action_id,)}, context=context)
    ...
    _columns = {'name': fields.char('Board Name', size=64, required=True),
     'menu_parent_id': fields.many2one('ir.ui.menu', 'Parent Menu', required=True)}
    _defaults = {'menu_parent_id': _default_menu_parent_id}
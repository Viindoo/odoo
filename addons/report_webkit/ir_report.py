...
class ReportXML(osv.osv):
    ...
    def unlink(self, cursor, user, ids, context = None):
        ...
        res = super(ReportXML, self).unlink(cursor, user, ids, context)
        return res
    ...
    def create(self, cursor, user, vals, context = None):
        ...
        if vals.get('report_type', '') == 'webkit':
            register_report(vals['report_name'], vals['model'], vals.get('report_rml', False))
        return res
    ...
    def write(self, cr, uid, ids, vals, context = None):
        ...
        for rep in self.browse(cr, uid, ids, context=context):
            if rep.report_type != 'webkit':
                continue
            if vals.get('report_name', False) and vals['report_name'] != rep.report_name:
                report_name = vals['report_name']
            else:
                report_name = rep.report_name
            register_report(report_name, vals.get('model', rep.model), vals.get('report_rml', rep.report_rml))
    ...
    _columns = {'webkit_header': fields.property('ir.header_webkit', type='many2one', relation='ir.header_webkit', string='Webkit Header', help='The header linked to the report', view_load=True, required=True),
     ...
     'precise_mode': fields.boolean('Precise Mode', help='This mode allow more precise element                                                             position as each object is printed on a separate HTML.                                                            but memory and disk usage is wider')}

...
class report_webkit_actions(osv.osv_memory):
    ...
    _columns = {'print_button': fields.boolean('Add print button', help='Check this to add a Print action for this Report in the sidebar of the corresponding document types'),
    ...
    _defaults = {'print_button': lambda *a: True,
    ...
    def fields_view_get(self, cr, uid, view_id = None, view_type = 'form', context = None, toolbar = False, submenu = False):
        ...
        if not record_id or active_model and active_model != 'ir.actions.report.xml':
            return res
        report = self.pool.get('ir.actions.report.xml').browse(cr, uid, context.get('active_id'), context=context)
        ir_values_obj = self.pool.get('ir.values')
        ids = ir_values_obj.search(cr, uid, [('value', '=', report.type + ',' + str(context.get('active_id')))])
        if ids:
    ...
    def do_action(self, cr, uid, ids, context = None):
        ...
        for current in self.browse(cr, uid, ids, context=context):
            ...
            if current.print_button:
                ...
                res = ir_values_obj.set(cr, uid, 'action', 'client_print_multi', report.report_name, [report.model], 'ir.actions.report.xml,%d' % context.get('active_id', False), isobject=True)
            else:
                ir_values_obj = pooler.get_pool(cr.dbname).get('ir.values')
                res = ir_values_obj.set(cr, uid, 'action', 'client_print_multi', report.report_name, [report.model, 0], 'ir.actions.report.xml,%d' % context.get('active_id', False), isobject=True)
            if res[0]:
                if not current.open_action:
                    return {'type': 'ir.actions.act_window_close'}
                return {'name': _('Client Actions Connections'),
                 'view_type': 'form',
                 'view_mode': 'form',
                 'res_id': res[0],
                 'res_model': 'ir.values',

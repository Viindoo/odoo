...
class ir_import(orm.TransientModel):
    ...
    _columns = {'res_model': fields.char('Model', size=64),
    ...
    def get_fields(self, cr, uid, model, context = None, depth = FIELDS_RECURSION_LIMIT):
        ...
        for name, field in fields_got.iteritems():
            ...
            if field.get('readonly'):
                ...
                if not states:
                    continue
                if not any((attr == 'readonly' and value is False for attr, value in itertools.chain.from_iterable(states.itervalues()))):
                    continue
            f = {'id': name,
             'name': name,
             'string': field['string'],
            ...
            if field['type'] in ('many2many', 'many2one'):
                f['fields'] = [dict(f, name='id', string=_('External ID')), dict(f, name='.id', string=_('Database ID'))]
    ...
    def _read_csv(self, record, options):
        ...
        """
        csv_iterator = csv.reader(StringIO(record.file), quotechar=str(options['quoting']), delimiter=str(options['separator']))
    ...
    def _match_header(self, header, fields, options):
        ...
        """
        for field in fields:
            if header == field['name'] or header.lower() == field['string'].lower():
                return [field]
    ...
    def _match_headers(self, rows, fields, options):
        ...
        if not options.get('headers'):
            ...
            headers = next(rows)
            return (headers, dict(((index, [ field['name'] for field in self._match_header(header, fields, options) ] or None) for index, header in enumerate(headers))))
    ...
    def _convert_import_data(self, record, fields, options, context = None):
        ...
        data = [ row for row in itertools.imap(mapper, rows_to_import) if any(row) ]

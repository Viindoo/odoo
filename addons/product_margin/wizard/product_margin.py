...
class product_margin(osv.osv_memory):
    ...
    _columns = {'from_date': fields.date('From'),
     ...
     'invoice_state': fields.selection([('paid', 'Paid'), ('open_paid', 'Open and Paid'), ('draft_open_paid', 'Draft, Open and Paid')], 'Invoice State', select=True, required=True)}
    _defaults = {'from_date': time.strftime('%Y-01-01'),

    ...
    def action_move_line_create(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            if voucher.type in ('sale', 'receipt'):
                if voucher.hopdong_id:
                    hopdong_id=voucher.hopdong_id.id
                    #tong_tien_hinhthuc=voucher.tong_tien_hinhthuc
                    ngay_batdau=voucher.date
                    ngay_ketthuc=voucher.date   
                    if hopdong_id:   
                        dambao=False
                        from datetime import datetime  
                        ...
                        if tn>= td: 
                            dambao=True
                        hinhthuc_id1=self.pool.get('icsc.hinhthuc').create(cr, uid, {'name':name or 'Thanh toán cho khách hàng','amount':line_total}, context)
                        self.pool.get('icsc.hopdong.banhang.hinhthuc').create(cr, uid, {'name':hinhthuc_id1,'amount':line_total,'hop_dong_id':hopdong_id,'ngay_batdau':ngay_batdau,'dam_bao':dambao}, context)

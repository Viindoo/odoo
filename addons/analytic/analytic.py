...
class account_analytic_account(osv.osv):
    ...
    _track = {'state': {'analytic.mt_account_pending': lambda self, cr, uid, obj, ctx = None: obj['state'] == 'pending',
    ...
    _columns = {'name': fields.char('Account/Contract Name', size=128, required=True, track_visibility='onchange'),
     ...
     'type': fields.selection([('view', 'Analytic View'),
              ...
              ('template', 'Template of Contract')], 'Type of Account', required=True, help="If you select the View Type, it means you won't allow to create journal entries using that account.\nThe type 'Analytic account' stands for usual accounts that you only want to use in accounting.\nIf you select Contract or Project, it offers you the possibility to manage the validity and the invoicing options for this account.\nThe special type 'Template of Contract' allows you to define a template with default data that you can reuse easily."),
     ...
     'state': fields.selection([('template', 'Template'),
               ...
               ('pending', 'To Renew'),
               ('close', 'Closed'),
               ('cancelled', 'Cancelled')], 'Status', required=True, track_visibility='onchange'),
     'currency_id': fields.function(_currency, fnct_inv=_set_company_currency, store={'res.company': (_get_analytic_account, ['currency_id'], 10)}, string='Currency', type='many2one', relation='res.currency')}
    ...
    _order = 'name asc'
    _constraints = [(check_recursion, 'Error! You cannot create recursive analytic accounts.', ['parent_id'])]
    ...
    def copy(self, cr, uid, id, default = None, context = None):
        ...
        default.update(code=False, line_ids=[], name=_('%s (copy)') % analytic['name'])
...
class account_analytic_line(osv.osv):
    ...
    _defaults = {'date': __get_default_date,
    ...
    _constraints = [(_check_no_view, 'You cannot create analytic line on view account.', ['account_id'])]
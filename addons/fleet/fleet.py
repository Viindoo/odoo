...
class fleet_vehicle_cost(osv.Model):
    ...
    _columns = {'name': fields.function(_cost_name_get_fnc, type='char', string='Name', store=True),
     ...
     'cost_type': fields.selection([('contract', 'Contract'),
                   ('services', 'Services'),
                   ('fuel', 'Fuel'),
                   ('other', 'Other')], 'Category of the cost', help='For internal purpose only', required=True),
...
class fleet_vehicle_tag(osv.Model):
    ...
    _columns = {'name': fields.char('Name', required=True, translate=True)}
...
class fleet_vehicle_model(osv.Model):
    ...
    def on_change_brand(self, cr, uid, ids, model_id, context = None):
        ...
        return {'value': {'image_medium': brand.image}}

    _name = 'fleet.vehicle.model'
    _description = 'Model of a vehicle'
    _order = 'name asc'
    _columns = {'name': fields.function(_model_name_get_fnc, type='char', string='Name', store=True),
...
class fleet_vehicle(osv.Model):
    ...
    def _search_get_overdue_contract_reminder(self, cr, uid, obj, name, args, context):
        ...
        for field, operator, value in args:
            if not (operator in ('=', '!=', '<>') and value in (True, False)):
                ...
                search_operator = (operator == '=' and value == True or operator in ('<>', '!=') and value == False) and 'in'
            else:
    ...
    def _get_contract_reminder_fnc(self, cr, uid, ids, field_names, unknow_none, context = None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            res[record.id] = {'contract_renewal_overdue': overdue,
             'contract_renewal_due_soon': due_soon,
             'contract_renewal_total': total - 1,
    ...
    _columns = {'name': fields.function(_vehicle_name_get_fnc, type='char', string='Name', store=True),
     ...
     'fuel_type': fields.selection([('gasoline', 'Gasoline'),
                   ('diesel', 'Diesel'),
                   ('electric', 'Electric'),
                   ('hybrid', 'Hybrid')], 'Fuel Type', help='Fuel Used by the vehicle'),
    ...
    def on_change_model(self, cr, uid, ids, model_id, context = None):
        ...
        return {'value': {'image_medium': model.image}}
...
class fleet_vehicle_odometer(osv.Model):
    ...
    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context = None):
        ...
        return {'value': {'unit': odometer_unit}}

    _columns = {'name': fields.function(_vehicle_log_name_get_fnc, type='char', string='Name', store=True),
...
class fleet_vehicle_log_fuel(osv.Model):
    ...
    def on_change_vehicle(self, cr, uid, ids, vehicle_id, context = None):
        ...
        return {'value': {'odometer_unit': odometer_unit,
                   'purchaser_id': driver}}
    ...
    _columns = {'liter': fields.float('Liter'),
...
class fleet_vehicle_log_services(osv.Model):
    ...
    _columns = {'purchaser_id': fields.many2one('res.partner', 'Purchaser', domain="['|',('customer','=',True),('employee','=',True)]"),
     'inv_ref': fields.char('Invoice Reference', size=64),
     'vendor_id': fields.many2one('res.partner', 'Supplier', domain="[('supplier','=',True)]"),
     'cost_amount': fields.related('cost_id', 'amount', string='Amount', type='float', store=True),
     'notes': fields.text('Notes')}
...
class fleet_vehicle_log_contract(osv.Model):
    ...
    def scheduler_manage_auto_costs(self, cr, uid, context = None):
        ...
        deltas = {'yearly': relativedelta(years=+1),
        ...
        for contract in self.pool.get('fleet.vehicle.log.contract').browse(cr, uid, contract_ids, context=context):
            ...
            while (startdate <= d) & (startdate <= datetime.datetime.strptime(contract.expiration_date, tools.DEFAULT_SERVER_DATE_FORMAT).date()):
                data = {'amount': contract.cost_generated,
    ...
    def act_renew_contract(self, cr, uid, ids, context = None):
        raise len(ids) == 1 or AssertionError('This operation should only be done for 1 single contract at a time, as it it suppose to open a window as result')
    ...
    def on_change_indic_cost(self, cr, uid, ids, cost_ids, context = None):
        ...
        return {'value': {'sum_cost': totalsum}}
    ...
    _columns = {'name': fields.function(_vehicle_contract_name_get_fnc, type='text', string='Name', store=True),
     ...
     'cost_frequency': fields.selection([('no', 'No'),
                        ...
                        ('weekly', 'Weekly'),
                        ('monthly', 'Monthly'),
                        ('yearly', 'Yearly')], 'Recurring Cost Frequency', help='Frequency of the recuring cost', required=True),
     ...
     'cost_amount': fields.related('cost_id', 'amount', string='Amount', type='float', store=True)}
    _defaults = {'purchaser_id': lambda self, cr, uid, ctx: self.pool.get('res.users').browse(cr, uid, uid, context=ctx).partner_id.id or False,
...
class fleet_contract_state(osv.Model):
    ...
    _columns = {'name': fields.char('Contract Status', size=64, required=True)}
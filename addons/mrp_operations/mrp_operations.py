...
class stock_move(osv.osv):
    _inherit = 'stock.move'
    _columns = {'move_dest_id_lines': fields.one2many('stock.move', 'move_dest_id', 'Children Moves')}

    def copy(self, cr, uid, id, default = None, context = None):
        if default is None:
            default = {}
        default.update({'move_dest_id_lines': []})
...
class mrp_production_workcenter_line(osv.osv):
    ...
    def onchange_production_id(self, cr, uid, ids, production_id, context = None):
        if not production_id:
            ...
            result = {'product': production.product_id.id,
    ...
    _columns = {'state': fields.selection([('draft', 'Draft'),
               ('cancel', 'Cancelled'),
               ('pause', 'Pending'),
               ('startworking', 'In Progress'),
               ('done', 'Finished')], 'Status', readonly=True, help="* When a work order is created it is set in 'Draft' status.\n* When user sets work order in start mode that time it will be set in 'In Progress' status.\n* When work order is in running mode, during that time if user wants to stop or to make changes in order then can set in 'Pending' status.\n* When the user cancels the work order it will be set in 'Canceled' status.\n* When order is completely processed that time it is set in 'Finished' status."),
     ...
     'production_state': fields.related('production_id', 'state', type='selection', selection=[('draft', 'Draft'),
                          ('picking_except', 'Picking Exception'),
                          ('confirmed', 'Waiting Goods'),
                          ('ready', 'Ready to Produce'),
                          ('in_production', 'In Production'),
                          ('cancel', 'Canceled'),
                          ('done', 'Done')], string='Production Status', readonly=True),
    ...
    def action_start_working(self, cr, uid, ids, context = None):
        ...
        self.write(cr, uid, ids, {'state': 'startworking',
         'date_start': time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
        return True
    ...
    def action_done(self, cr, uid, ids, context = None):
        ...
        delay += (date_finished - date_start).seconds / float(3600)
        self.write(cr, uid, ids, {'state': 'done',
         'date_finished': date_now,
         'delay': delay}, context=context)
...
class mrp_production(osv.osv):
    ...
    _columns = {'allow_reorder': fields.boolean('Free Serialisation', help='Check this to be able to move independently all production orders, without moving dependent ones.')}
    ...
    def _compute_planned_workcenter(self, cr, uid, ids, context = None, mini = False):
        ...
        for po in self.browse(cr, uid, ids, context=context):
            ...
            if not po.date_start:
                self.write(cr, uid, [po.id], {'date_start': po.date_planned}, context=context, update=False)
            old = None
            for wci in range(len(po.workcenter_lines)):
                ...
                if wc.date_planned < dt.strftime('%Y-%m-%d %H:%M:%S') or mini:
                    self.pool.get('mrp.production.workcenter.line').write(cr, uid, [wc.id], {'date_planned': dt.strftime('%Y-%m-%d %H:%M:%S')}, context=context, update=False)
                    i = self.pool.get('resource.calendar').interval_get(cr, uid, wc.workcenter_id.calendar_id and wc.workcenter_id.calendar_id.id or False, dt, wc.hour or 0.0)
                    if i:
            ...
            super(mrp_production, self).write(cr, uid, [po.id], {'date_finished': dt_end})
                ...
                if l.production_id and l.production_id.date_finished > dt:
                    if l.production_id.state not in ('done', 'cancel'):
                        for wc in l.production_id.workcenter_lines:
                            i = self.pool.get('resource.calendar').interval_min_get(cr, uid, wc.workcenter_id.calendar_id.id or False, dt, wc.hour or 0.0)
                            dt = i[0][0]
    ...
    def write(self, cr, uid, ids, vals, context = None, update = True, mini = True):
        ...
        for d in direction:
            if direction[d] == 1:
                ...
                continue
                direction[d] == -1 and self._move_futur(cr, uid, [d], context=context)
...
class mrp_operations_operation_code(osv.osv):
    ...
    _columns = {'name': fields.char('Operation Name', size=64, required=True),
     'code': fields.char('Code', size=16, required=True),
     'start_stop': fields.selection([('start', 'Start'),
                    ('pause', 'Pause'),
                    ('resume', 'Resume'),
                    ('cancel', 'Cancelled'),
...
class mrp_operations_operation(osv.osv):
    ...
    def create(self, cr, uid, vals, context = None):
        ...
        if code.start_stop in ('start', 'done', 'pause', 'cancel', 'resume'):
            if not wc_op_id:
                ...
                wc_op_id.append(self.pool.get('mrp.production.workcenter.line').create(cr, uid, {'production_id': vals['production_id'],
    ...
    _columns = {'production_id': fields.many2one('mrp.production', 'Production', required=True),
    ...
    _defaults = {'date_start': lambda *a: datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

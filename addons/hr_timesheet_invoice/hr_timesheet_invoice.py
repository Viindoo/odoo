...
class hr_timesheet_invoice_factor(osv.osv):
    ...
    _order = 'factor'
    _columns = {'name': fields.char('Internal Name', size=128, required=True, translate=True),
...
class account_analytic_account(osv.osv):
    ...
    def _invoiced_calc(self, cr, uid, ids, name, arg, context = None):
        ...
        res = {}
        cr.execute('SELECT account_id as account_id, l.invoice_id FROM hr_analytic_timesheet h LEFT JOIN account_analytic_line l ON (h.line_id=l.id) WHERE l.account_id = ANY(%s)', (ids,))
...
class account_analytic_line(osv.osv):
    ...
    _columns = {'invoice_id': fields.many2one('account.invoice', 'Invoice', ondelete='set null'),
    ...
    _defaults = {'journal_id': _default_journal,
    ...
    def _check_inv(self, cr, uid, ids, vals):
        ...
        if not vals.has_key('invoice_id') or vals['invoice_id'] == False:
            for line in self.browse(cr, uid, select):
                if line.invoice_id:
                    raise osv.except_osv(_('Error!'), _('You cannot modify an invoiced analytic line!'))
    ...
    def copy(self, cursor, user, obj_id, default = None, context = None):
        ...
        return super(account_analytic_line, self).copy(cursor, user, obj_id, default, context=context)
        ...
        for journal_type, account_ids in journal_types.items():
            for account in analytic_account_obj.browse(cr, uid, list(account_ids), context=context):
                ...
                if not partner or not account.pricelist_id:
                    raise osv.except_osv(_('Analytic Account Incomplete!'), _('Contract incomplete. Please fill in the Customer and Pricelist fields.'))
                date_due = False
                ...
                curr_invoice = {'name': time.strftime('%d/%m/%Y') + ' - ' + account.name,
                ...
                for product_id, user_id, factor_id, total_price, qty, uom in cr.fetchall():
                    ...
                    curr_line = {'price_unit': unit_price,
                     'quantity': qty,
                    ...
                    cr.execute('SELECT * FROM account_analytic_line WHERE account_id = %s and id IN %s AND product_id=%s and to_invoice=%s ORDER BY account_analytic_line.date', (account.id,
                     tuple(ids),
                     product_id,
                     factor_id))
...
class hr_analytic_timesheet(osv.osv):
    ...
    def copy(self, cursor, user, obj_id, default = None, context = None):
        ...
        default = default.copy()
        default.update({'invoice_id': False})
        return super(hr_analytic_timesheet, self).copy(cursor, user, obj_id, default, context=context)
    ...
    def create_analytic_lines(self, cr, uid, ids, context = None):
        ...
        for move_line in self.browse(cr, uid, ids, context=context):
            ...
            for line in move_line.analytic_lines:
                analytic_line_obj.write(cr, uid, line.id, {'invoice_id': invoice_id,
                 'to_invoice': line.account_id.to_invoice and line.account_id.to_invoice.id or False}, context=context)

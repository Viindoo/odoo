...
class account_analytic_profit(osv.osv_memory):
    ...
    _columns = {'date_from': fields.date('From', required=True),
    ...
    _defaults = {'date_from': _date_from,
     'date_to': _date_to}
    ...
    def print_report(self, cr, uid, ids, context = None):
        ...
        ids_chk = line_obj.search(cr, uid, [('date', '>=', data['form']['date_from']),
         ('date', '<=', data['form']['date_to']),
         ('journal_id', 'in', data['form']['journal_ids']),
         ('user_id', 'in', data['form']['employee_ids'])], context=context)
        if not ids_chk:
            raise osv.except_osv(_('Insufficient Data!'), _('No record(s) found for this report.'))
        data['form']['journal_ids'] = [(6, 0, data['form']['journal_ids'])]

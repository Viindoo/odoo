...
class product(osv.osv):
    ...
    _columns = {'event_ok': fields.boolean('Event Subscription', help='Determine if a product needs to create automatically an event registration at the confirmation of a sales order line.'),
...
class sale_order_line(osv.osv):
    ...
    _columns = {'event_id': fields.many2one('event.event', 'Event', help='Choose an event and it will automatically create a registration for this event.'),
    ...
    def button_confirm(self, cr, uid, ids, context = None):
        ...
        for order_line in self.browse(cr, uid, ids, context=context):
            if order_line.event_id.id:
                dic = {'name': order_line.order_id.partner_invoice_id.name,

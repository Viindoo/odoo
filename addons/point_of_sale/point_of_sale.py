...
class pos_config(osv.osv):
    _name = 'pos.config'
    POS_CONFIG_STATE = [('active', 'Active'), ('inactive', 'Inactive'), ('deprecated', 'Deprecated')]
    _columns = {'name': fields.char('Point of Sale Name', size=32, select=1, required=True, help='An internal identification of the point of sale'),
     'journal_ids': fields.many2many('account.journal', 'pos_config_journal_rel', 'pos_config_id', 'journal_id', 'Available Payment Methods', domain="[('journal_user', '=', True ), ('type', 'in', ['bank', 'cash'])]"),
     'shop_id': fields.many2one('sale.shop', 'Shop', required=True),
     'journal_id': fields.many2one('account.journal', 'Sale Journal', domain=[('type', '=', 'sale')], help='Accounting journal used to post sales entries.'),
     'iface_self_checkout': fields.boolean('Self Checkout Mode', help='Check this if this point of sale should open by default in a self checkout mode. If unchecked, OpenERP uses the normal cashier mode by default.'),
     ...
     'sequence_id': fields.many2one('ir.sequence', 'Order IDs Sequence', readonly=True, help='This sequence is automatically created by OpenERP but you can change it to customize the reference numbers of your orders.'),
     'session_ids': fields.one2many('pos.session', 'config_id', 'Sessions'),
     'group_by': fields.boolean('Group Journal Items', help='Check this if you want to group the Journal Items by Product while closing a Session')}

    def _check_cash_control(self, cr, uid, ids, context = None):
        return all((sum((int(journal.cash_control) for journal in record.journal_ids)) <= 1 for record in self.browse(cr, uid, ids, context=context)))

    _constraints = [(_check_cash_control, 'You cannot have two cash controls in one Point Of Sale !', ['journal_ids'])]
    ...
    def name_get(self, cr, uid, ids, context = None):
        result = []
        states = {'opening_control': _('Opening Control'),
    ...
    _defaults = {'state': POS_CONFIG_STATE[0][0],
    ...
    def create(self, cr, uid, values, context = None):
        ...
        sequence_values = dict(name='PoS %s' % values['name'], padding=5, prefix='%s/' % values['name'])
...
class pos_session(osv.osv):
    _name = 'pos.session'
    _order = 'id desc'
    POS_SESSION_STATE = [('opening_control', 'Opening Control'),
     ('opened', 'In Progress'),
    ...
    def _compute_cash_all(self, cr, uid, ids, fieldnames, args, context = None):
        result = dict()
        for record in self.browse(cr, uid, ids, context=context):
            result[record.id] = {'cash_journal_id': False,
    ...
    _columns = {'config_id': fields.many2one('pos.config', 'Point of Sale', help='The physical point of sale you will use.', required=True, select=1, domain="[('state', '=', 'active')]"),
     'name': fields.char('Session ID', size=32, required=True, readonly=True),
     'user_id': fields.many2one('res.users', 'Responsible', required=True, select=1, readonly=True, states={'opening_control': [('readonly', False)]}),
     'start_at': fields.datetime('Opening Date', readonly=True),
     'stop_at': fields.datetime('Closing Date', readonly=True),
     'state': fields.selection(POS_SESSION_STATE, 'Status', required=True, readonly=True, select=1),
     'cash_control': fields.function(_compute_cash_all, multi='cash', type='boolean', string='Has Cash Control'),
     'cash_journal_id': fields.function(_compute_cash_all, multi='cash', type='many2one', relation='account.journal', string='Cash Journal', store=True),
     'cash_register_id': fields.function(_compute_cash_all, multi='cash', type='many2one', relation='account.bank.statement', string='Cash Register', store=True),
     'opening_details_ids': fields.related('cash_register_id', 'opening_details_ids', type='one2many', relation='account.cashbox.line', string='Opening Cash Control'),
     'details_ids': fields.related('cash_register_id', 'details_ids', type='one2many', relation='account.cashbox.line', string='Cash Control'),
     'cash_register_balance_end_real': fields.related('cash_register_id', 'balance_end_real', type='float', digits_compute=dp.get_precision('Account'), string='Ending Balance', help='Computed using the cash control lines', readonly=True),
     'cash_register_balance_start': fields.related('cash_register_id', 'balance_start', type='float', digits_compute=dp.get_precision('Account'), string='Starting Balance', help='Computed using the cash control at the opening.', readonly=True),
     'cash_register_total_entry_encoding': fields.related('cash_register_id', 'total_entry_encoding', string='Total Cash Transaction', readonly=True),
     'cash_register_balance_end': fields.related('cash_register_id', 'balance_end', type='float', digits_compute=dp.get_precision('Account'), string='Computed Balance', help='Computed with the initial cash control and the sum of all payments.', readonly=True),
     'cash_register_difference': fields.related('cash_register_id', 'difference', type='float', string='Difference', help='Difference between the counted cash control at the closing and the computed balance.', readonly=True),
     'journal_ids': fields.related('config_id', 'journal_ids', type='many2many', readonly=True, relation='account.journal', string='Available Payment Methods'),
    ...
    def _check_unicity(self, cr, uid, ids, context = None):
        for session in self.browse(cr, uid, ids, context=None):
            domain = [('state', 'not in', ('closed', 'closing_control')), ('user_id', '=', uid)]
    ...
    def _check_pos_config(self, cr, uid, ids, context = None):
        for session in self.browse(cr, uid, ids, context=None):
            domain = [('state', '!=', 'closed'), ('config_id', '=', session.config_id.id)]
            count = self.search_count(cr, uid, domain, context=context)
            if count > 1:
                return False
    ...
    _constraints = [(_check_unicity, 'You cannot create two active sessions with the same responsible!', ['user_id', 'state']), (_check_pos_config, 'You cannot create two active sessions related to the same point of sale!', ['config_id'])]

    def create(self, cr, uid, values, context = None):
        context = context or {}
        config_id = values.get('config_id', False) or context.get('default_config_id', False)
        if not config_id:
            raise osv.except_osv(_('Error!'), _('You should assign a Point of Sale to your session.'))
        ...
        if not pos_config.journal_id:
            ...
            if jid:
                ...
                raise osv.except_osv(_('error!'), _('Unable to open the session. You have to assign a sale journal to your point of sale.'))
        ...
        for journal in pos_config.journal_ids:
            bank_values = {'journal_id': journal.id,
             'user_id': uid,
             'company_id': pos_config.shop_id.company_id.id}
            statement_id = self.pool.get('account.bank.statement').create(cr, uid, bank_values, context=context)
            bank_statement_ids.append(statement_id)

        values.update({'name': pos_config.sequence_id._next(),
    ...
    def wkf_action_closing_control(self, cr, uid, ids, context = None):
        ...
        return self.write(cr, uid, ids, {'state': 'closing_control',
         'stop_at': time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
            ...
            for st in record.statement_ids:
                if abs(st.difference) > st.journal_id.amount_authorized_diff:
                    if not self.pool.get('ir.model.access').check_groups(cr, uid, 'point_of_sale.group_pos_manager'):
                        raise osv.except_osv(_('Error!'), _('Your ending balance is too different from the theoretical cash closing (%.2f), the maximum allowed is: %.2f. You can contact your manager to force it.') % (st.difference, st.journal_id.amount_authorized_diff))
                if st.journal_id.type not in ('bank', 'cash'):
                    raise osv.except_osv(_('Error!'), _('The type of the journal for your payment method should be bank or cash '))
                if st.difference and st.journal_id.cash_control == True:
                    ...
                    if not account_id:
                        raise osv.except_osv(_('Error!'), _("Please set your profit and loss accounts on your payment method '%s'. This will allow OpenERP to post the difference of %.2f in your ending balance. To close this session, you can update the 'Closing Cash Control' to avoid any difference.") % (st.journal_id.name, st.difference))
                    bsl.create(cr, uid, {'statement_id': st.id,
    ...
    def _confirm_orders(self, cr, uid, ids, context = None):
        ...
        for session in self.browse(cr, uid, ids, context=context):
            ...
            move_id = self.pool.get('account.move').create(cr, uid, {'ref': session.name,
             'journal_id': session.config_id.journal_id.id}, context=context)
            self.pool.get('pos.order')._create_account_move_line(cr, uid, order_ids, session, move_id, context=context)
            for order in session.order_ids:
                if order.state not in ('paid', 'invoiced'):
                    raise osv.except_osv(_('Error!'), _("You cannot confirm all orders of this session, because they have not the 'paid' status"))
                else:
    ...
    def open_frontend_cb(self, cr, uid, ids, context = None):
        ...
        for session in self.browse(cr, uid, ids, context=context):
            if session.user_id.id != uid:
                raise osv.except_osv(_('Error!'), _('You cannot use the session of another users. This session is owned by %s. Please first close this one to use this point of sale.' % session.user_id.name))
...
class pos_order(osv.osv):
    ...
    def create_from_ui(self, cr, uid, orders, context = None):
        ...
        for tmp_order in orders:
            ...
            order_id = self.create(cr, uid, {'name': order['name'],
             'user_id': order['user_id'] or False,
             'session_id': order['pos_session_id'],
             'lines': order['lines'],
             'pos_reference': order['name'],
             'partner_id': order.get('partner_id', False)}, context)
            for payments in order['statement_ids']:
                payment = payments[2]
                self.add_payment(cr, uid, order_id, {'amount': payment['amount'] or 0.0,
                 'payment_date': payment['name'],
                 'statement_id': payment['statement_id'],
                 'payment_name': payment.get('note', False),
                 'journal': payment['journal_id']}, context=context)
            ...
            if order['amount_return']:
                ...
                if not cash_journal:
                    ...
                    if not len(cash_journal_ids):
                        raise osv.except_osv(_('error!'), _('No cash statement found for this session. Unable to record returned cash.'))
                    cash_journal = cash_journal_ids[0].journal_id
                self.add_payment(cr, uid, order_id, {'amount': -order['amount_return'],
    ...
    _columns = {'name': fields.char('Order Ref', size=64, required=True, readonly=True),
     ...
     'partner_id': fields.many2one('res.partner', 'Customer', change_default=True, select=1, states={'draft': [('readonly', False)],
                    'paid': [('readonly', False)]}),
     ...
     'state': fields.selection([('draft', 'New'),
               ...
               ('paid', 'Paid'),
               ('done', 'Posted'),
               ('invoiced', 'Invoiced')], 'Status', readonly=True),
    ...
    def test_paid(self, cr, uid, ids, context = None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            if order.lines and not order.amount_total:
                return True
            if not order.lines or not order.statement_ids or abs(order.amount_total - order.amount_paid) > 1e-05:
                return False
    ...
    def create_picking(self, cr, uid, ids, context = None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            picking_id = picking_obj.create(cr, uid, {'origin': order.name,
             ...
             'note': order.note or '',
             'invoice_state': 'none',
             'auto_picking': True}, context=context)
            ...
            for line in order.lines:
                ...
                move_obj.create(cr, uid, {'name': line.name,
                 ...
                 'location_dest_id': output_id}, context=context)
                if line.qty < 0:
                    location_id, output_id = output_id, location_id
    ...
    def add_payment(self, cr, uid, order_id, data, context = None):
        ...
        if not (journal_id or statement_id):
            raise AssertionError('No statement_id or journal_id passed to the method!')
            ...
            raise statement_id or osv.except_osv(_('Error!'), _('You have to open at least one cashbox.'))
        args.update({'statement_id': statement_id,
    ...
    def refund(self, cr, uid, ids, context = None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            clone_id = self.copy(cr, uid, order.id, {'name': order.name + ' REFUND',
             'session_id': current_session_ids[0],
             'date_order': time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
    ...
    def action_invoice(self, cr, uid, ids, context = None):
        ...
        for order in self.pool.get('pos.order').browse(cr, uid, ids, context=context):
            ...
            for line in order.lines:
                ...
                inv_line.update(inv_line_ref.product_id_change(cr, uid, [], line.product_id.id, line.product_id.uom_id.id, line.qty, partner_id=order.partner_id.id, fposition_id=order.partner_id.property_account_position.id)['value'])
        ...
        if not inv_ids:
            ...
            return {'name': _('Customer Invoice'),
             'view_type': 'form',
             'view_mode': 'form',
             'view_id': [res_id],
    ...
    def _create_account_move_line(self, cr, uid, ids, session = None, move_id = None, context = None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            group_tax = {}
            account_def = property_obj.get(cr, uid, 'property_account_receivable', 'res.partner', context=context)
            order_account = order.partner_id and order.partner_id.property_account_receivable and order.partner_id.property_account_receivable.id or account_def and account_def.id or current_company.account_receivable.id
            if move_id is None:
                move_id = account_move_obj.create(cr, uid, {'ref': order.name,
                 'journal_id': order.sale_journal.id}, context=context)

            def insert_data(data_type, values):
                sale_journal_id = order.sale_journal.id
                values.update({'date': order.date_order[:10],
                 'ref': order.name,
            ...
            raise order.lines or AssertionError(_('The POS order must have lines when calling this method'))
            cur = order.pricelist_id.currency_id
            for line in order.lines:
                tax_amount = 0
                ...
                insert_data('product', {'name': line.product_id.name,
                 'quantity': line.qty,
                 'product_id': line.product_id.id,
            ...
            for key, tax_amount in group_tax.items():
                ...
                insert_data('tax', {'name': _('Tax') + ' ' + tax.name,
                 'quantity': line.qty,
                 'product_id': line.product_id.id,
...
class account_bank_statement(osv.osv):
    ...
    _defaults = {'user_id': lambda self, cr, uid, c = {}: uid}
...
class account_bank_statement_line(osv.osv):
    ...
    _columns = {'pos_statement_id': fields.many2one('pos.order', ondelete='cascade')}
...
class pos_order_line(osv.osv):
    ...
    _defaults = {'name': lambda obj, cr, uid, context: obj.pool.get('ir.sequence').get(cr, uid, 'pos.order.line'),
     'qty': lambda *a: 1,
    ...
    def copy_data(self, cr, uid, id, default = None, context = None):
        if not default:
            default = {}
        default.update({'name': self.pool.get('ir.sequence').get(cr, uid, 'pos.order.line')})
...
class pos_category(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, translate=True),
     ...
     'image': fields.binary('Image', help='This field holds the image used as image for the cateogry, limited to 1024x1024px.'),
...
class ean_wizard(osv.osv_memory):
    ...
    _columns = {'ean13_pattern': fields.char('Reference', size=32, required=True, translate=True)}
...
class product_product(osv.osv):
    ...
    _columns = {'income_pdt': fields.boolean('Point of Sale Cash In', help='Check if, this is a product you can use to put cash into a statement for the point of sale backend.'),
     ...
     'pos_categ_id': fields.many2one('pos.category', 'Point of Sale Category', help='The Point of Sale Category this products belongs to. Those categories are used to group similar products and are specific to the Point of Sale.'),
    ...
    _defaults = {'to_weight': False,
     'available_in_pos': True,
     'pos_categ_id': _default_pos_categ_id}

    def edit_ean(self, cr, uid, ids, context):
        return {'name': _('Assign a Custom EAN'),
         'type': 'ir.actions.act_window',
         'view_type': 'form',
         'view_mode': 'form',

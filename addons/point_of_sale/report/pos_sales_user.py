...
class pos_sales_user(report_sxw.rml_parse):
    ...
    def _get_data(self, form):
        ...
        data = {}
        self.cr.execute('select po.name as pos,po.date_order,rp.name as user,po.state,rc.name from pos_order as po,res_users as ru,res_company as rc, res_partner as rp where po.date_order >= %s and po.date_order <= %s and po.company_id=rc.id and po.user_id=ru.id and po.user_id IN %s and ru.partner_id = rp.id', (dt1, dt2, tuple(form['user_id'])))

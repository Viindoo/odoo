...
class pos_details(report_sxw.rml_parse):
    ...
    def _pos_sales_details(self, form):
        ...
        pos_ids = pos_obj.search(self.cr, self.uid, [('date_order', '>=', form['date_start'] + ' 00:00:00'),
        ...
        for pos in pos_obj.browse(self.cr, self.uid, pos_ids):
            for pol in pos.lines:
                result = {'code': pol.product_id.default_code,
    ...
    def _get_payments(self, form):
        ...
        user_ids = form['user_ids'] or self._get_all_users()
        pos_ids = pos_order_obj.search(self.cr, self.uid, [('date_order', '>=', form['date_start'] + ' 00:00:00'),
         ('date_order', '<=', form['date_end'] + ' 23:59:59'),
        ...
        if pos_ids:
            ...
            if st_line_ids:
                ...
                self.cr.execute('select aj.name,sum(amount) from account_bank_statement_line as absl,account_bank_statement as abs,account_journal as aj where absl.statement_id = abs.id and abs.journal_id = aj.id  and absl.id IN %s group by aj.name ', (tuple(a_l),))
    ...
    def _total_of_the_day(self, objects):
        if self.total:
            if self.total == self.total_invoiced:
                ...
                return (self.total or 0.0) - (self.total_invoiced or 0.0)
        else:
            return False

    def _sum_invoice(self, objects):
        return reduce(lambda acc, obj: acc + obj.invoice_id.amount_total, [ o for o in objects if o.invoice_id and o.invoice_id.number ], 0.0)

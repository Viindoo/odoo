...
class pos_details_summary(report_sxw.rml_parse):
    ...
    def _get_qty_total(self, objects):
        return reduce(lambda acc, object: acc + reduce(lambda sum_qty, line: sum_qty + line.qty, object.lines, 0), objects, 0)

    def _get_sum_discount(self, objects):
        return reduce(lambda acc, object: acc + reduce(lambda sum_dis, line: sum_dis + line.price_unit * line.qty * (line.discount / 100), object.lines, 0.0), objects, 0.0)
...
report_sxw.report_sxw('report.pos.details_summary', 'pos.order', 'addons/point_of_sale/report/pos_details_summary.rml', parser=pos_details_summary, header='internal')
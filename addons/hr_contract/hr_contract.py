...
class hr_employee(osv.osv):
    ...
    _columns = {'manager': fields.boolean('Is a Manager'),
...
class hr_contract_type(osv.osv):
    ...
    _columns = {'name': fields.char('Contract Type', size=32, required=True)}
...
class hr_contract(osv.osv):
    ...
    _columns = {'name': fields.char('Contract Reference', size=64, required=True),
    ...
    _defaults = {'date_start': lambda *a: time.strftime('%Y-%m-%d'),
     'type_id': _get_type}
    ...
    _constraints = [(_check_dates, 'Error! Contract start-date must be less than contract end-date.', ['date_start', 'date_end'])]

...
class acquirer(osv.Model):
    ...
    def render(self, cr, uid, id, object, reference, currency, amount, context = None, **kwargs):
        ...
        try:
            ...
            result = MakoTemplate(this.form_template).render_unicode(object=object, reference=reference, currency=currency, amount=amount, kind=i18n_kind, quote=quote, ctx=context, format_exceptions=True)
            return result.strip()
        except Exception:

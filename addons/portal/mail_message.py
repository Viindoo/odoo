...
class mail_message(osv.Model):
    ...
    def _search(self, cr, uid, args, offset = 0, limit = None, order = None, context = None, count = False, access_rights_uid = None):
        ...
        if group_user_id not in [ group.id for group in group_ids ]:
            args = ['&',
             '|',
             ('type', '!=', 'comment'),
             ('subtype_id', '!=', False)] + list(args)
        return super(mail_message, self)._search(cr, uid, args, offset=offset, limit=limit, order=order, context=context, count=False, access_rights_uid=access_rights_uid)
    ...
    def check_access_rule(self, cr, uid, ids, operation, context = None):
        ...
        if group_user_id not in [ group.id for group in group_ids ]:
            ...
            if cr.fetchall():
                raise orm.except_orm(_('Access Denied'), _('The requested operation cannot be completed due to security restrictions. Please contact your system administrator.\n\n(Document type: %s, Operation: %s)') % (self._description, operation))

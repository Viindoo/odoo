...
class wizard(osv.osv_memory):
    ...
    _columns = {'portal_id': fields.many2one('res.groups', domain=[('is_portal', '=', True)], required=True, string='Portal', help='The portal that users can be added in or removed from.'),
     'user_ids': fields.one2many('portal.wizard.user', 'wizard_id', string='Users'),
     'welcome_message': fields.text(string='Invitation Message', help='This text is included in the email sent to new users of the portal.')}
    ...
    _defaults = {'portal_id': _default_portal}
        ...
        for partner in res_partner.browse(cr, SUPERUSER_ID, partner_ids, context):
            for contact in partner.child_ids or [partner]:
                if contact.id not in contact_ids:
                    ...
                    user_changes.append((0, 0, {'partner_id': contact.id,
                      'email': contact.email,
                      'in_portal': in_portal}))
...
class wizard_user(osv.osv_memory):
    ...
    _columns = {'wizard_id': fields.many2one('portal.wizard', string='Wizard', required=True),
    ...
    def _create_user(self, cr, uid, wizard_user, context = None):
        ...
        values = {'login': extract_email(wizard_user.email),
    ...
    def _send_email(self, cr, uid, wizard_user, context = None):
        ...
        if not this_user.email:
            raise osv.except_osv(_('Email Required'), _('You must have an email address in your User Preferences to send emails.'))
        user = self._retrieve_user(cr, SUPERUSER_ID, wizard_user, context)
        context = dict(this_context or {}, lang=user.lang)
        data = {'company': this_user.company_id.name,
        ...
        mail_values = {'email_from': this_user.email,
         'email_to': user.email,

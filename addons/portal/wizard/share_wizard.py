...
class share_wizard_portal(osv.TransientModel):
    ...
    def _user_type_selection(self, cr, uid, context = None):
        ...
        selection.extend([('existing', _('Users you already shared with')), ('groups', _('Existing Groups (e.g Portal Groups)'))])
        return selection

    _columns = {'user_ids': fields.many2many('res.users', 'share_wizard_res_user_rel', 'share_id', 'user_id', 'Existing users', domain=[('share', '=', True)]),
    ...
    def _create_or_get_submenu_named(self, cr, uid, parent_menu_id, menu_name, context = None):
        if not parent_menu_id:
            ...
            parent_menu = Menus.browse(cr, uid, parent_menu_id)
            menu_id = None
            max_seq = 10
            ...
            if not menu_id:
                menu_id = Menus.create(cr, UID_ROOT, {'name': menu_name,

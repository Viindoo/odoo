...
class hr_evaluation_plan(osv.osv):
    ...
    _columns = {'name': fields.char('Appraisal Plan', size=64, required=True),
...
class hr_evaluation_plan_phase(osv.osv):
    ...
    _order = 'sequence'
    _columns = {'name': fields.char('Phase', size=64, required=True),
     ...
     'wait': fields.boolean('Wait Previous Phases', help='Check this box if you want to wait that all preceding phases ' + 'are finished before launching this phase.'),
...
class hr_employee(osv.osv):
    ...
    _columns = {'evaluation_plan_id': fields.many2one('hr_evaluation.plan', 'Appraisal Plan'),
    ...
    def run_employee_evaluation(self, cr, uid, automatic = False, use_new_cursor = False, context = None):
        ...
        for emp in self.browse(cr, uid, emp_ids, context=context):
            ...
            plan_id = obj_evaluation.create(cr, uid, {'employee_id': emp.id,
             'plan_id': emp.evaluation_plan_id.id}, context=context)
...
class hr_evaluation(osv.osv):
    ...
    _columns = {'date': fields.date('Appraisal Deadline', required=True, select=True),
     ...
     'rating': fields.selection([('0', 'Significantly bellow expectations'),
                ...
                ('4', 'Significantly exceeds expectations')], 'Appreciation', help='This is the appreciation on which the evaluation is summarized.'),
    ...
    _defaults = {'date': lambda *a: (parser.parse(datetime.now().strftime('%Y-%m-%d')) + relativedelta(months=+1)).strftime('%Y-%m-%d'),
        ...
        for evaluation in self.browse(cr, uid, ids, context=context):
            ...
            for phase in evaluation.plan_id.phase_ids:
                ...
                for child in children:
                    int_id = hr_eval_inter_obj.create(cr, uid, {'evaluation_id': evaluation.id,
                     ...
                     'user_to_review_id': evaluation.employee_id.id}, context=context)
                    if phase.wait:
                        wait = True
                    if not wait:
...
class survey_request(osv.osv):
    ...
    _columns = {'is_evaluation': fields.boolean('Is Appraisal?')}
...
class hr_evaluation_interview(osv.osv):
    ...
    _columns = {'request_id': fields.many2one('survey.request', 'Request_id', ondelete='cascade', required=True),
    ...
    def action_print_survey(self, cr, uid, ids, context = None):
        ...
        context.update({'survey_id': record.survey_id.id,

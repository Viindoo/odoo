...
class hr_job(osv.osv):
    ...
    def _no_of_employee(self, cr, uid, ids, name, args, context = None):
        ...
        for job in self.browse(cr, uid, ids, context=context):
            ...
            res[job.id] = {'no_of_employee': nb_employees,
    ...
    _columns = {
         ...
         'expected_employees': fields.function(_no_of_employee, string='Total Forecasted Employees', help='Expected number of employees for this job position after new recruitment.', store={'hr.job': (lambda self, cr, uid, ids, c = None: ids, ['no_of_recruitment'], 10),
                                'hr.employee': (_get_job_position, ['job_id'], 10)}, multi='no_of_employee'),
         'no_of_employee': fields.function(_no_of_employee, string='Current Number of Employees', help='Number of employees currently occupying this job position.', store={'hr.employee': (_get_job_position, ['job_id'], 10)}, multi='no_of_employee'),
         ...
         'state': fields.selection([('open', 'No Recruitment'), ('recruit', 'Recruitement in Progress')], 'Status', readonly=True, required=True, help="By default 'In position', set it to 'In Recruitment' if recruitment process is going on for this job position.")
         }
    _defaults = {
         'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'hr.job', context=c),
         'state': 'open'
         }
    _sql_constraints = [('name_company_uniq', 'unique(name, company_id)', 'The name of the job position must be unique per company!')]
    ...
    def job_recruitement(self, cr, uid, ids, *args):
        for job in self.browse(cr, uid, ids):
            ...
            self.write(cr, uid, [job.id], {'state': 'recruit',
...
class hr_employee(osv.osv):
    ...
    _columns = {
         ...
         'marital': fields.selection([('single', 'Single'),
                     ('married', 'Married'),
                     ('widower', 'Widower'),
                     ('divorced', 'Divorced')], 'Marital Status'),
         'department_id': fields.many2one('hr.department', 'Department'),
         ...
         'image_medium': fields.function(_get_image, fnct_inv=_set_image, string='Medium-sized photo', type='binary', multi='_get_image', store={'hr.employee': (lambda self, cr, uid, ids, c = {}: ids, ['image'], 10)}, help='Medium-sized photo of the employee. It is automatically resized as a 128x128px image, with aspect ratio preserved. Use this field in form views or some kanban views.'),
    ...
    def create(self, cr, uid, data, context = None):
        ...
        try:
            ...
            self.pool.get('mail.group').message_post(cr, uid, [mail_group_id], body=_('Welcome to %s! Please help him/her take the first steps with OpenERP!') % employee.name, subtype='mail.mt_comment', context=context)
        except:
            pass
    ...
    def onchange_address_id(self, cr, uid, ids, address, context = None):
        if address:
            ...
            return {'value': {'work_phone': address.phone,
    ...
    _constraints = [(_check_recursion, 'Error! You cannot create recursive hierarchy of Employee(s).', ['parent_id'])]
...
class hr_department(osv.osv):
    ...
    _columns = {'manager_id': fields.many2one('hr.employee', 'Manager'),
...
class res_users(osv.osv):
    ...
    _columns = {'employee_ids': fields.one2many('hr.employee', 'user_id', 'Related employees')}

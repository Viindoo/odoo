...
class contribution_register_report(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({'get_payslip_lines': self._get_payslip_lines,
    ...
    def _get_payslip_lines(self, obj):
        ...
        res = []
        self.regi_total = 0.0
        self.cr.execute("SELECT pl.id from hr_payslip_line as pl LEFT JOIN hr_payslip AS hp on (pl.slip_id = hp.id) WHERE (hp.date_from >= %s) AND (hp.date_to <= %s) AND pl.register_id = %s AND hp.state = 'done' ORDER BY pl.slip_id, pl.sequence", (self.date_from, self.date_to, obj.id))
        payslip_lines = [ x[0] for x in self.cr.fetchall() ]
        for line in payslip_line.browse(self.cr, self.uid, payslip_lines):
            res.append({'payslip_name': line.slip_id.name,
             'name': line.name,
             'code': line.code,

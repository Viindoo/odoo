...
class payslip_details_report(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({'get_details_by_rule_category': self.get_details_by_rule_category,
    ...
    def get_details_by_rule_category(self, obj):
        ...
        if ids:
            self.cr.execute('SELECT pl.id, pl.category_id FROM hr_payslip_line as pl                 LEFT JOIN hr_salary_rule_category AS rc on (pl.category_id = rc.id)                 WHERE pl.id in %s                 GROUP BY rc.parent_id, pl.sequence, pl.id, pl.category_id                 ORDER BY pl.sequence, rc.parent_id', (tuple(ids),))
            ...
            for key, value in result.iteritems():
                ...
                for parent in parents:
                    res.append({'rule_category': parent.name,
                     'name': parent.name,
                     'code': parent.code,
                     'level': level,
                ...
                for line in payslip_line.browse(self.cr, self.uid, value):
                    res.append({'rule_category': line.name,
                     'name': line.name,
                     'code': line.code,
                     'total': line.total,
                     'level': level})

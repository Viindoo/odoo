...
def emp_create_xml(self, cr, uid, dept, holiday_type, row_id, empid, name, som, eom):
    ...
    if dept == 0:
        ...
        ids_date = pooler.get_pool(cr.dbname).get('hr.holidays').read(cr, uid, p_id, ['date_from',
         'date_to',
         'holiday_status_id',
         'state'])
...
class report_custom(report_rml):
    ...
    def create_xml(self, cr, uid, ids, data, context):
        ...
        for l in range(0, len(legend)):
            date_xml += ['<legend row="%d" id="%d" name="%s" color="%s" />' % (l + 1,
              legend[l][0],
              _(legend[l][1]),
              legend[l][2])]
        ...
        elif day_diff.days >= lengthmonth(som.year, som.month) - som.day:
            date_xml += [ '<dayy number="%d" name="%s" cell="%d"/>' % (x, _(som.replace(day=x).strftime('%a')), x - som.day + 1) for x in range(som.day, lengthmonth(som.year, som.month) + 1) ]
        else:
        ...
        if data['model'] == 'hr.employee':
            ...
            for id in data['form']['depts']:
                ...
                cr.execute('SELECT id FROM hr_employee                 WHERE department_id = %s', (id,))

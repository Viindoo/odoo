...
class hr_payroll_structure(osv.osv):
    ...
    _defaults = {'company_id': lambda self, cr, uid, context: self.pool.get('res.users').browse(cr, uid, uid, context=context).company_id.id,
     'parent_id': _get_parent}
    _constraints = [(osv.osv._check_recursion, 'Error ! You cannot create a recursive Salary Structure.', ['parent_id'])]
    ...
    def copy(self, cr, uid, id, default = None, context = None):
        ...
        if not default:
            default = {}
        default.update(code=_('%s (copy)') % self.browse(cr, uid, id, context=context).code, company_id=self.pool.get('res.users').browse(cr, uid, uid, context=context).company_id.id)
...
class hr_contract(osv.osv):
    ...
    _columns = {'struct_id': fields.many2one('hr.payroll.structure', 'Salary Structure'),
     'schedule_pay': fields.selection([('monthly', 'Monthly'),
                      ...
                      ('weekly', 'Weekly'),
                      ('bi-weekly', 'Bi-weekly'),
                      ('bi-monthly', 'Bi-monthly')], 'Scheduled Pay', select=True)}
    _defaults = {'schedule_pay': 'monthly'}
...
class hr_salary_rule_category(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, readonly=False),
...
class hr_payslip_run(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, readonly=True, states={'draft': [('readonly', False)]}),
     'slip_ids': fields.one2many('hr.payslip', 'payslip_run_id', 'Payslips', required=False, readonly=True, states={'draft': [('readonly', False)]}),
     'state': fields.selection([('draft', 'Draft'), ('close', 'Close')], 'Status', select=True, readonly=True),
...
class hr_payslip(osv.osv):
    ...
    def _get_lines_salary_rule_category(self, cr, uid, ids, field_names, arg = None, context = None):
        ...
        cr.execute('SELECT pl.slip_id, pl.id FROM hr_payslip_line AS pl                     LEFT JOIN hr_salary_rule_category AS sh on (pl.category_id = sh.id)                     WHERE pl.slip_id in %s                     GROUP BY pl.slip_id, pl.sequence, pl.id ORDER BY pl.sequence', (tuple(ids),))
        res = cr.fetchall()
        for r in res:
    ...
    _columns = {'struct_id': fields.many2one('hr.payroll.structure', 'Structure', readonly=True, states={'draft': [('readonly', False)]}, help="Defines the rules that have to be applied to this payslip, accordingly to the contract chosen. If you let empty the field contract, this field isn't mandatory anymore and thus the rules applied will be all the rules set on the structure of all contracts of the employee valid for the chosen period"),
     ...
     'state': fields.selection([('draft', 'Draft'),
               ('verify', 'Waiting'),
               ('done', 'Done'),
               ('cancel', 'Rejected')], 'Status', select=True, readonly=True, help="* When the payslip is created the status is 'Draft'.            \n* If the payslip is under verification, the status is 'Waiting'.             \n* If the payslip is confirmed then status is set to 'Done'.            \n* When user cancel payslip the status is 'Rejected'."),
    ...
    def process_sheet(self, cr, uid, ids, context = None):
        return self.write(cr, uid, ids, {'paid': True,
    ...
    def refund_sheet(self, cr, uid, ids, context = None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            id_copy = self.copy(cr, uid, payslip.id, {'credit_note': True,
             'name': _('Refund: ') + payslip.name}, context=context)
    ...
    def compute_sheet(self, cr, uid, ids, context = None):
        ...
        for payslip in self.browse(cr, uid, ids, context=context):
            ...
            self.write(cr, uid, [payslip.id], {'line_ids': lines,
    ...
    def get_worked_day_lines(self, cr, uid, contract_ids, date_from, date_to, context = None):
        ...
        for contract in self.pool.get('hr.contract').browse(cr, uid, contract_ids, context=context):
            if not contract.working_hours:
                continue
            attendances = {'name': _('Normal Working Days paid at 100%'),
             'sequence': 1,
             'code': 'WORK100',
             'number_of_days': 0.0,
             'number_of_hours': 0.0,
                ...
                if working_hours_on_day:
                    ...
                    if leave_type:
                        if leave_type in leaves:
                            ...
                            leaves[leave_type] = {'name': leave_type,
                             'sequence': 5,
                             'code': leave_type,
                             'number_of_days': 1.0,
    ...
    def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, context):
        ...
        class InputLine(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date = None):
                ...
                result = 0.0
                self.cr.execute("SELECT sum(amount) as sum                            FROM hr_payslip as hp, hr_payslip_input as pi                             WHERE hp.employee_id = %s AND hp.state = 'done'                             AND hp.date_from >= %s AND hp.date_to <= %s AND hp.id = pi.payslip_id AND pi.code = %s", (self.employee_id,
                 from_date,
                 to_date,
                 code))
        ...
        class WorkedDays(BrowsableObject):
            ...
            def _sum(self, code, from_date, to_date = None):
                if to_date is None:
                    to_date = datetime.now().strftime('%Y-%m-%d')
                result = 0.0
                self.cr.execute("SELECT sum(number_of_days) as number_of_days, sum(number_of_hours) as number_of_hours                            FROM hr_payslip as hp, hr_payslip_worked_days as pi                             WHERE hp.employee_id = %s AND hp.state = 'done'                            AND hp.date_from >= %s AND hp.date_to <= %s AND hp.id = pi.payslip_id AND pi.code = %s", (self.employee_id,
                 from_date,
                 to_date,
                 code))
        ...
        class Payslips(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date = None):
                if to_date is None:
                    to_date = datetime.now().strftime('%Y-%m-%d')
                self.cr.execute("SELECT sum(case when hp.credit_note = False then (pl.total) else (-pl.total) end)                            FROM hr_payslip as hp, hr_payslip_line as pl                             WHERE hp.employee_id = %s AND hp.state = 'done'                             AND hp.date_from >= %s AND hp.date_to <= %s AND hp.id = pl.slip_id AND pl.code = %s", (self.employee_id,
                 from_date,
                 to_date,
                 code))
        ...
        for contract in self.pool.get('hr.contract').browse(cr, uid, contract_ids, context=context):
            ...
            localdict.update({'employee': employee,
             'contract': contract})
            for rule in obj_rule.browse(cr, uid, sorted_rule_ids, context=context):
                ...
                if obj_rule.satisfy_condition(cr, uid, rule.id, localdict, context=context) and rule.id not in blacklist:
                    ...
                    result_dict[key] = {'salary_rule_id': rule.id,
                     'contract_id': contract.id,
                     'name': rule.name,
                     'code': rule.code,
    ...
    def onchange_employee_id(self, cr, uid, ids, date_from, date_to, employee_id = False, contract_id = False, context = None):
        ...
        if not employee_id or not date_from or not date_to:
            ...
            res['value'].update({'name': _('Salary Slip of %s for %s') % (employee_id.name, tools.ustr(ttyme.strftime('%B-%Y'))),
...
class hr_payslip_worked_days(osv.osv):
    ...
    _columns = {'name': fields.char('Description', size=256, required=True),
...
class hr_salary_rule(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=256, required=True, readonly=False),
     ...
     'quantity': fields.char('Quantity', size=256, help='It is used in computation for percentage and fixed amount.For e.g. A rule for Meal Voucher having fixed amount of 1\xe2\x82\xac per worked day can have its quantity defined in expression like worked_days.WORK100.number_of_days.'),
     ...
     'amount_select': fields.selection([('percentage', 'Percentage (%)'), ('fix', 'Fixed Amount'), ('code', 'Python Code')], 'Amount Type', select=True, required=True, help='The computation method for the rule amount.'),
...
class hr_payslip_line(osv.osv):
    ...
    _columns = {'slip_id': fields.many2one('hr.payslip', 'Pay Slip', required=True, ondelete='cascade'),
    ...
    def _calculate_total_wage(self, cr, uid, ids, name, args, context):
        ...
        for employee in self.browse(cr, uid, ids, context=context):
            if not employee.contract_ids:
                ...
                continue
            cr.execute('SELECT SUM(wage) FROM hr_contract WHERE employee_id = %s AND date_start <= %s AND (date_end > %s OR date_end is NULL)', (employee.id, current_date, current_date))
            result = dict(cr.dictfetchone())
            res[employee.id] = {'basic': result['sum']}

        return res

    _columns = {'slip_ids': fields.one2many('hr.payslip', 'employee_id', 'Payslips', required=False, readonly=True),

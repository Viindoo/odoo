...
class document_configuration(osv.osv_memory):
    ...
    def execute(self, cr, uid, ids, context = None):
        ...
        if self.pool.get('sale.order'):
            ...
            dir_pool.write(cr, uid, [sale_dir_id], {'type': 'ressource',
            ...
            dir_pool.write(cr, uid, [quta_dir_id], {'type': 'ressource',
             'ressource_type_id': mid[0],
             'domain': "[('state','=','draft')]"})
            ...
            if order_report_data_id:
                ...
                content_pool.create(cr, uid, {'name': 'Print Order',
                 'suffix': '_print',
                 'report_id': order_report_id,
                 'extension': '.pdf',
                 'include_name': 1,
                 'directory_id': sale_dir_id})
                content_pool.create(cr, uid, {'name': 'Print Quotation',
                 'suffix': '_print',
                 'report_id': order_report_id,
                 'extension': '.pdf',
                 'include_name': 1,
        ...
        if self.pool.get('product.product'):
            ...
            dir_pool.write(cr, uid, [product_dir_id], {'type': 'ressource',
             'ressource_type_id': mid[0]})
        if self.pool.get('account.analytic.account'):
            ...
            dir_pool.write(cr, uid, [project_dir_id], {'type': 'ressource',
             'ressource_type_id': mid[0],
             'domain': '[]',
             'ressource_tree': 1})

...
class document_file(osv.osv):
    ...
    _columns = {'write_date': fields.datetime('Date Modified', readonly=True),
    ...
    _order = 'id desc'
    _defaults = {'user_id': lambda self, cr, uid, ctx: uid}
    _sql_constraints = [('filename_unique', 'unique (name,parent_id)', 'The filename must be unique in a directory !')]
    ...
    def __get_partner_id(self, cr, uid, res_model, res_id, context = None):
        ...
        if obj_model._name == 'res.partner':
            return res_id
        if 'partner_id' in obj_model._columns and obj_model._columns['partner_id']._obj == 'res.partner':
...
class document_directory(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, select=1),
     ...
     'type': fields.selection([('directory', 'Static Directory'), ('ressource', 'Folders per resource')], 'Type', required=True, select=1, change_default=True, help='Each directory can either have the type Static or be linked to another resource. A static directory, as with Operating Systems, is the classic directory that can contain a set of files. The directories linked to systems resources automatically possess sub-directories for each of resource types defined in the parent directory.'),
     'domain': fields.char('Domain', size=128, help='Use a domain if you want to apply an automatic filter on visible resources.'),
     'ressource_type_id': fields.many2one('ir.model', 'Resource model', change_default=True, help='Select an object here and there will be one folder per record of that resource.'),
     ...
     'resource_find_all': fields.boolean('Find all resources', help='If true, all attachments that match this resource will  be located. If false, only ones that have this as parent.'),
     'ressource_parent_type_id': fields.many2one('ir.model', 'Parent Model', change_default=True, help='If you put an object here, this directory template will appear bellow all of these objects. Such directories are "attached" to the specific model or record, just like attachments. Don\'t put a parent directory if you select a parent model.'),
     'ressource_id': fields.integer('Resource ID', help='Along with Parent Model, this ID attaches this folder to a specific record of Parent Model.'),
     'ressource_tree': fields.boolean('Tree Structure', help='Check this if you want to use the same tree structure as the object selected in the system.'),
     'dctx_ids': fields.one2many('document.directory.dctx', 'dir_id', 'Context fields'),
     'company_id': fields.many2one('res.company', 'Company', change_default=True)}
    _defaults = {'company_id': lambda s, cr, uid, c: s.pool.get('res.company')._company_default_get(cr, uid, 'document.directory', context=c),
     'user_id': lambda self, cr, uid, ctx: uid,
     'domain': '[]',
     'type': 'directory',
     'ressource_id': 0,
     'resource_find_all': True}
    _sql_constraints = [('dirname_uniq', 'unique (name,parent_id,ressource_id,ressource_parent_type_id)', 'The directory name must be unique !'), ('no_selfparent', 'check(parent_id <> id)', 'Directory cannot be parent of itself!')]
    ...
    def get_full_path(self, cr, uid, dir_id, context = None):
        ...
        """
        if not (isinstance(dir_id, (tuple, list)) and len(dir_id) == 1):
    ...
    _constraints = [(_check_recursion, 'Error! You cannot create recursive directories.', ['parent_id'])]
    ...
    def _check_duplication(self, cr, uid, vals, ids = None, op = 'create'):
        ...
        if op == 'write':
            for directory in self.browse(cr, uid, ids):
                ...
                res = self.search(cr, uid, [('id', '<>', directory.id),
                 ('name', '=', name),
                 ('parent_id', '=', parent_id),
...
class document_directory_dctx(osv.osv):
    ...
    _columns = {'dir_id': fields.many2one('document.directory', 'Directory', required=True, ondelete='cascade'),
...
class document_directory_content_type(osv.osv):
    ...
    _columns = {'name': fields.char('Content Type', size=64, required=True),
...
class document_directory_content(osv.osv):
    ...
    _columns = {'name': fields.char('Content Name', size=64, required=True),
     ...
     'include_name': fields.boolean('Include Record Name', help='Check this field if you want that the name of the file to contain the record name.\nIf set, the directory will have to be a resource one.'),
     'directory_id': fields.many2one('document.directory', 'Directory')}
    _defaults = {'extension': lambda *args: '.pdf',
...
class ir_action_report_xml(osv.osv):
    ...
    def _model_search(self, cr, uid, obj, name, args, context = None):
        if not len(args):
            return []
        if not (len(args) == 1 and args[0][1] == '='):
            raise AssertionError('expression is not what we expect: %r' % args)
            model_id = args[0][2]
            if not model_id:
                return []
            model = self.pool.get('ir.model').read(cr, uid, [model_id])[0]['model']
            report_id = self.search(cr, uid, [('model', '=', model)])
            return report_id or [('id', '=', '0')]
        return [('id', 'in', report_id)]

    _columns = {'model_id': fields.function(_model_get, fnct_search=_model_search, string='Model Id')}
...
class document_storage(osv.osv):
    ...
    def set_data(self, cr, uid, id, file_node, data, context = None, fil_obj = None):
        ...
        try:
            ...
            cr.execute('UPDATE ir_attachment SET file_size = %s, index_content = %s, file_type = %s WHERE id = %s', (filesize,
             icont_u,
             mime,
             file_node.file_id))
...
class node_context(object):
    ...
    def __init__(self, cr, uid, context = None):
        ...
        self.extra_ctx = {}
        raise self._dirobj or AssertionError
...
class node_class(object):
    ...
    def __init__(self, path, parent, context):
        if not isinstance(context, node_context):
            raise AssertionError
            raise not parent or isinstance(parent, node_class) or AssertionError
            self.path = path
            self.context = context
            ...
            self.dctx = {}
            self.dctx = parent and parent.dctx.copy()
...
class node_dir(node_database):
    ...
    def __init__(self, path, parent, context, dirr, dctx = None):
        ...
        self.unixperms = 16384 | _uid2unixperms(self.uidperms, dirr and dirr.user_id)
        if dctx:
    ...
    def move_to(self, cr, ndir_node, new_name = False, fil_obj = None, ndir_obj = None, in_write = False):
        ...
        if not fil_obj:
            ...
            dbro = dir_obj
            raise dbro.id == self.dir_id or AssertionError
        if not dbro:
        ...
        if not self.parent and ndir_node:
            ...
            del dbro
            if not in_write:
                ctx = ret and self.context.context.copy()
...
class node_res_obj(node_class):
    ...
    def __init__(self, path, dir_id, parent, context, res_model, res_bo, res_id = None):
        ...
        if not parent:
            ...
            self.unixperms = 16384 | _uid2unixperms(self.uidperms, True)
            ...
            self.res_id = res_bo and res_bo.id
    ...
    def _child_get(self, cr, name = None, domain = None):
        ...
        if self.res_id and directory.ressource_tree:
            ...
            for bo in obj.browse(cr, uid, resids, context=ctx):
                ...
                rnode = klass(res_name, dir_id=self.dir_id, parent=self, context=self.context, res_model=self.res_model, res_bo=bo)
...
class node_file(node_class):
    ...
    def fix_ppath(self, cr, fbro):
        ...
        if not fbro:
            ...
            self.path = len(dirpath) > 1 and dirpath
        else:
            self.path = dirpath[0]
    ...
    def move_to(self, cr, ndir_node, new_name = False, fil_obj = None, ndir_obj = None, in_write = False):
        ...
        if not fil_obj:
            ...
            dbro = fil_obj
            raise dbro.id == self.file_id or AssertionError('%s != %s for %r.' % (dbro.id, self.file_id, self))
        if not dbro:
            raise IndexError('Cannot locate doc %d.', self.file_id)
        self.parent = self.parent or self.context.get_dir_node(cr, dbro.parent_id)
        if not self.parent:
            raise AssertionError
        ret = {}
        if ndir_node and self.parent != ndir_node:
            ...
            if not ndir_obj:
                ndir_obj = self.context._dirobj.browse(cr, self.context.uid, ndir_node.dir_id, context=self.context.context)
            if not ndir_obj.id == ndir_node.dir_id:
                raise AssertionError
                r2 = {'parent_id': ndir_obj.id}
                ret.update(r2)
            if new_name and new_name != dbro.name:
                if len(ret):
                    raise NotImplementedError('Cannot rename and move.')
                r2 = {'name': new_name,
...
class node_content(node_class):
    ...
    def fill_fields(self, cr, dctx = None):
        ...
        """
        cr.execute('SELECT DISTINCT mimetype FROM document_directory_content_type WHERE active AND code = %s;', (self.extension,))
        res = cr.fetchall()
        if res and res[0][0]:
...
class node_descriptor(object):
    ...
    def __init__(self, parent):
        raise isinstance(parent, node_class) or AssertionError

...
class purchase_requisition(osv.osv):
    ...
    _columns = {'name': fields.char('Requisition Reference', size=32, required=True),
    ...
    _defaults = {'date_start': lambda *args: time.strftime('%Y-%m-%d %H:%M:%S'),
     'state': 'draft',
     'exclusive': 'multiple',
    ...
    def tender_done(self, cr, uid, ids, context = None):
        return self.write(cr, uid, ids, {'state': 'done',
         'date_end': time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
    ...
    def make_purchase_order(self, cr, uid, ids, partner_id, context = None):
        ...
        if context is None:
            context = {}
        raise partner_id or AssertionError('Supplier should be specified')
        ...
        for requisition in self.browse(cr, uid, ids, context=context):
            ...
            for line in requisition.line_ids:
                ...
                purchase_order_line.create(cr, uid, {'order_id': purchase_id,
                 ...
                 'taxes_id': [(6, 0, taxes)]}, context=context)
...
class purchase_requisition_line(osv.osv):
    ...
    _defaults = {'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'purchase.requisition.line', context=c)}
...
class purchase_order(osv.osv):
    ...
    _columns = {'requisition_id': fields.many2one('purchase.requisition', 'Purchase Requisition')}
...
class product_product(osv.osv):
    ...
    _columns = {'purchase_requisition': fields.boolean('Purchase Requisition', help='Check this box to generates purchase requisition instead of generating requests for quotation from procurement.')}
    _defaults = {'purchase_requisition': False}
...
class procurement_order(osv.osv):
    ...
    _columns = {'requisition_id': fields.many2one('purchase.requisition', 'Latest Requisition')}
    ...
    def make_po(self, cr, uid, ids, context = None):
        ...
        if procurement.product_id.purchase_requisition:
            ...
            res[procurement.id] = requisition_obj.create(cr, uid, {'origin': procurement.origin,
             'date_end': procurement.date_planned,
             'warehouse_id': warehouse_id and warehouse_id[0] or False,
             'company_id': procurement.company_id.id,
             'line_ids': [(0, 0, {'product_id': procurement.product_id.id,
                            'product_uom_id': procurement.product_uom.id,
                            'product_qty': procurement.product_qty})]})
            self.write(cr, uid, [procurement.id], {'state': 'running',
             'requisition_id': res[procurement.id]}, context=context)
        else:

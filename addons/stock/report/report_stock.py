...
class stock_report_prodlots(osv.osv):
    ...
    def init(self, cr):
        ...
        cr.execute("create or replace view stock_report_prodlots as (select max(id) as id,\
                    location_id,\
                    product_id,\
                    prodlot_id,\
                    sum(qty) as qty\
                    ...
                    on (uo.id=sm.product_uom)\
                    where sm.state = 'done'\
                    group by sm.location_dest_id, sm.product_id, sm.product_uom, sm.prodlot_id) as report\
...
class stock_report_tracklots(osv.osv):
    ...
    def init(self, cr):
        ...
        cr.execute("create or replace view stock_report_tracklots as (select max(id) as id,\
                    location_id,\
                    product_id,\
                    tracking_id,\
                    sum(qty) as name\
                    ...
                    on (uo.id=sm.product_uom)\
                    where sm.state = 'done'\
                    group by sm.location_dest_id, sm.product_id, sm.product_uom, sm.tracking_id) as report\
...
class report_stock_lines_date(osv.osv):
    ...
    def init(self, cr):
        ...
        cr.execute("create or replace view report_stock_lines_date as (select\
                    p.id as id,\
                    p.id as product_id,\
                    max(s.date) as date\
                    from\

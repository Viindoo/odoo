...
class report_stock(report_int):
    ...
    def create(self, cr, uid, ids, datas, context = None):
        ...
        if not loc_ids or not product_ids:
            ...
            cr.execute('select sum(r.product_qty * u.factor), r.date, r.product_id from stock_move r left join product_uom u on (r.product_uom=u.id) where state IN %sand location_id IN %sand product_id IN %sgroup by date,product_id', (('confirmed', 'assigned', 'waiting'), tuple(loc_ids), tuple(product_ids)))
            ...
            cr.execute('select sum(r.product_qty * u.factor), r.date, r.product_id from stock_move r left join product_uom u on (r.product_uom=u.id) where state IN %sand location_dest_id IN %sand product_id IN %sgroup by date,product_id', (('confirmed', 'assigned', 'waiting'), tuple(loc_ids), tuple(product_ids)))
            for qty, dt, prod_id in cr.fetchall():
                if dt <= dt_from:
                    dt = (datetime.now() + relativedelta(days=1)).strftime('%Y-%m-%d')
                else:

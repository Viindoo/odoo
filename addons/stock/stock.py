...
class stock_incoterms(osv.osv):
    ...
    _columns = {
         'name': fields.char('Name', size=64, required=True, track_visibility='onchange', help='Incoterms are series of sales terms.They are used to divide transaction costs and responsibilities between buyer and seller and reflect state-of-the-art transportation practices.'),
...
class stock_location(osv.osv):
    ...
    _columns = {
         'name': fields.char('Location Name', size=1000, required=True, track_visibility='onchange'),
         'code': fields.char('Mã địa điểm', size=64, track_visibility='onchange'),
         ...
         'location_id': fields.many2one('stock.location', 'Parent Location', select=True, ondelete='restrict', track_visibility='onchange'),
         ...
         'partner_id': fields.many2one('res.partner', 'Người sở hữu địa điểm', track_visibility='onchange', help='Address of  customer or supplier.'),
         ...
         'loai_hinh': fields.selection([('kho_congty', 'Kho công ty'), ('kho_taptrung', 'Kho tập trung'), ('kho_daily', 'Kho đại lý')], 'Loại hình', track_visibility='onchange'),
         'allow': fields.boolean('Cho phép xuất âm', track_visibility='onchange')
         }
    _defaults = {'active': True,
        ...
        for id in self.search(cr, uid, [('location_id', 'child_of', ids)]):
            if lock:
                try:
                    ...
                    cr.execute("SELECT id FROM stock_move WHERE product_id=%s AND \
                              ((location_dest_id=%s AND location_id<>%s AND state='done') OR (location_id=%s AND location_dest_id<>%s AND state in ('done', 'assigned')))\
...
class stock_picking(osv.osv):
    ...
    def _set_maximum_date(self, cr, uid, ids, name, value, arg, context = None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            sql_str = "update stock_move set date_expected='%s' where picking_id=%d " % (value, pick.id)
            if pick.max_date:
    ...
    def _set_minimum_date(self, cr, uid, ids, name, value, arg, context = None):
        ...
        if isinstance(ids, (int, long)):
            ids = [ids]
        for pick in self.browse(cr, uid, ids, context=context):
            sql_str = "update stock_move set date_expected='%s' where picking_id=%s " % (value, pick.id)
            if pick.min_date:
    ...
    def get_min_max_date(self, cr, uid, ids, field_name, arg, context = None):
        ...
        if not ids:
            return res
        cr.execute('select picking_id, min(date_expected), max(date_expected) from stock_move where picking_id IN %s group by picking_id', (tuple(ids),))
    ...
    _columns = {
         ...
         'date': fields.datetime('Creation Date', help='Creation date, usually the time of the order.', select=True, states={'done': [('readonly', True)],
                  'cancel': [('readonly', True)]}),
         'date_done': fields.datetime('Ngày hoàn tất', help='Date of Completion', states={'done': [('readonly', True)],
                       'cancel': [('readonly', True)]}),
         ...
         'company_id': fields.many2one('res.company', 'Company', required=True, select=True, states={'done': [('readonly', True)],
                        'cancel': [('readonly', True)]}),
         'loai_xuatkho': fields.selection([('thongthuong', 'VAT'), ('noibo', 'Nội bộ'), ('doitra', 'Đổi trả')], 'Loại xuất kho'),
    ...
    def tach_phieu(self, cr, uid, ids, context = None):
        ...
        for pick in self.browse(cr, uid, ids, context):
            ...
            count = 0
            if pick_id:
                query_soluong = 'select count(*) as soluong,location_id from stock_move where  picking_id= ' + str(pick_id) + ' group by location_id'
            ...
            if count > 1:
                ...
                query_location = 'select location_id from stock_move  where  picking_id= ' + str(pick_id) + ' group by location_id'
    ...
    def action_process(self, cr, uid, ids, context = None):
        ...
        for pick in self.browse(cr, uid, ids, context):
            if pick.type == 'out':
                ...
                loai_xuatkho = pick.loai_xuatkho
                kynhan = pick.sale_id.state_kynhan
                ...
                for move in pick.move_lines:
                    if kynhan !='da_ky':
                        raise osv.except_osv(_('Thông báo!'), _('Lệnh xuất hàng chưa được ký, bạn không thể thực hiện việc giao hàng. Hãy kiểm tra lại lệnh giao hàng '))                    
                    location_id = move.location_id.loai_hinh
                    if type_kho != location_id and move.state != 'cancel':
                        raise osv.except_osv(_('Thông báo!'), _('Loại hình kho ở Địa điểm nguồn phải giống loại hình kho trên phiếu xuất kho '))
                    ...
                    if loai_xuatkho == 'doitra':
                        location_dest_id = move.location_dest_id.id
                        if location_dest_id == 9:
                            raise osv.except_osv(_('Thông báo!'), _('Địa điểm đích không được là %s .') % move.location_dest_id.name)							
                    if type_kho == 'kho_taptrung' or type_kho == 'kho_daily':
                        if location_id == kho_khach_hang:
                            raise osv.except_osv(_('Thông báo!'), _('Vui lòng chọn địa điểm nguồn trên chi tiết giao hàng. Địa điểm nguồn \xc4\x91\xe1\xbb\x91i v\xe1\xbb\x9bi Kho đại lý và Kho tập trung phải khác %s.') % move.location_id.name)
            self.write(cr, uid, ids, {'ma_vung': string}, context)

        context.update({'active_model': self._name,'active_ids': ids,'active_id': len(ids) and ids[0] or False})
        return {
             'view_type': 'form',
             'view_mode': 'form',
    ...
    def action_invoice_create(self, cr, uid, ids, journal_id = False, group = False, type = 'out_invoice', context = None):
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            ...
            if count > 1:
                raise osv.except_osv(_('Cảnh báo!'), _('Trường hợp xuất trên các kho khác nhau thì phải tách phiếu. Đề nghị tách phiếu xuất kho trước khi tạo hóa đơn!'))
    ...
    def allow_cancel(self, cr, uid, ids, context = None):
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            if nguoilap:
                if nguoilap.id != uid:
                    raise osv.except_osv(_('Thông báo!'), _('Bạn không có quyền hủy PXK. Vui lòng liên hệ với người lập phiếu Người lập phiếu: %s để được hủy PXK.') % nguoilap.name)
            if not pick.move_lines:
                return True
            for move in pick.move_lines:
                if move.state == 'done':
            ...
            for move in pick.move_lines:
                ...
                if move.product_qty == partial_qty[move.id]:
                    ...
                    if move.location_id.allow == False:
                        if (round(product_qty, 3)) > (round(tonkho, 3)):
...
class stock_production_lot(osv.osv):
    ...
    def _get_stock(self, cr, uid, ids, field_name, arg, context = None):
        ...
        if locations:
            cr.execute('select prodlot_id, sum(qty) from stock_report_prodlots where location_id IN %s and prodlot_id IN %s group by prodlot_id', (tuple(locations), tuple(ids)))
    ...
    def _stock_search(self, cr, uid, obj, name, args, context = None):
        ...
        cr.execute('select prodlot_id, sum(qty) from stock_report_prodlots where location_id IN %s group by prodlot_id\
    ...
    _columns = {
         ...
         'date': fields.datetime('Creation Date', required=True),
         'stock_available': fields.function(_get_stock, fnct_search=_stock_search, type='float', string='Available', \
                            select=True, help='Current quantity of products with this Serial Number available in company warehouses', digits_compute=dp.get_precision('Product Unit of Measure')),
...
class stock_move(osv.osv):
    ...
    _columns = {
         'name': fields.char('Description', select=True, states={'done': [('readonly', True)]}),
         ...
         'product_uom': fields.many2one('product.uom', 'Đơn vị', required=True, states={'done': [('readonly', True)]}),
         'product_uos_qty': fields.float('Số lượng', digits_compute=dp.get_precision('Product Unit of Measure'), states={'done': [('readonly', True)]}),
         'product_uos': fields.many2one('product.uom', 'Đơn vị', states={'done': [('readonly', True)]}),
         'product_packaging': fields.many2one('product.packaging', 'Packaging', help='It specifies attributes of packaging like type, quantity of packaging,etc.'),
         'location_id': fields.many2one('stock.location', 'Kho xuất', required=True, select=True, states={'done': [('readonly', False)]}, help='Sets a location if you produce at a fixed location. This can be a partner location if you subcontract the manufacturing operations.'),
    ...
    def onchange_quantity(self, cr, uid, ids, product_id, product_qty, product_uom, product_uos):
        ...
        @return: Dictionary of values
        """
        result = {'product_uos_qty': 0.000}
        warning = {}
        if not product_id or product_qty <= 0.000:
            result['product_qty'] = 0.000
            return {'value': result}
        product_obj = self.pool.get('product.product')
        ...
        if ids:
            for move in self.read(cr, uid, ids, ['product_qty']):
                if product_qty < move['product_qty']:
                    warning.update({'title': _('Thông báo'),
                     'message': _('Bạn đã giao khối lượng trên PXK.Số lượng mới đã nhập thêm: Số chênh lệch so với LXH. Nhấn xác thực nếu bạn không muốn tiếp tục.')})
                break
    ...
    def onchange_uos_quantity(self, cr, uid, ids, product_id, product_uos_qty, product_uos, product_uom):
        ...
        result = {'product_qty': 0.000}
        warning = {}
        if not product_id or product_uos_qty <= 0.000:
            result['product_uos_qty'] = 0.000
            return {'value': result}
        product_obj = self.pool.get('product.product')
        uos_coeff = product_obj.read(cr, uid, product_id, ['uos_coeff'])
    ...
    def onchange_product_id(self, cr, uid, ids, prod_id = False, loc_id = False, loc_dest_id = False, partner_id = False):
        ...
        result = {'product_uom': product.uom_id.id,
         'product_uos': uos_id,
         'product_qty': 1.000,
         'product_uos_qty': self.pool.get('stock.move').onchange_quantity(cr, uid, ids, prod_id, 1.000, product.uom_id.id, uos_id)['value']['product_uos_qty'],
         'prodlot_id': False}
        if not ids:
    ...
    def unlink(self, cr, uid, ids, context = None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.state != 'draft' and move.product_qty > 0 and not ctx.get('call_unlink', False):
                raise osv.except_osv(_('Thông báo!'), _('Không thể xóa dòng chi tiết khi số lượng giao giao và trạng thái khác dự thảo.'))
    ...
    def do_partial(self, cr, uid, ids, partial_datas, context = None):
        ...
        for move in complete:
            ...
            if move.picking_id.id:
                cr.execute('SELECT move.id FROM stock_picking pick RIGHT JOIN stock_move move ON move.picking_id = pick.id AND move.state = %s WHERE pick.id = %s', ('done', move.picking_id.id))
                res = cr.fetchall()
...
class stock_warehouse(osv.osv):
    ...
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _columns = {
         'name': fields.char('Name', size=600, required=True, select=True, track_visibility='onchange'),
         ...
         'type_kho': fields.selection([('kho_congty', 'Kho công ty'), ('kho_taptrung', 'Kho tập trung'), ('kho_daily', 'Kho đại lý')], 'Loại hình kho', required=True, track_visibility='onchange'),
         'ma_kho': fields.char('Mã kho', size=256, required=True, select=True, track_visibility='onchange')
     }
...
class stock_picking_in(osv.osv):
    ...
    _columns = {
         'backorder_id': fields.many2one('stock.picking.in', 'Back Order of', states={'done': [('readonly', True)],
                          'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
         'state': fields.selection([('draft', 'Draft'),
                   ('auto', 'Waiting Another Operation'),
                   ('confirmed', 'Waiting Availability'),
...
class stock_picking_out(osv.osv):
    ...
    _columns = {
     'backorder_id': fields.many2one('stock.picking.out', 'PXK liên quan', states={'done': [('readonly', True)],
                      'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'state': fields.selection([('draft', 'Draft'),
               ('auto', 'Waiting Another Operation'),
               ('confirmed', 'Waiting Availability'),

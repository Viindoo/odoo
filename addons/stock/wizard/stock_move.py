...
class stock_move_consume(osv.osv_memory):
    ...
    def do_move_consume(self, cr, uid, ids, context = None):
        ...
        for data in self.browse(cr, uid, ids, context=context):
            move_obj.action_consume(cr, uid, move_ids, data.product_qty, data.location_id.id, context=context)
...
class stock_move_scrap(osv.osv_memory):
    ...
    def move_scrap(self, cr, uid, ids, context = None):
        ...
        for data in self.browse(cr, uid, ids):
            move_obj.action_scrap(cr, uid, move_ids, data.product_qty, data.location_id.id, context=context)
...
class split_in_production_lot(osv.osv_memory):
    ...
    def split(self, cr, uid, ids, move_ids, context = None):
        ...
        if context is None:
            context = {}
        assert context.get('active_model') == 'stock.move' or AssertionError('Incorrect use of the stock move split wizard')
            ...
            for move in move_obj.browse(cr, uid, move_ids, context=context):
                ...
                for line in lines:
                    ...
                    if total_move_qty > move_qty:
                        raise osv.except_osv(_('Processing Error!'), _('Serial number quantity %d of %s is larger than available quantity (%d)!') % (total_move_qty, move.product_id.name, move_qty))
...
class stock_move_split_lines_exist(osv.osv_memory):
    ...
    def onchange_lot_id(self, cr, uid, ids, prodlot_id = False, product_qty = False, loc_id = False, product_id = False, uom_id = False, context = None):
        return self.pool.get('stock.move').onchange_lot_id(cr, uid, [], prodlot_id, product_qty, loc_id, product_id, uom_id, context)
...
class stock_partial_picking(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields, context = None):
        ...
        if not picking_ids or len(picking_ids) != 1:
            ...
            if not active_model in ('stock.picking', 'stock.picking.in', 'stock.picking.out'):
                assert AssertionError('Bad context propagation')
                ...
                'user_id' in fields and res.update({'user_id': uid})
            return res
    ...
    def do_partial(self, cr, uid, ids, context = None):
        count = 0
        if not len(ids) == 1:
            raise AssertionError('Partial picking processing may only be done one at a time.')
            stock_picking = self.pool.get('stock.picking')
            stock_move = self.pool.get('stock.move')
            uom_obj = self.pool.get('product.uom')
            partial = self.browse(cr, uid, ids[0], context=context)
            partial_data = {'delivery_date': partial.date,'user_id': partial.user_id.id}
                    ...
                    if not kehoach or kehoach is None:
                        ...
                        if chitiet_khobj:
                            ...
                            if ton_lenh < 0 or round(sl_giao, 2) > round(ton_lenh, 2) and ton_lenh >= 0:
                                raise osv.except_osv(_('Thông báo!'), _('Không thể giao một sản phẩm có số lượng lớn hơn số lượng tồn trên KHVC (tồn lệnh: %s %s).') % (ton_lenh, line_uom.name))
            ...
            if partial.picking_id.type == 'out':
                return delivered_picking and {
                     'name': _('Phiếu xuất kho đa hoàn tất'),
            ...
            if partial.picking_id.type == 'in':
                return {
                     'name': _('Phiếu nhập kho đã hoàn tất'),
            ...
            return {
                 'name': _('Phiếu luân chuyển kho đã hoàn tất'),
                 'domain': "[('name', '=', ['" + partial.picking_id.name + "'])]",
                 'view_type': 'form',
                 'view_mode': 'tree,form',

...
class stock_return_picking(osv.osv_memory):
        ...
        for m in pick.move_lines:
            if m.state == 'done':
                ...
                for rec in m.move_history_ids2:
                    if rec.location_dest_id.id == m.location_id.id and rec.location_id.id == m.location_dest_id.id:
    ...
    def create_returns(self, cr, uid, ids, context = None):
        ...
        if pick.type == 'out':
            ...
            new_type = 'out'
            seq_obj_name = 'stock.picking.' + pick.loai_xuatkho
            new_pick_name = self.pool.get('sequence.custormize.pickingout').get_name(cr, uid, seq_obj_name, 'stock_picking', pick.loai_xuatkho)
        ...
        for v in val_id:
            ...
            if new_qty:
                returned_lines += 1
                new_move = move_obj.copy(cr, uid, move.id, {'product_qty': new_qty,

...
class stock_split_into(osv.osv_memory):
    ...
    _columns = {'quantity': fields.float('Quantity', digits_compute=dp.get_precision('Product Unit of Measure'))}
    ...
    def split(self, cr, uid, data, context = None):
        ...
        for move in move_obj.browse(cr, uid, rec_id, context=context):
            ...
            if quantity > move.product_qty:
                raise osv.except_osv(_('Error!'), _('Total quantity after split exceeds the quantity to split for this product: "%s" (id: %d).') % (move.product_id.name, move.product_id.id))
            if quantity > 0:
                move_obj.setlast_tracking(cr, uid, [move.id], context=context)
                move_obj.write(cr, uid, [move.id], {'product_qty': quantity,
            ...
            if quantity_rest > 0:
                ...
                if quantity == 0.0:
                    ...
                    default_val = {'product_qty': quantity_rest,

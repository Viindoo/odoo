...
class sequence_custormize_pickingout(osv.osv):
    ...
    def get_names(self, cr, uid, code, table_name, type, so):
        ...
        if res:
            ...
            query_d = 'select max(substr(name, ' + str(prefix) + ', ' + str(res['padding']) + ')::int) as max  from ' + table_name + " where type='out' and  loai_xuatkho = '" + str(type) + "' and (right(name,6)!='return' or right(name,7)!='tr\xe1\xbb\x9f l\xe1\xba\xa1i') and  (left(name,3)='PXK' or left(name,4)='XKNB') and EXTRACT(year FROM create_date)= " + str(year)
            cr.execute(query_d)
...
class stock_incoterms(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, track_visibility='onchange', help='Incoterms are series of sales terms.They are used to divide transaction costs and responsibilities between buyer and seller and reflect state-of-the-art transportation practices.'),
     'code': fields.char('Code', size=3, required=True, track_visibility='onchange', help='Code for Incoterms'),
     'active': fields.boolean('Active', track_visibility='onchange', help='By unchecking the active field, you may hide an INCOTERM without deleting it.')}
...
class stock_journal(osv.osv):
    ...
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _columns = {'name': fields.char('Stock Journal', size=32, required=True, track_visibility='onchange'),
...
class stock_location(osv.osv):
    ...
    _columns = {'name': fields.char('Location Name', size=1000, required=True, track_visibility='onchange'),
     'code': fields.char('M\xc3\xa3 \xc4\x91\xe1\xbb\x8ba \xc4\x91i\xe1\xbb\x83m', size=64, track_visibility='onchange'),
     'active': fields.boolean('Active', track_visibility='onchange', help='By unchecking the active field, you may hide a location without deleting it.'),
     'usage': fields.selection([('supplier', 'Supplier Location'),
               ...
               ('transit', 'Transit Location for Inter-Companies Transfers')], 'Location Type', required=True, help="* Supplier Location: Virtual location representing the source location for products coming from your suppliers\n                       \n* View: Virtual location used to create a hierarchical structures for your warehouse, aggregating its child locations ; can't directly contain products\n                       \n* Internal Location: Physical locations inside your own warehouses,\n                       \n* Customer Location: Virtual location representing the destination location for products sent to your customers\n                       \n* Inventory: Virtual location serving as counterpart for inventory operations used to correct stock levels (Physical inventories)\n                       \n* Procurement: Virtual location serving as temporary counterpart for procurement operations when the source (supplier or production) is not known yet. This location should be empty when the procurement scheduler has finished running.\n                       \n* Production: Virtual counterpart location for production operations: this location consumes the raw material and produces finished products\n                      ", select=True),
     'complete_name': fields.function(_complete_name, type='char', size=256, track_visibility='onchange', string='Location Name', store={'stock.location': (_get_sublocations, ['name', 'location_id'], 10)}),
     'stock_real': fields.function(_product_value, type='float', string='Real Stock', multi='stock', track_visibility='onchange'),
     'stock_virtual': fields.function(_product_value, type='float', string='Virtual Stock', multi='stock', track_visibility='onchange'),
     'location_id': fields.many2one('stock.location', 'Parent Location', select=True, ondelete='cascade', track_visibility='onchange'),
     'child_ids': fields.one2many('stock.location', 'location_id', 'Contains', track_visibility='onchange'),
     'chained_journal_id': fields.many2one('stock.journal', 'Chaining Journal', track_visibility='onchange', help='Inventory Journal in which the chained move will be written, if the Chaining Type is not Transparent (no journal is used if left empty)'),
     'chained_location_id': fields.many2one('stock.location', 'Chained Location If Fixed', track_visibility='onchange'),
     'chained_location_type': fields.selection([('none', 'None'), ('customer', 'Customer'), ('fixed', 'Fixed Location')], 'Chained Location Type', required=True, track_visibility='onchange', help='Determines whether this location is chained to another location, i.e. any incoming product in this location \nshould next go to the chained location. The chained location is determined according to the type :\n* None: No chaining at all\n* Customer: The chained location will be taken from the Customer Location field on the Partner form of the Partner that is specified in the Picking list of the incoming products.\n* Fixed Location: The chained location is taken from the next field: Chained Location if Fixed.'),
     'chained_auto_packing': fields.selection([('auto', 'Automatic Move'), ('manual', 'Manual Operation'), ('transparent', 'Automatic No Step Added')], 'Chaining Type', track_visibility='onchange', help="This is used only if you select a chained location type.\nThe 'Automatic Move' value will create a stock move after the current one that will be validated automatically. With 'Manual Operation', the stock move has to be validated by a worker. With 'Automatic No Step Added', the location is replaced in the original move."),
     'chained_picking_type': fields.selection([('out', 'Sending Goods'), ('in', 'Getting Goods'), ('internal', 'Internal')], 'Shipping Type', track_visibility='onchange', help='Shipping Type of the Picking List that will contain the chained move (leave empty to automatically detect the type based on the source and destination locations).'),
     'chained_company_id': fields.many2one('res.company', 'Chained Company', track_visibility='onchange', help='The company the Picking List containing the chained move will belong to (leave empty to use the default company determination rules'),
     'chained_delay': fields.integer('Chaining Lead Time', track_visibility='onchange', help='Delay between original move and chained move in days'),
     'partner_id': fields.many2one('res.partner', 'Ng\xc6\xb0\xe1\xbb\x9di s\xe1\xbb\x9f h\xe1\xbb\xafu c\xe1\xbb\xa7a \xc4\x91\xe1\xbb\x8ba \xc4\x91i\xe1\xbb\x83m', track_visibility='onchange', help='Address of  customer or supplier.'),
     'icon': fields.selection(tools.icons, 'Icon', size=64, track_visibility='onchange', help='Icon show in  hierarchical tree view'),
     'comment': fields.text('Additional Information', track_visibility='onchange'),
     'posx': fields.integer('Corridor (X)', track_visibility='onchange', help='Optional localization details, for information purpose only'),
     'posy': fields.integer('Shelves (Y)', track_visibility='onchange', help='Optional localization details, for information purpose only'),
     'posz': fields.integer('Height (Z)', track_visibility='onchange', help='Optional localization details, for information purpose only'),
     'parent_left': fields.integer('Left Parent', select=1, track_visibility='onchange'),
     'parent_right': fields.integer('Right Parent', select=1, track_visibility='onchange'),
     'stock_real_value': fields.function(_product_value, type='float', track_visibility='onchange', string='Real Stock Value', multi='stock', digits_compute=dp.get_precision('Account')),
     'stock_virtual_value': fields.function(_product_value, type='float', track_visibility='onchange', string='Virtual Stock Value', multi='stock', digits_compute=dp.get_precision('Account')),
     'company_id': fields.many2one('res.company', 'Company', select=1, track_visibility='onchange', help='Let this field empty if this location is shared between all companies'),
     'scrap_location': fields.boolean('Scrap Location', track_visibility='onchange', help='Check this box to allow using this location to put scrapped/damaged goods.'),
     'valuation_in_account_id': fields.many2one('account.account', 'Stock Valuation Account (Incoming)', track_visibility='onchange', domain=[('type', '=', 'other')], help='Used for real-time inventory valuation. When set on a virtual location (non internal type), this account will be used to hold the value of products being moved from an internal location into this location, instead of the generic Stock Output Account set on the product. This has no effect for internal locations.'),
     'valuation_out_account_id': fields.many2one('account.account', 'Stock Valuation Account (Outgoing)', track_visibility='onchange', domain=[('type', '=', 'other')], help='Used for real-time inventory valuation. When set on a virtual location (non internal type), this account will be used to hold the value of products being moved out of this location and into an internal location, instead of the generic Stock Output Account set on the product. This has no effect for internal locations.'),
     'partner_id_diadiem': fields.many2one('res.partner', 'Location Address', track_visibility='onchange', help='Address of  customer or supplier.'),
     'loai_hinh': fields.selection([('kho_congty', 'Kho c\xc3\xb4ng ty'), ('kho_taptrung', 'Kho t\xe1\xba\xadp trung'), ('kho_daily', 'Kho \xc4\x91\xe1\xba\xa1i l\xc3\xbd')], 'Lo\xe1\xba\xa1i h\xc3\xacnh', track_visibility='onchange'),
     'allow': fields.boolean('Cho ph\xc3\xa9p t\xe1\xbb\x93n kho \xc3\xa2m', track_visibility='onchange')}
    _defaults = {'active': True,
     'usage': 'internal',
    ...
    def _product_get_report(self, cr, uid, ids, product_ids = False, context = None, recursive = False):
        ...
        for id in ids:
            ...
            for uom_id in products_by_uom.keys():
                ...
                ctx = context.copy()
                ctx['uom'] = uom_id
                qty = fnc(cr, uid, id, [ x.id for x in products_by_uom[uom_id] ], context=ctx)
                for product_id in qty.keys():
                    ...
                    total_price += price
                    result['product'].append({'price': amount_unit,
...
class stock_tracking(osv.osv):
    ...
    _columns = {'name': fields.char('Pack Reference', size=64, required=True, select=True, help='By default, the pack reference is generated following the sscc standard. (Serial number + 1 check digit)'),
    ...
    def name_get(self, cr, uid, ids, context = None):
        ...
        if not len(ids):
            return []
        res = [ (r['id'], r['serial'] and '%s [%s]' % (r['name'], r['serial']) or r['name']) for r in self.read(cr, uid, ids, ['name', 'serial'], context=context) ]
        return res
...
class stock_picking(osv.osv):
    ...
    def create(self, cr, user, vals, context = None):
        ...
        self.merge_move_line(cr, user, new_id, context=context)
        return new_id

    _columns = {'name': fields.char('Reference', size=64, select=True, states={'done': [('readonly', True)],
              'cancel': [('readonly', True)]}),
     'origin': fields.char('Source Document', size=64, states={'done': [('readonly', True)],
                'cancel': [('readonly', True)]}, help='Reference of the document', select=True),
     'backorder_id': fields.many2one('stock.picking', 'Back Order of', states={'done': [('readonly', True)],
                      'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'type': fields.selection([('out', 'Sending Goods'), ('in', 'Getting Goods'), ('internal', 'Internal')], 'Shipping Type', required=True, select=True, help='Shipping type specify, goods coming in or going out.', states={'done': [('readonly', True)],
              'cancel': [('readonly', True)]}),
     ...
     'location_id': fields.many2one('stock.location', 'Location', readonly=True, states={'draft': [('readonly', False)],
                     ...
                     'assigned': [('readonly', False)]}, help='Keep empty if you produce at the location where the finished products are needed.Set a location if you produce at a fixed location. This can be a partner location if you subcontract the manufacturing operations.', select=True),
     'location_dest_id': fields.many2one('stock.location', 'Dest. Location', states={'done': [('readonly', True)],
                          'cancel': [('readonly', True)]}, help='Location where the system will stock the finished products.', select=True),
     'move_type': fields.selection([('direct', 'Partial'), ('one', 'All at once')], 'Delivery Method', required=True, states={'done': [('readonly', True)],
                   'cancel': [('readonly', True)]}, help='It specifies goods to be deliver partially or all at once'),
     'state': fields.selection([('draft', 'Draft'),
               ('cancel', 'Cancelled'),
               ('auto', 'Waiting Another Operation'),
               ('confirmed', 'Waiting Availability'),
               ('assigned', 'Ready to Transfer'),
               ('done', 'Transferred')], 'Status', readonly=True, select=True, track_visibility='onchange', help="\n            * Draft: not confirmed yet and will not be scheduled until confirmed\n\n            * Waiting Another Operation: waiting for another move to proceed before it becomes automatically available (e.g. in Make-To-Order flows)\n\n            * Waiting Availability: still waiting for the availability of products\n\n            * Ready to Transfer: products reserved, simply waiting for confirmation.\n\n            * Transferred: has been processed, can't be modified or cancelled anymore\n\n            * Cancelled: has been cancelled, can't be confirmed anymore"),
     ...
     'date': fields.datetime('Creation Date', help='Creation date, usually the time of the order.', select=True, states={'done': [('readonly', True)],
              'cancel': [('readonly', True)]}),
     'date_done': fields.datetime('Ng\xc3\xa0y ho\xc3\xa0n t\xe1\xba\xa5t', help='Date of Completion', states={'done': [('readonly', True)],
                   'cancel': [('readonly', True)]}),
     'max_date': fields.function(get_min_max_date, fnct_inv=_set_maximum_date, multi='min_max_date', store=True, type='datetime', string='Max. Expected Date', select=2),
     'move_lines': fields.one2many('stock.move', 'picking_id', 'Internal Moves', states={'done': [('readonly', True)],
                    'cancel': [('readonly', True)]}),
     ...
     'partner_id': fields.many2one('res.partner', 'Partner', states={'done': [('readonly', True)],
                    'cancel': [('readonly', True)]}),
     'invoice_state': fields.selection([('invoiced', 'Invoiced'), ('2binvoiced', 'To Be Invoiced'), ('none', 'Not Applicable')], 'Invoice Control', select=True, required=True, readonly=True, track_visibility='onchange', states={'draft': [('readonly', False)]}),
     'company_id': fields.many2one('res.company', 'Company', required=True, select=True, states={'done': [('readonly', True)],
                    'cancel': [('readonly', True)]}),
     'loai_xuatkho': fields.selection([('thongthuong', 'VAT'), ('noibo', 'N\xe1\xbb\x99i b\xe1\xbb\x99')], 'Lo\xe1\xba\xa1i xu\xe1\xba\xa5t kho'),
     'tach_id': fields.many2one('stock.picking', 'Back Order of', states={'done': [('readonly', True)],
                 'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'tach_ids': fields.one2many('stock.picking', 'tach_id', 'Back Order of', states={'done': [('readonly', True)],
                  'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'backorder_ids': fields.one2many('stock.picking', 'backorder_id', 'Back Order of', states={'done': [('readonly', True)],
                       'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'nguoi_lap_phieu': fields.many2one('res.users', 'Ng\xc6\xb0\xe1\xbb\x9di l\xe1\xba\xadp phi\xe1\xba\xbfu')}
    _defaults = {'loai_xuatkho': 'thongthuong',
    ...
    def tach_phieu(self, cr, uid, ids, context = None):
            ...
            if count > 1:
                ...
                for check_chitiet in cr.dictfetchall():
                    ...
                    if location_id_start != location_id_split:
                        ...
                        new_picking = self.create(cr, uid, {'name': a,
                         'state': 'done',
                        ...
                        check += 1
                        for move_line_tach in self.browse(cr, uid, pick_id, context=context).move_lines:
        ...
        if check > 0:
            return {'name': _('Phi\xe1\xba\xbfu xu\xe1\xba\xa5t kho \xc4\x91\xc3\xa3 ho\xc3\xa0n t\xe1\xba\xa5t'),
             'domain': "[('tach_id', 'in', [" + str(ids[0]) + '])]',
             'view_type': 'form',
             'view_mode': 'tree,form',
    ...
    def action_process(self, cr, uid, ids, context = None):
        if context is None:
            context = {}
        query_kh = "select * from stock_location where name='Customers' or name='C\xc3\xa1c kh\xc3\xa1ch h\xc3\xa0ng' "
        cr.execute(query_kh)
        ...
        for pick in self.browse(cr, uid, ids, context):
            if pick.type == 'out':
                ...
                if type_kho == 'kho_taptrung':
                    loai_kho = 'Kho t\xe1\xba\xadp trung'
                if type_kho == 'kho_daily':
                    loai_kho = 'Kho \xc4\x91\xe1\xba\xa1i l\xc3\xbd'
                if type_kho == 'kho_congty':
                    loai_kho = 'Kho c\xc3\xb4ng ty'
                loai_xuatkho = pick.loai_xuatkho
                ...
                for move in pick.move_lines:
                    ...
                    if type_kho != location_id and move.state != 'cancel':
                        raise osv.except_osv(_('Th\xc3\xb4ng b\xc3\xa1o!'), _('Lo\xe1\xba\xa1i h\xc3\xacnh kho \xe1\xbb\x9f \xc4\x90\xe1\xbb\x8ba \xc4\x91i\xe1\xbb\x83m ngu\xe1\xbb\x93n ph\xe1\xba\xa3i gi\xe1\xbb\x91ng lo\xe1\xba\xa1i h\xc3\xacnh kho tr\xc3\xaan phi\xe1\xba\xbfu xu\xe1\xba\xa5t kho'))
                    ...
                    if loai_xuatkho == 'noibo':
                        ...
                        if location_dest_id == 9:
                            raise osv.except_osv(_('Th\xc3\xb4ng b\xc3\xa1o!'), _('\xc4\x90\xe1\xbb\x8ba \xc4\x91i\xe1\xbb\x83m \xc4\x91\xc3\xadch kh\xc3\xb4ng \xc4\x91\xc6\xb0\xe1\xbb\xa3c l\xc3\xa0 %s .') % move.location_dest_id.name)
                    if type_kho == 'kho_taptrung' or type_kho == 'kho_daily':
                        if location_id == kho_khach_hang:
                            raise osv.except_osv(_('Th\xc3\xb4ng b\xc3\xa1o!'), _('Vui l\xc3\xb2ng ch\xe1\xbb\x8dn \xc4\x91\xe1\xbb\x8ba \xc4\x91i\xe1\xbb\x83m ngu\xe1\xbb\x93n tr\xc3\xaan chi ti\xe1\xba\xbft giao h\xc3\xa0ng. \xc4\x90\xe1\xbb\x8ba \xc4\x91i\xe1\xbb\x83m ngu\xe1\xbb\x93n \xc4\x91\xe1\xbb\x91i v\xe1\xbb\x9bi Kho \xc4\x91\xe1\xba\xa1i l\xc3\xbd v\xc3\xa0 Kho t\xe1\xba\xadp trung ph\xe1\xba\xa3i kh\xc3\xa1c %s.') % move.location_id.name)
        ...
        if res:
            for move in picking_obj.move_lines:
                ...
                if state == 'cancel' or move.state == 'cancel':
                    ...
                    move_obj.write(cr, uid, [move.id], {'tracking_id': False,
                     'prodlot_id': False,
                     'move_history_ids2': [(6, 0, [])],
                     'move_history_ids': [(6, 0, [])]})
                else:
                    move_obj.write(cr, uid, [move.id], {'tracking_id': False,
                     'prodlot_id': False,
                     'move_history_ids2': [(6, 0, [])],
    ...
    def action_confirm(self, cr, uid, ids, context = None):
        ...
        if len(todo):
            self.pool.get('stock.move').write(cr, uid, todo, {'date': picking.date_done or time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
    ...
    def draft_validate(self, cr, uid, ids, context = None):
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            self.pool.get('stock.move').write(cr, uid, move_ids, {'date': pick.date_done or time.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
            self.pool.get('stock.move').force_assign(cr, uid, move_ids)
            wf_service.trg_write(uid, 'stock.picking', pick.id, cr)
    ...
    def action_done(self, cr, uid, ids, context = None):
        ...
        @return: True
        """
		ngayxk=data.date_done
		datepxk = datetime.strptime(ngayxk, "%Y-%m-%d")
		date_done = datepxk + timedelta(days=0, hours=7)	
    ...
    def _prepare_invoice_group(self, cr, uid, picking, partner, invoice, context = None):
        ...
        return {'name': (invoice.name or '') + ', ' + (picking.name or ''),
    ...
    def _prepare_invoice(self, cr, uid, picking, partner, inv_type, journal_id, context = None):
        ...
        if picking.sale_id:
            ...
            lxh = False
            km = False
        invoice_vals = {'name': picking.name,
    ...
    def action_invoice_create(self, cr, uid, ids, journal_id = False, group = False, type = 'out_invoice', context = None):
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            ...
            if count > 1:
                raise osv.except_osv(_('C\xe1\xba\xa3nh b\xc3\xa1o!'), _('Tr\xc6\xb0\xe1\xbb\x9dng h\xe1\xbb\xa3p xu\xe1\xba\xa5t tr\xc3\xaan c\xc3\xa1c kho kh\xc3\xa1c nhau th\xc3\xac ph\xe1\xba\xa3i t\xc3\xa1ch phi\xe1\xba\xbfu. \xc4\x90\xe1\xbb\x81 ngh\xe1\xbb\x8b t\xc3\xa1ch phi\xe1\xba\xbfu xu\xe1\xba\xa5t kho tr\xc6\xb0\xe1\xbb\x9bc khi t\xe1\xba\xa1o h\xc3\xb3a \xc4\x91\xc6\xa1n!'))
            ...
            invoice_obj.button_compute(cr, uid, [invoice_id], context=context, set_total=inv_type in ('in_invoice', 'in_refund'))
            self.write(cr, uid, [picking.id], {'invoice_state': 'invoiced',
             'hoa_don_id': invoice_id}, context=context)
    ...
    def allow_cancel(self, cr, uid, ids, context = None):
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            if nguoilap:
                if nguoilap.id != uid:
                    raise osv.except_osv(_('Th\xc3\xb4ng b\xc3\xa1o!'), _('B\xe1\xba\xa1n kh\xc3\xb4ng c\xc3\xb3 quy\xe1\xbb\x81n h\xe1\xbb\xa7y PXK. Vui l\xc3\xb2ng li\xc3\xaan h\xe1\xbb\x87 v\xe1\xbb\x9bi ng\xc6\xb0\xe1\xbb\x9di l\xe1\xba\xadp phi\xe1\xba\xbfu: %s \xc4\x91\xe1\xbb\x83 \xc4\x91\xc6\xb0\xe1\xbb\xa3c h\xe1\xbb\xa7y PXK.') % nguoilap.name)
            if not pick.move_lines:
                return True
            for move in pick.move_lines:
                if move.state == 'done':
        ...
        for pick in self.browse(cr, uid, ids, context=context):
            ...
            for move in pick.move_lines:
                ...
                if move.product_qty == partial_qty[move.id]:
                    ...
                    if move.location_id.allow == False:
                        if round(product_qty, 4) > round(tonkho, 4):
                            raise osv.except_osv(_('L\xe1\xbb\x97i!'), _('S\xe1\xba\xa3n ph\xe1\xba\xa9m trong kho %s kh\xc3\xb4ng \xc4\x91\xe1\xbb\xa7 \xc4\x91\xe1\xbb\x83 th\xe1\xbb\xb1c hi\xe1\xbb\x87n vi\xe1\xbb\x87c giao h\xc3\xa0ng!') % a)
                ...
                if pick.type == 'in' and move.product_id.cost_method == 'average':
                    ...
                    if qty > 0:
                        ...
                        move_obj.write(cr, uid, [move.id], {'price_unit': product_price,
            ...
            for move in too_few:
                ...
                if not new_picking:
                    ...
                    new_picking = self.copy(cr, uid, pick.id, {'date_done': partial_datas.get('delivery_date'),
                     'date': partial_datas.get('delivery_date'),
                     'move_lines': [],
                     'state': 'draft',
                     'ma_vung': matinh,
                    ...
                    self.write(cr, uid, [pick.id], {'ma_vung': False,
                     'daidien_muahang': False,
                ...
                if product_qty != 0:
                    ...
                    if move.location_id.allow == False:
                        if round(product_qty, 4) > round(tonkho, 4):
                            raise osv.except_osv(_('L\xe1\xbb\x97i!'), _('S\xe1\xba\xa3n ph\xe1\xba\xa9m trong kho kh\xc3\xb4ng \xc4\x91\xe1\xbb\xa7 \xc4\x91\xe1\xbb\x83 th\xe1\xbb\xb1c hi\xe1\xbb\x87n vi\xe1\xbb\x87c giao h\xc3\xa0ng!'))
                    prodlot_id = prodlot_ids[move.id]
                    if prodlot_id:
                        defaults.update(prodlot_id=prodlot_id)
                    move_obj.copy(cr, uid, move.id, defaults)
                move_obj.write(cr, uid, [move.id], {'product_qty': move.product_qty - partial_qty[move.id],
            ...
            if new_picking:
                ...
                self.action_move(cr, 1, [pick.id], context=context)
                ...
                if fist_1 == 'OUT':
                    ...
                    self.write(cr, uid, ids, {'name': name,
                     'date_done': partial_datas.get('delivery_date'),
                     'date': partial_datas.get('delivery_date'),
                     'nguoi_lap_phieu': partial_datas.get('user_id') or uid}, context)
                else:
                    self.write(cr, uid, ids, {'date_done': partial_datas.get('delivery_date'),
                     'date': partial_datas.get('delivery_date')}, context)
            delivered_pack = self.browse(cr, uid, delivered_pack_id, context=context)
            res[pick.id] = {'delivered_picking': delivered_pack.id or False}

        return res

    _VIEW_LIST = {'out': 'view_picking_out_form',
...
class stock_production_lot(osv.osv):
    ...
    _columns = {'name': fields.char('Serial Number', size=64, required=True, help='Unique Serial Number, will be displayed as: PREFIX/SERIAL [INT_REF]'),
...
class stock_production_lot_revision(osv.osv):
    ...
    _columns = {'name': fields.char('Revision Name', size=64, required=True),
    ...
    _defaults = {'author_id': lambda x, y, z, c: z,
...
class stock_move(osv.osv):
    ...
    def action_partial_move(self, cr, uid, ids, context = None):
        ...
        partial_id = self.pool.get('stock.partial.move').create(cr, uid, {}, context=context)
        return {'name': _('Products to Process'),
         'view_mode': 'form',
         'view_id': False,
         'view_type': 'form',
    ...
    _columns = {'name': fields.char('Description', select=True, states={'done': [('readonly', True)]}),
     ...
     'product_uom': fields.many2one('product.uom', '\xc4\x90\xc6\xa1n v\xe1\xbb\x8b', required=True, states={'done': [('readonly', True)]}),
     'product_uos_qty': fields.float('S\xe1\xbb\x91 l\xc6\xb0\xe1\xbb\xa3ng', digits_compute=dp.get_precision('Product Unit of Measure'), states={'done': [('readonly', True)]}),
     'product_uos': fields.many2one('product.uom', '\xc4\x90\xc6\xa1n v\xe1\xbb\x8b', states={'done': [('readonly', True)]}),
     'product_packaging': fields.many2one('product.packaging', 'Packaging', help='It specifies attributes of packaging like type, quantity of packaging,etc.'),
     'location_id': fields.many2one('stock.location', 'Kho xu\xe1\xba\xa5t', required=True, select=True, states={'done': [('readonly', False)]}, help='Sets a location if you produce at a fixed location. This can be a partner location if you subcontract the manufacturing operations.'),
     'location_dest_id': fields.many2one('stock.location', 'Destination Location', required=True, states={'done': [('readonly', False)]}, select=True, help='Location where the system will stock the finished products.'),
     ...
     'state': fields.selection([('draft', 'New'),
               ...
               ('confirmed', 'Waiting Availability'),
               ('assigned', 'Available'),
               ('done', 'Done')], 'Status', readonly=True, select=True, help="* New: When the stock move is created and not yet confirmed.\n* Waiting Another Move: This state can be seen when a move is waiting for another one, for example in a chained flow.\n* Waiting Availability: This state is reached when the procurement resolution is not straight forward. It may need the scheduler to run, a component to me manufactured...\n* Available: When products are reserved, it is set to 'Available'.\n* Done: When the shipment is processed, the state is 'Done'."),
     ...
     'company_id': fields.many2one('res.company', 'Company', required=True, select=True, states={'done': [('readonly', True)]}),
     'backorder_id': fields.related('picking_id', 'backorder_id', type='many2one', relation='stock.picking', string='PXK li\xc3\xaan quan', select=True),
    ...
    _constraints = [(_check_tracking, 'You must assign a serial number for this product.', ['prodlot_id']), (_check_location, 'You cannot move products from or to a location of the type view.', ['location_id', 'location_dest_id']), (_check_product_lot, 'You try to assign a lot which is not from the same product.', ['prodlot_id'])]
    ...
    _defaults = {'location_id': _default_location_source,
    ...
    def copy(self, cr, uid, id, default = None, context = None):
        if default is None:
            default = {}
        default = default.copy()
        default.update({'move_history_ids2': [],
         'move_history_ids': [],
         'check': False})
    ...
    def onchange_lot_id(self, cr, uid, ids, prodlot_id = False, product_qty = False, loc_id = False, product_id = False, uom_id = False, context = None):
        ...
        if location.usage == 'internal' and product_qty > (amount_actual or 0.0):
            warning = {'title': _('Insufficient Stock for Serial Number !'),
             'message': _('You are moving %.2f %s but only %.2f %s available for this serial number.') % (product_qty,
                         uom.name,
                         amount_actual,
                         uom.name)}
        ...
        if ids:
            for move in self.read(cr, uid, ids, ['product_qty']):
                if product_qty < move['product_qty']:
                    warning.update({'title': _('Th\xc3\xb4ng b\xc3\xa1o'),
                     'message': _('B\xe1\xba\xa1n \xc4\x91\xc3\xa3 gi\xe1\xba\xa3m kh\xe1\xbb\x91i l\xc6\xb0\xe1\xbb\xa3ng tr\xc3\xaan PXK.S\xe1\xbb\x91 l\xc6\xb0\xe1\xbb\xa3ng m\xe1\xbb\x9bi \xc4\x91\xc6\xb0\xe1\xbb\xa3c nh\xe1\xba\xadp th\xc3\xaam: S\xe1\xba\xbd ch\xc3\xaanh l\xe1\xbb\x87ch so v\xe1\xbb\x9bi LXH. Nh\xe1\xba\xa5n x \xc4\x91\xe1\xbb\x83 tho\xc3\xa1t n\xe1\xba\xbfu b\xe1\xba\xa1n kh\xc3\xb4ng mu\xe1\xbb\x91n ti\xe1\xba\xbfp t\xe1\xbb\xa5c.')})
                break
    ...
    def onchange_uos_quantity(self, cr, uid, ids, product_id, product_uos_qty, product_uos, product_uom):
        ...
        for move in self.read(cr, uid, ids, ['product_uos_qty']):
            if product_uos_qty < move['product_uos_qty']:
                warning.update({'title': _('Warning: No Back Order'),
                 'message': _('By changing the quantity here, you accept the new quantity as complete: OpenERP will not automatically generate a Back Order.')})
                break
    ...
    def onchange_move_type(self, cr, uid, ids, type, context = None):
        ...
        return {'value': {'location_id': source_location and source_location[1] or False,
                   'location_dest_id': dest_location and dest_location[1] or False}}
        ...
        for m in moves:
            ...
            if dest:
                if dest[1] == 'transparent':
                    ...
                    self.write(cr, uid, [m.id], {'date': newdate,
                     'location_dest_id': dest[0].id})
                    if m.picking_id and (dest[3] or dest[5]):
                        self.pool.get('stock.picking').write(cr, uid, [m.picking_id.id], {'stock_journal_id': dest[3] or m.picking_id.stock_journal_id.id,
                         'type': dest[5] or m.picking_id.type}, context=context)
        ...
        for picking, todo in self._chain_compute(cr, uid, moves, context=context).items():
            ...
            if picking:
                ...
                if old_ptype != picking.type:
                    ...
                    self.pool.get('stock.picking').write(cr, uid, [picking.id], {'name': old_pick_name,
                     'type': old_ptype}, context=context)
            else:
                pickid = False
            for move, (loc, dummy, delay, dummy, company_id, ptype, invoice_state) in todo:
                new_id = move_obj.copy(cr, uid, move.id, {'location_id': move.location_dest_id.id,
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.state in ('confirmed', 'waiting'):
                ...
                if res:
                    ...
                    while res:
                        r = res.pop(0)
                        move_id = self.copy(cr, uid, move.id, {'product_uos_qty': product_uos_qty,
                         'product_qty': r[0],
                         'location_id': r[1]})
                        done.append(move_id)
    ...
    def _get_accounting_data_for_valuation(self, cr, uid, move, context = None):
        ...
        if not acc_src:
            raise osv.except_osv(_('Error!'), _('Please define stock input account for this product or its category: "%s" (id: %d)') % (move.product_id.name, move.product_id.id))
        if not acc_dest:
            raise osv.except_osv(_('Error!'), _('Please define stock output account for this product or its category: "%s" (id: %d)') % (move.product_id.name, move.product_id.id))
        if not journal_id:
            raise osv.except_osv(_('Error!'), _('Please define journal on the product category: "%s" (id: %d)') % (move.product_id.categ_id.name, move.product_id.categ_id.id))
        if not acc_valuation:
            raise osv.except_osv(_('Error!'), _('Please define inventory valuation account on the product category: "%s" (id: %d)') % (move.product_id.categ_id.name, move.product_id.categ_id.id))
        return (journal_id,
         acc_src,
         acc_dest,
         acc_valuation)
    ...
    def _create_product_valuation_moves(self, cr, uid, move, context = None):
        ...
        if move.product_id.valuation == 'real_time':
            ...
            for j_id, move_lines in account_moves:
                move_obj.create(cr, uid, {'journal_id': j_id,
                 'line_id': move_lines,
    ...
    def unlink(self, cr, uid, ids, context = None):
        ...
        ctx = context.copy()
        for move in self.browse(cr, uid, ids, context=context):
            if move.state != 'draft' and move.product_qty > 0 and not ctx.get('call_unlink', False):
                raise osv.except_osv(_('Th\xc3\xb4ng b\xc3\xa1o!'), _('Kh\xc3\xb4ng th\xe1\xbb\x83 x\xc3\xb3a d\xc3\xb2ng chi ti\xe1\xba\xbft khi s\xe1\xbb\x91 l\xc6\xb0\xe1\xbb\xa3ng giao v\xc3\xa0 tr\xe1\xba\xa1ng th\xc3\xa1i kh\xc3\xa1c d\xe1\xbb\xb1 th\xe1\xba\xa3o.'))
    ...
    def _create_lot(self, cr, uid, ids, product_id, prefix = False):
        ...
        prodlot_id = prodlot_obj.create(cr, uid, {'prefix': prefix,
    ...
    def action_scrap(self, cr, uid, ids, quantity, location_id, context = None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            default_val = {'location_id': source_location.id,
    ...
    def action_split(self, cr, uid, ids, quantity, split_by_qty = 1, prefix = False, with_lot = True, context = None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            update_val = {'product_qty': split_by_qty,
             'product_uos_qty': uos_qty}
    ...
    def do_partial(self, cr, uid, ids, partial_datas, context = None):
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if not partial_data:
                ...
                if move.picking_id.type == 'in' and move.product_id.cost_method == 'average':
                    ...
                    product_obj.write(cr, uid, [product.id], {'standard_price': new_std_price})
                    self.write(cr, uid, [move.id], {'price_unit': product_price,
                     'price_currency_id': product_currency,
                     'check': False})
        ...
        for move in too_few:
            ...
            self.write(cr, uid, [move.id], {'product_qty': move.product_qty - product_qty,
        ...
        for move in too_many:
            self.write(cr, uid, [move.id], {'product_qty': move.product_qty,
...
class stock_inventory(osv.osv):
    ...
    _columns = {'name': fields.char('Inventory Reference', size=64, required=True, readonly=True, states={'draft': [('readonly', False)]}),
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            for line in inv.inventory_line_id:
                ...
                if change:
                    ...
                    value = {'name': _('INV:') + (line.inventory_id.name or ''),
                     'product_id': line.product_id.id,
                     'product_uom': line.product_uom.id,
                     'prodlot_id': lot_id,
                     'date': inv.date}
                    if change > 0:
                        value.update({'product_qty': change,
                ...
                if account_move_ids:
                    ...
                    for account_move in account_move_data_l:
                        if account_move['state'] == 'posted':
                            raise osv.except_osv(_('User Error!'), _('In order to cancel this inventory, you must first unpost related journal entries.'))
...
class stock_inventory_line(osv.osv):
    ...
    _order = 'location_id'
    _columns = {'inventory_id': fields.many2one('stock.inventory', 'Inventory', ondelete='cascade', select=True),
    ...
    def on_change_product_id(self, cr, uid, ids, location_id, product, uom = False, to_date = False):
        ...
        amount = self.pool.get('stock.location')._product_get(cr, uid, location_id, [product], {'uom': uom,
         'to_date': to_date,
         'compute_child': False})[product]
        result = {'product_qty': amount,
         'product_uom': uom,
         'prod_lot_id': False}
        return {'value': result}
...
class stock_warehouse(osv.osv):
    ...
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _columns = {'name': fields.char('Name', size=600, required=True, select=True, track_visibility='onchange'),
     'company_id': fields.many2one('res.company', 'Company', required=True, select=True, track_visibility='onchange'),
     'partner_id': fields.many2one('res.partner', 'Owner Address', domain=[('kho', '=', True)], track_visibility='onchange'),
     'lot_input_id': fields.many2one('stock.location', 'Location Input', required=True, domain=[('usage', '<>', 'view')], track_visibility='onchange'),
     'lot_stock_id': fields.many2one('stock.location', 'Location Stock', required=True, domain=[('usage', '=', 'internal')], track_visibility='onchange'),
     'lot_output_id': fields.many2one('stock.location', 'Location Output', required=True, domain=[('usage', '<>', 'view')], track_visibility='onchange'),
     'type_kho': fields.selection([('kho_congty', 'Kho c\xc3\xb4ng ty'), ('kho_taptrung', 'Kho t\xe1\xba\xadp trung'), ('kho_daily', 'Kho \xc4\x91\xe1\xba\xa1i l\xc3\xbd')], 'Lo\xe1\xba\xa1i h\xc3\xacnh kho', required=True, track_visibility='onchange'),
     'ma_kho': fields.char('M\xc3\xa3 Kho', size=256, required=True, select=True, track_visibility='onchange')}
    ...
    _defaults = {'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'stock.inventory', context=c),
    ...
    def create(self, cr, uid, vals, context = None):
        ...
        kho_id = super(stock_warehouse, self).create(cr, uid, vals, context=context)
        ...
        kh_id = partner_pool.create(cr, uid, {'name': name,
         'kho': True,
         'customer': False,
         'is_diadiem': True,
         'company_id': company_id,
         'street': name,
         'type_kho': type_kho}, context=context)
        self.write(cr, uid, [kho_id], {'partner_id': kh_id}, context=context)
        return kho_id
...
class stock_picking_in(osv.osv):
    ...
    _columns = {'backorder_id': fields.many2one('stock.picking.in', 'Back Order of', states={'done': [('readonly', True)],
                      'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'state': fields.selection([('draft', 'Draft'),
               ('auto', 'Waiting Another Operation'),
               ('confirmed', 'Waiting Availability'),
               ...
               ('done', 'Received'),
               ('cancel', 'Cancelled')], 'Status', readonly=True, select=True, help="* Draft: not confirmed yet and will not be scheduled until confirmed\n\n                 * Waiting Another Operation: waiting for another move to proceed before it becomes automatically available (e.g. in Make-To-Order flows)\n\n                 * Waiting Availability: still waiting for the availability of products\n\n                 * Ready to Receive: products reserved, simply waiting for confirmation.\n\n                 * Received: has been processed, can't be modified or cancelled anymore\n\n                 * Cancelled: has been cancelled, can't be confirmed anymore")}
    _defaults = {'type': 'in',
     'invoice_state': 'none'}
...
class stock_picking_out(osv.osv):
    ...
    _columns = {'backorder_id': fields.many2one('stock.picking.out', 'PXK li\xc3\xaan quan', states={'done': [('readonly', True)],
                      'cancel': [('readonly', True)]}, help='If this shipment was split, then this field links to the shipment which contains the already processed part.', select=True),
     'state': fields.selection([('draft', 'Draft'),
               ('auto', 'Waiting Another Operation'),
               ('confirmed', 'Waiting Availability'),

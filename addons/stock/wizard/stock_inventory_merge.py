...
class stock_inventory_merge(osv.osv_memory):
    ...
    def fields_view_get(self, cr, uid, view_id = None, view_type = 'form', context = None, toolbar = False, submenu = False):
        ...
        if context.get('active_model', '') == 'stock.inventory' and len(context['active_ids']) < 2:
            raise osv.except_osv(_('Warning!'), _('Please select multiple physical inventories to merge in the list view.'))
        return res
    ...
    def do_merge(self, cr, uid, ids, context = None):
        ...
        for inventory in invent_obj.browse(cr, uid, context['active_ids'], context=context):
            if inventory.state == 'done':
                raise osv.except_osv(_('Warning!'), _('Merging is only allowed on draft inventories.'))
        ...
        new_invent = invent_obj.create(cr, uid, {'name': 'Merged inventory'}, context=context)

...
class stock_partial_move_line(osv.osv_memory):
    ...
    _columns = {'wizard_id': fields.many2one('stock.partial.move', string='Wizard', ondelete='CASCADE')}
...
class stock_partial_move(osv.osv_memory):
    ...
    def do_partial(self, cr, uid, ids, context = None):
        assert len(ids) == 1 or AssertionError('Partial move processing may only be done one form at a time.')
        ...
        for move in partial.move_ids:
            ...
            partial_data[
                 'move%s' % move_id] = {'product_id': move.product_id.id,
            ...
            if move.move_id.picking_id.type == 'in' and move.product_id.cost_method == 'average':
                partial_data['move%s' % move_id].update(product_price=move.cost, product_currency=move.currency.id)

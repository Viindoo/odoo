...
class note_stage(osv.osv):
    ...
    _columns = {'name': fields.char('Stage Name', translate=True, required=True),
...
class note_tag(osv.osv):
    ...
    _columns = {'name': fields.char('Tag Name', required=True)}
...
class note_note(osv.osv):
    ...
    def onclick_note_is_done(self, cr, uid, ids, context = None):
        return self.write(cr, uid, ids, {'open': False,
         'date_done': fields.date.today()}, context=context)
    ...
    _columns = {'name': fields.function(_get_note_first_line, string='Note Summary', type='text', store=True),
     'memo': fields.html('Note Content'),
     'sequence': fields.integer('Sequence'),
     'stage_id': fields.function(_get_stage_per_user, fnct_inv=_set_stage_per_user, string='Stage', type='many2one', relation='note.stage'),
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset = 0, limit = None, context = None, orderby = False):
        if groupby and groupby[0] == 'stage_id':
            ...
            if current_stage_ids:
                ...
                result = [ {'__context': {'group_by': groupby[1:]},
                 '__domain': domain + [('stage_ids.id', '=', current_stage_id)],
                 'stage_id': (current_stage_id, stage_name[current_stage_id]),
                 'stage_id_count': self.search(cr, uid, domain + [('stage_ids', '=', current_stage_id)], context=context, count=True)} for current_stage_id in current_stage_ids ]
                ...
                if nb_notes_ws:
                    ...
                    if result and result[0]['stage_id'][0] == current_stage_ids[0]:
                        ...
                        result = [{'__context': {'group_by': groupby[1:]},
                          ...
                          'stage_id_count': nb_notes_ws}] + result
            else:
            ...
            return super(note_note, self).read_group(self, cr, uid, domain, fields, groupby, offset=offset, limit=limit, context=context, orderby=orderby)


class note_base_config_settings(osv.osv_memory):
    _inherit = 'base.config.settings'
    _columns = {'module_note_pad': fields.boolean('Use collaborative pads (etherpad)'),
...
class res_users(osv.Model):
    ...
    def create(self, cr, uid, data, context = None):
        ...
        if group_id in [ x.id for x in user.groups_id ]:
            for note_xml_id in ['note_stage_00',
             'note_stage_01',
             'note_stage_02',
             'note_stage_03',
             'note_stage_04']:
                ...
                note_obj.copy(cr, uid, stage_id, default={'user_id': user_id}, context=context)

...
try:
    ...
    _logger.warning('VAT validation partially unavailable because the `vatnumber` Python library cannot be found. Install it to support more countries, for example with `easy_install vatnumber`.')
    vatnumber = None
...
_ref_vat = {
     ...
     'be': 'BE0477472701',
     'bg': 'BG1234567892',
     'ch': 'CHE-123.456.788 TVA or CH TVA 123456',
     'cy': 'CY12345678F',
     'cz': 'CZ12345679',
     'de': 'DE123456788',
     'dk': 'DK12345674',
...
class res_partner(osv.osv):
    ...
    _columns = {'vat_subjected': fields.boolean('VAT Legal Statement', help='Check this box if the partner is subjected to the VAT. It will be used for the VAT legal statement.')}
    ...
    __check_vat_ch_re1 = re.compile('(MWST|TVA|IVA)[0-9]{6}$')
    __check_vat_ch_re2 = re.compile('E([0-9]{9}|-[0-9]{3}\\.[0-9]{3}\\.[0-9]{3})(MWST|TVA|IVA)$')
    ...
    def check_vat_ch(self, vat):
        ...
        if match:
            num = filter(lambda s: s.isdigit(), match.group(1))
    ...
    __check_vat_mx_re = re.compile('(?P<primeras>[A-Za-z\\xd1\\xf1&]{3,4})[ \\-_]?(?P<ano>[0-9]{2})(?P<mes>[01][0-9])(?P<dia>[0-3][0-9])[ \\-_]?(?P<code>[A-Za-z0-9&\\xd1\\xf1]{3})$')

    def check_vat_mx(self, vat):
        """ Mexican VAT verification
        
        Verificar RFC M\xc3\xa9xico
        """
    ...
    def check_vat_no(self, vat):
        ...
        sum = 3 * int(vat[0]) + 2 * int(vat[1]) + 7 * int(vat[2]) + 6 * int(vat[3]) + 5 * int(vat[4]) + 4 * int(vat[5]) + 3 * int(vat[6]) + 2 * int(vat[7])
        check = 11 - sum % 11
        if check == 11:
            check = 0
        if check == 10:

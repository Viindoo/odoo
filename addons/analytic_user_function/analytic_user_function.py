...
class account_analytic_account(osv.osv):
    ...
    _columns = {'user_product_ids': fields.one2many('analytic.user.funct.grid', 'account_id', 'Users/Products Rel.')}
...
class hr_analytic_timesheet(osv.osv):
    ...
    def on_change_account_id(self, cr, uid, ids, account_id, user_id = False, unit_amount = 0):
        ...
        if not a:
            raise osv.except_osv(_('Error!'), _('There is no expense account defined for this product: "%s" (id:%d)') % (r.product_id.name, r.product_id.id))
        if unit_amount:
            amount_unit = self.on_change_unit_amount(cr, uid, ids, r.product_id.id, unit_amount, False, r.product_id.uom_id.id)['value']['amount']

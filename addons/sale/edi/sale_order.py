...
SALE_ORDER_LINE_EDI_STRUCT = {'sequence': True,
 'name': True,
 'product_id': True,
 'product_uom': True,
 'price_unit': True,
 'discount': True,
 'price_subtotal': True}
SALE_ORDER_EDI_STRUCT = {'name': True,
 'origin': True,
 'company_id': True,
 'date_order': True,
 'partner_id': True,
...
class sale_order(osv.osv, EDIMixin):
    ...
    def edi_export(self, cr, uid, records, edi_struct = None, context = None):
        ...
        for order in records:
            ...
            edi_doc.update({'__import_model': 'purchase.order',
             ...
             'currency': self.pool.get('res.currency').edi_export(cr, uid, [order.pricelist_id.currency_id], context=context)[0],
    ...
    def _edi_import_company(self, cr, uid, edi_document, context = None):
        ...
        edi_document.pop('partner_address', None)
        return partner_id
    ...
    def _edi_get_pricelist(self, cr, uid, partner_id, currency, context = None):
        ...
        if not pricelist.currency_id == currency:
            ...
            match_pricelist_ids = product_pricelist.search(cr, uid, [('type', '=', pricelist_type), ('currency_id', '=', currency.id)])
    ...
    def _edi_paypal_url(self, cr, uid, ids, field, arg, context = None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            if order.order_policy in ('prepaid', 'manual') and order.company_id.paypal_account and order.state != 'draft':
                params = {'cmd': '_xclick',
    ...
    _columns = {'paypal_url': fields.function(_edi_paypal_url, type='char', string='Paypal Url')}
...
class sale_order_line(osv.osv, EDIMixin):
    ...
    def edi_export(self, cr, uid, records, edi_struct = None, context = None):
        ...
        for line in records:
            ...
            if line.product_uos:
                edi_doc.update(product_uom=line.product_uos, product_qty=line.product_uos_qty)
            edi_doc_list.append(edi_doc)

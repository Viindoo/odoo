...
class sale_advance_payment_inv(osv.osv_memory):
    ...
    _columns = {'advance_payment_method': fields.selection([('all', 'Invoice the whole sales order'),
                                ('percentage', 'Percentage'),
                                ('fixed', 'Fixed price (deposit)'),
                                ('lines', 'Some order lines')], 'What do you want to invoice?', required=True, help='Use All to create the final invoice.\n                Use Percentage to invoice a percentage of the total amount.\n                Use Fixed Price to invoice a specific amound in advance.\n                Use Some Order Lines to invoice a selection of the sales order lines.'),
     'qtty': fields.float('Quantity', digits=(16, 2), required=True),
     'product_id': fields.many2one('product.product', 'Advance Product', help="Select a product of type service which is called 'Advance Product'.\n                You may have to create it and set it as a default value on this field."),
    ...
    def _prepare_advance_invoice_vals(self, cr, uid, ids, context = None):
        ...
        for sale in sale_obj.browse(cr, uid, sale_ids, context=context):
            val = inv_line_obj.product_id_change(cr, uid, [], wizard.product_id.id, uom_id=False, partner_id=sale.partner_id.id, fposition_id=sale.fiscal_position.id)
            res = val['value']
    ...
    def create_invoices(self, cr, uid, ids, context = None):
        ...
        if wizard.advance_payment_method == 'lines':
            ...
            res['context'] = {'search_default_uninvoiced': 1,
             'search_default_order_id': sale_ids and sale_ids[0] or False}
            return res
        if not wizard.advance_payment_method in ('fixed', 'percentage'):
            ...
            return context.get('open_invoices', False) and self.open_invoices(cr, uid, ids, inv_ids, context=context)
        return {'type': 'ir.actions.act_window_close'}

...
class mail_group(osv.Model):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, translate=True),
     ...
     'public': fields.selection([('public', 'Public'), ('private', 'Private'), ('groups', 'Selected Group Only')], 'Privacy', required=True, help='This group is visible by non members.             Invisible groups can add members through the invite button.'),
     'group_public_id': fields.many2one('res.groups', string='Authorized Group'),
     'group_ids': fields.many2many('res.groups', rel='mail_group_res_group_rel', id1='mail_group_id', id2='groups_id', string='Auto Subscription', help='Members of those groups will automatically added as followers. Note that they will be able to manage their subscription manually if necessary.'),
     'image': fields.binary('Photo', help='This field holds the image used as photo for the group, limited to 1024x1024px.'),
     'image_medium': fields.function(_get_image, fnct_inv=_set_image, string='Medium-sized photo', type='binary', multi='_get_image', store={'mail.group': (lambda self, cr, uid, ids, c = {}: ids, ['image'], 10)}, help='Medium-sized photo of the group. It is automatically resized as a 128x128px image, with aspect ratio preserved. Use this field in form views or some kanban views.'),
     ...
     'alias_id': fields.many2one('mail.alias', 'Alias', ondelete='restrict', required=True, help='The email address associated with this group. New emails received will automatically create new topics.')}
    ...
    def create(self, cr, uid, vals, context = None):
        ...
        if not vals.get('alias_id'):
            ...
            alias_id = mail_alias.create_unique_alias(cr, uid, {'alias_name': 'group+' + vals['name']}, model_name=self._name, context=context)
        ...
        menu_id = mobj.create(cr, SUPERUSER_ID, {'name': vals['name'],
         'parent_id': menu_parent}, context=context)
        ...
        if ref:
            ...
            params = {'search_view_id': search_ref and search_ref[1] or False,
            ...
            newref = cobj.copy(cr, SUPERUSER_ID, ref[1], default={'params': str(params),
             'name': vals['name']}, context=context)
            mobj.write(cr, SUPERUSER_ID, menu_id, {'action': 'ir.actions.client,' + str(newref),
             'mail_group_id': mail_group_id}, context=context)
    ...
    def write(self, cr, uid, ids, vals, context = None):
        ...
        if vals.get('name'):
            mobj = self.pool.get('ir.ui.menu')
            mobj.write(cr, SUPERUSER_ID, [ group.menu_id.id for group in self.browse(cr, uid, ids, context=context) ], {'name': vals.get('name')}, context=context)
        return result

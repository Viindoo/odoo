...
class mail_alias(osv.Model):
    ...
    _columns = {'alias_name': fields.char('Alias', required=True, help="The name of the email alias, e.g. 'jobs' if you want to catch emails for <jobs@example.my.openerp.com>"),
     'alias_model_id': fields.many2one('ir.model', 'Aliased Model', required=True, ondelete='cascade', help='The model (OpenERP Document Kind) to which this alias corresponds. Any incoming email that does not reply to an existing record will cause the creation of a new record of this model (e.g. a Project Task)', domain="[('field_id.name', '=', 'message_ids')]"),
     'alias_user_id': fields.many2one('res.users', 'Owner', help='The owner of records created upon receiving emails on this alias. If this field is not set the system will attempt to find the right owner based on the sender (From) address, or will use the Administrator account if no system user is found for that address.'),
     'alias_defaults': fields.text('Default Values', required=True, help='A Python dictionary that will be evaluated to provide default values when creating new records for this alias.'),
     'alias_force_thread_id': fields.integer('Record Thread ID', help='Optional ID of a thread (record) to which all incoming messages will be attached, even if they did not reply to it. If set, this will disable the creation of new records completely.'),
     'alias_domain': fields.function(_get_alias_domain, string='Alias domain', type='char', size=None)}
    _defaults = {'alias_defaults': '{}',
    ...
    _sql_constraints = [('alias_unique', 'UNIQUE(alias_name)', 'Unfortunately this email alias is already used, please choose a unique one')]
    ...
    _constraints = [(_check_alias_defaults, 'Invalid expression, it must be a literal python dictionary definition e.g. "{\'field\': \'value\'}"', ['alias_defaults'])]
    ...
    def name_get(self, cr, uid, ids, context = None):
        ...
        """
        return [ (record['id'], '%s@%s' % (record['alias_name'], record['alias_domain'])) for record in self.read(cr, uid, ids, ['alias_name', 'alias_domain'], context=context) ]
    ...
    def migrate_to_alias(self, cr, child_model_name, child_table_name, child_model_auto_init_fct, alias_id_column, alias_key, alias_prefix = '', alias_force_key = '', alias_defaults = {}, context = None):
        ...
        try:
            ...
            _logger.warning("Table '%s': unable to set a NOT NULL constraint on column '%s' !\nIf you want to have it, you should update the records and execute manually:\nALTER TABLE %s ALTER COLUMN %s SET NOT NULL", child_table_name, 'alias_id', child_table_name, 'alias_id')
    ...
    def create_unique_alias(self, cr, uid, vals, model_name = None, context = None):
        ...
        alias_name = re.sub('[^\\w+.]+', '-', alias_name)

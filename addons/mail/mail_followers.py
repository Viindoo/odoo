...
class mail_followers(osv.Model):
    ...
    _columns = {'res_model': fields.char('Related Document Model', size=128, required=True, select=1, help='Model of the followed resource'),
     'res_id': fields.integer('Related Document ID', select=1, help='Id of the followed resource'),
     'partner_id': fields.many2one('res.partner', string='Related Partner', ondelete='cascade', required=True, select=1),
...
class mail_notification(osv.Model):
    ...
    _columns = {'partner_id': fields.many2one('res.partner', string='Contact', ondelete='cascade', required=True, select=1),
    ...
    def _notify(self, cr, uid, msg_id, partners_to_notify = None, context = None):
        ...
        if context.get('mail_notify_noemail'):
            ...
            mail_values = {'mail_message_id': msg.id,
             'auto_delete': True,
             'body_html': body_html,

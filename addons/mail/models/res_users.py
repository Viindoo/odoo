...
class Users(models.Model):
    ...
    def systray_get_activities(self):
        self.env["mail.activity"].flush_model(
            ["res_model_id", "res_model", "res_id", "date_deadline", "user_id"])
        today = fields.Date.context_today(self)
        self.env.cr.execute("""
            SELECT res_model_id, res_model
              FROM mail_activity
             WHERE user_id = %s AND res_model_id IS NOT NULL
          GROUP BY res_model_id, res_model
        """, [self.env.uid])
        model_rows = self.env.cr.fetchall()
        model_ids = [r[0] for r in model_rows]
        IrModel = self.env["ir.model"].sudo()
        user_activities = {}
        for res_model_id, model_name in model_rows:
            if not model_name or model_name not in self.env:
                continue
            Model = self.env[model_name]
            if not Model.check_access_rights("read", raise_exception=False):
                continue
            rule_query = Model._where_calc([])
            Model._apply_ir_rules(rule_query, "read")
            from_clause, where_clause, where_params = rule_query.get_sql()
            allowed_sub = 'SELECT "%s".id FROM %s' % (Model._table, from_clause)
            if where_clause:
                allowed_sub += " WHERE " + where_clause
            self.env.cr.execute("""
                SELECT COUNT(*) FILTER (WHERE a.date_deadline < %%s),
                       COUNT(*) FILTER (WHERE a.date_deadline = %%s),
                       COUNT(*) FILTER (WHERE a.date_deadline > %%s)
                  FROM mail_activity a
                 WHERE a.user_id = %%s AND a.res_model = %%s
                   AND a.res_id IN (%s)
            """ % allowed_sub,
                [today, today, today, self.env.uid, model_name] + list(where_params))
            overdue_count, today_count, planned_count = self.env.cr.fetchone()
            if not (overdue_count or today_count or planned_count):
                continue
            module = Model._original_module
            icon = module and modules.module.get_module_icon(module)
            model = IrModel.browse(res_model_id).with_prefetch(model_ids)

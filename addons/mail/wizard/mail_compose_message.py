...
class mail_compose_message(osv.TransientModel):
    ...
    _columns = {'composition_mode': fields.selection(lambda s, *a, **k: s._get_composition_mode_selection(*a, **k), string='Composition mode'),
     'partner_ids': fields.many2many('res.partner', 'mail_compose_message_res_partner_rel', 'wizard_id', 'partner_id', 'Additional contacts'),
     'attachment_ids': fields.many2many('ir.attachment', 'mail_compose_message_ir_attachments_rel', 'wizard_id', 'attachment_id', 'Attachments'),
     'filter_id': fields.many2one('ir.filters', 'Filters')}
    _defaults = {'composition_mode': 'comment',
    ...
    def check_access_rule(self, cr, uid, ids, operation, context = None):
        ...
        if operation == 'create' and uid != SUPERUSER_ID:
            ...
            for id, rmod, rid in cr.fetchall():
                message_values[id] = {'model': rmod,
                 'res_id': rid}

            author_ids = [ mid for mid, message in message_values.iteritems() if message.get('model') and not message.get('res_id') ]
    ...
    def get_message_data(self, cr, uid, message_id, context = None):
        ...
        if not message_id:
            ...
            result = {'record_name': message_data.record_name,
    ...
    def send_mail(self, cr, uid, ids, context = None):
        ...
        for wizard in self.browse(cr, uid, ids, context=context):
            ...
            for res_id in res_ids:
                post_values = {'subject': wizard.subject,
                 'body': wizard.body,
                ...
                if mass_mail_mode and wizard.model:
                    ...
                    for attach_id in post_values.pop('attachment_ids'):
                        new_attach_id = ir_attachment_obj.copy(cr, uid, attach_id, {'res_model': self._name,
                         'res_id': wizard.id}, context=context)
    ...
    def render_message(self, cr, uid, wizard, res_id, context = None):
        ...
        return {'subject': self.render_template(cr, uid, wizard.subject, wizard.model, res_id, context),
    ...
    def render_template(self, cr, uid, template, model, res_id, context = None):
        ...
        def merge(match):
            ...
            result = eval(exp, {'user': self.pool.get('res.users').browse(cr, uid, uid, context=context),

...
class ir_ui_menu(osv.osv):
    ...
    """
    _inherit = 'ir.ui.menu'
    _columns = {'mail_group_id': fields.many2one('mail.group', 'Mail Group')}
    ...
    def search(self, cr, uid, args, offset = 0, limit = None, order = None, context = None, count = False):
        ...
        for menu in self.browse(cr, uid, ids, context=context):
            if menu.mail_group_id:
                sub_ids = follower_obj.search(cr, SUPERUSER_ID, [('partner_id', '=', partner_id), ('res_model', '=', 'mail.group'), ('res_id', '=', menu.mail_group_id.id)], context=context)
                if not sub_ids:
                    ids.remove(menu.id)

...
class res_users(osv.Model):
    ...
    _columns = {'alias_id': fields.many2one('mail.alias', 'Alias', ondelete='restrict', required=True, help="Email address internally associated with this user. Incoming emails will appear in the user's notifications.")}
    ...
    def _auto_init(self, cr, context = None):
        ...
        self.pool.get('mail.alias').migrate_to_alias(cr, self._name, self._table, super(res_users, self)._auto_init, self._columns['alias_id'], 'login', alias_force_key='id', context=context)
    ...
    def _create_welcome_message(self, cr, uid, user, context = None):
        ...
        return self.pool.get('res.partner').message_post(cr, SUPERUSER_ID, [user.partner_id.id], body=body, context=context)
    ...
    def _message_post_get_pid(self, cr, uid, thread_id, context = None):
        if not thread_id:
            raise AssertionError('res.users does not support posting global messages')
            if context and 'thread_model' in context:
                context['thread_model'] = 'res.partner'
            thread_id = isinstance(thread_id, (list, tuple)) and thread_id[0]

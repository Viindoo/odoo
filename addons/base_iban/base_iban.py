...
class res_partner_bank(osv.osv):
    ...
    def _construct_constraint_msg(self, cr, uid, ids, context = None):
        ...
        if default_iban_check(iban_country):
            if iban_country in _ref_iban:
                return (_('The IBAN does not seem to be correct. You should have entered something like this %s'), '%s \nWhere B = National bank code, S = Branch code, C = Account No, K = Check digit' % _ref_iban[iban_country])
    ...
    _columns = {'iban': fields.related('acc_number', string='IBAN', size=34, readonly=True, help='International Bank Account Number', type='char')}

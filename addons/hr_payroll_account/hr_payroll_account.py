...
class hr_payslip(osv.osv):
    ...
    _columns = {'period_id': fields.many2one('account.period', 'Force Period', states={'draft': [('readonly', False)]}, readonly=True, domain=[('state', '<>', 'done')], help='Keep empty to use the period of the validation(Payslip) date.'),
    ...
    _defaults = {'journal_id': _get_default_journal}
        ...
        for slip in self.browse(cr, uid, ids, context=context):
            ...
            for line in slip.details_by_salary_rule_category:
                ...
                if credit_account_id:
                    credit_line = (0, 0, {'name': line.name,
                      'date': timenow,
            ...
            if debit_sum > credit_sum:
                ...
                adjust_credit = (0, 0, {'name': _('Adjustment Entry'),
                  'date': timenow,
                  'partner_id': False,
                  'account_id': acc_id,
                  'journal_id': slip.journal_id.id,
                ...
                adjust_debit = (0, 0, {'name': _('Adjustment Entry'),
                  'date': timenow,
                  'partner_id': False,
                  'account_id': acc_id,
                  'journal_id': slip.journal_id.id,
            ...
            self.write(cr, uid, [slip.id], {'move_id': move_id,
...
class hr_payslip_run(osv.osv):
    ...
    _columns = {'journal_id': fields.many2one('account.journal', 'Salary Journal', states={'draft': [('readonly', False)]}, readonly=True, required=True)}

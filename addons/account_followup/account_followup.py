...
class followup(osv.osv):
    ...
    _rec_name = 'name'
    _columns = {'followup_line': fields.one2many('account_followup.followup.line', 'followup_id', 'Follow-up'),
     'company_id': fields.many2one('res.company', 'Company', required=True),
     'name': fields.related('company_id', 'name', string='Name')}
    _defaults = {'company_id': lambda s, cr, uid, c: s.pool.get('res.company')._company_default_get(cr, uid, 'account_followup.followup', context=c)}
...
class followup_line(osv.osv):
    ...
    _columns = {'name': fields.char('Follow-Up Action', size=64, required=True),
    ...
    _constraints = [(_check_description, 'Your description is invalid, use the right legend or %% if you want to use the percent character.', ['description'])]
...
class account_move_line(osv.osv):
    ...
    _columns = {'followup_line_id': fields.many2one('account_followup.followup.line', 'Follow-up Level', ondelete='restrict'),
...
class res_partner(osv.osv):
    ...
    def _get_latest(self, cr, uid, ids, names, arg, context = None, company_id = None):
        ...
        for partner in self.browse(cr, uid, ids, context=context):
            ...
            for aml in amls:
                ...
                if aml.company_id == company and aml.blocked == False and aml.followup_line_id != False and (not latest_days_without_lit or latest_days_without_lit < aml.followup_line_id.delay):
    ...
    def get_followup_table_html(self, cr, uid, ids, context = None):
        ...
        if not len(ids) == 1:
            ...
            followup_table = ''
            company = partner.unreconciled_aml_ids and self.pool.get('res.users').browse(cr, uid, uid, context=context).company_id
        ...
        if vals.get('payment_responsible_id', False):
            for part in self.browse(cr, uid, ids, context=context):
                if part.payment_responsible_id != vals['payment_responsible_id']:
                    ...
                    self.pool.get('mail.thread').message_post(cr, uid, 0, body=_('You became responsible to do the next action for the payment follow-up of') + " <b><a href='#id=" + str(part.id) + "&view_type=form&model=res.partner'> " + part.name + ' </a></b>', type='comment', subtype='mail.mt_comment', context=context, model='res.partner', res_id=part.id, partner_ids=[responsible_partner_id])

        return super(res_partner, self).write(cr, uid, ids, vals, context=context)

    def action_done(self, cr, uid, ids, context = None):
        return self.write(cr, uid, ids, {'payment_next_action_date': False,
         'payment_next_action': '',
         'payment_responsible_id': False}, context=context)

    def do_button_print(self, cr, uid, ids, context = None):
        if not len(ids) == 1:
            raise AssertionError
            company_id = self.pool.get('res.users').browse(cr, uid, uid, context=context).company_id.id
            if not self.pool.get('account.move.line').search(cr, uid, [('partner_id', '=', ids[0]),
            ...
            raise followup_ids or osv.except_osv(_('Error!'), _('There is no followup plan defined for the current company.'))
    ...
    def _payment_earliest_date_search(self, cr, uid, obj, name, args, context = None):
        ...
        having_values = [ x[2] for x in args ]
        query = self.pool.get('account.move.line')._query_get(cr, uid, context=context)
        cr.execute("SELECT partner_id FROM account_move_line l WHERE account_id IN (SELECT id FROM account_account WHERE type='receivable' AND active) AND l.company_id = %s AND reconcile_id IS NULL AND " + query + ' AND partner_id IS NOT NULL GROUP BY partner_id HAVING ' + having_where_clause, [company_id] + having_values)
        res = cr.fetchall()
        if not res:
            return [('id', '=', '0')]
        return [('id', 'in', [ x[0] for x in res ])]
    ...
    _columns = {'payment_responsible_id': fields.many2one('res.users', ondelete='set null', string='Follow-up Responsible', help='Optionally you can assign a user to this field, which will make him responsible for the action.', track_visibility='onchange'),
     'payment_note': fields.text('Customer Payment Promise', help='Payment Note', track_visibility='onchange'),
     'payment_next_action': fields.text('Next Action', help='This is the next action to be taken.  It will automatically be set when the partner gets a follow-up level that requires a manual action. ', track_visibility='onchange'),
     'payment_next_action_date': fields.date('Next Action Date', help='This is when the manual follow-up is needed. The date will be set to the current date when the partner gets a follow-up level that requires a manual action. Can be practical to set manually e.g. to see if he keeps his promises.'),
     'unreconciled_aml_ids': fields.one2many('account.move.line', 'partner_id', domain=['&',
                              ...
                              ('account_id.type', '=', 'receivable'),
                              ('state', '!=', 'draft')]),
     'latest_followup_date': fields.function(_get_latest, method=True, type='date', string='Latest Follow-up Date', help='Latest date that the follow-up level of the partner was changed', store=False, multi='latest'),
     'latest_followup_level_id': fields.function(_get_latest, method=True, type='many2one', relation='account_followup.followup.line', string='Latest Follow-up Level', help='The maximum follow-up level', store=False, multi='latest'),
     'latest_followup_level_id_without_lit': fields.function(_get_latest, method=True, type='many2one', relation='account_followup.followup.line', string='Latest Follow-up Level without litigation', help='The maximum follow-up level without taking into account the account move lines with litigation', store=False, multi='latest'),
     'payment_amount_due': fields.function(_get_amounts_and_date, type='float', string='Amount Due', store=False, multi='followup', fnct_search=_payment_due_search),
     'payment_amount_overdue': fields.function(_get_amounts_and_date, type='float', string='Amount Overdue', store=False, multi='followup', fnct_search=_payment_overdue_search),
     'payment_earliest_due_date': fields.function(_get_amounts_and_date, type='date', string='Worst Due Date', multi='followup', fnct_search=_payment_earliest_date_search)}
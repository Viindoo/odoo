...
class account_followup_stat_by_partner(osv.osv):
    ...
    _columns = {'partner_id': fields.many2one('res.partner', 'Partner', readonly=True),
     ...
     'max_followup_id': fields.many2one('account_followup.followup.line', 'Max Follow Up Level', readonly=True, ondelete='cascade'),
...
class account_followup_sending_results(osv.osv_memory):
    ...
    _columns = {'description': fields.text('Description', readonly=True),
     'needprinting': fields.boolean('Needs Printing')}
    _defaults = {'needprinting': _get_need_printing,
...
class account_followup_print(osv.osv_memory):
    ...
    _columns = {'date': fields.date('Follow-up Sending Date', required=True, help='This field allow you to select a forecast date to plan your follow-ups'),
    ...
    def do_process(self, cr, uid, ids, context = None):
        ...
        context.update({'description': restot['resulttext'],
         ...
         'report_data': res})
        return {'name': _('Send Letters and Emails: Actions Summary'),
         'view_type': 'form',
         'context': context,
         'view_mode': 'tree,form',
    ...
    def _get_partners_followp(self, cr, uid, ids, context = None):
        ...
        cr.execute('SELECT * FROM account_followup_followup_line WHERE followup_id=%s ORDER BY delay', (fup_id,))
        ...
        return {'partner_ids': partner_list,
         'to_update': to_update}

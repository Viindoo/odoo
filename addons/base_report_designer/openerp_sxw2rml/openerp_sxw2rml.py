...
class DomApiGeneral:
    ...
    def __init__(self, content_string = '', file = ''):
        ...
        self.re_digits = re.compile('(.*?\\d)(pt|cm|mm|inch|in)')
...
class PyOpenOffice(object):
    ...
    def oo_replace(self, content):
        regex = [('<para[^>]*/>', ''), ('<para(.*)>(.*?)<text:line-break[^>]*/>', '<para$1>$2</para><para$1>')]
...
if __name__ == '__main__':
    import optparse
    parser = optparse.OptionParser(version='OpenERP Report v%s' % __version__, usage='openerp_sxw2rml.py [options] file.sxw')

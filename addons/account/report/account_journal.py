...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def tax_codes(self, period_id, journal_id):
        ids_journal_period = self.pool.get('account.journal.period').search(self.cr, self.uid, [('journal_id', '=', journal_id), ('period_id', '=', period_id)])
        self.cr.execute("select distinct tax_code_id from account_move_line where period_id=%s and journal_id=%s and tax_code_id is not null and state<>'draft'", (period_id, journal_id))
    ...
    def _sum_vat(self, period_id, journal_id, tax_code_id):
        self.cr.execute("select sum(tax_amount) from account_move_line where period_id=%s and journal_id=%s and tax_code_id=%s and state<>'draft'", (period_id, journal_id, tax_code_id))
    ...
    def _sum_debit(self, period_id = False, journal_id = False):
        ...
        if self.target_move == 'posted':
            move_state = ['posted']
        self.cr.execute('SELECT SUM(debit) FROM account_move_line l, account_move am WHERE l.move_id=am.id AND am.state IN %s AND l.period_id IN %s AND l.journal_id IN %s ' + self.query_get_clause + ' ', (tuple(move_state), tuple(period_id), tuple(journal_id)))
        return self.cr.fetchone()[0] or 0.0
    ...
    def _sum_credit(self, period_id = False, journal_id = False):
        ...
        if self.target_move == 'posted':
            move_state = ['posted']
        self.cr.execute('SELECT SUM(l.credit) FROM account_move_line l, account_move am WHERE l.move_id=am.id AND am.state IN %s AND l.period_id IN %s AND l.journal_id IN %s ' + self.query_get_clause + '', (tuple(move_state), tuple(period_id), tuple(journal_id)))
        return self.cr.fetchone()[0] or 0.0
    ...
    def lines(self, period_id, journal_id = False):
        ...
        self.cr.execute('update account_journal_period set state=%s where journal_id IN %s and period_id=%s and state=%s', ('printed',
         self.journal_ids,
         period_id,
         'draft'))
        move_state = ['draft', 'posted']
    ...
    def _set_get_account_currency_code(self, account_id):
        self.cr.execute('SELECT c.symbol AS code FROM res_currency c,account_account AS ac WHERE ac.id = %s AND ac.currency_id = c.id' % account_id)

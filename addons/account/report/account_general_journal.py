...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def _sum_debit_period(self, period_id, journal_id = False):
        ...
        if self.target_move == 'posted':
            move_state = ['posted']
        self.cr.execute('SELECT SUM(l.debit) FROM account_move_line l LEFT JOIN account_move am ON (l.move_id=am.id) WHERE am.state IN %s AND l.period_id=%s AND l.journal_id IN %s ' + self.query_get_clause + " AND l.state<>'draft'", (tuple(move_state), period_id, tuple(journals)))
    ...
    def _sum_credit_period(self, period_id, journal_id = None):
        ...
        if not journals:
            return 0.0
        self.cr.execute('SELECT SUM(l.credit) FROM account_move_line l LEFT JOIN account_move am ON (l.move_id=am.id) WHERE am.state IN %s AND l.period_id=%s AND l.journal_id IN %s ' + self.query_get_clause + " AND l.state<>'draft'", (tuple(move_state), period_id, tuple(journals)))
        return self.cr.fetchone()[0] or 0.0

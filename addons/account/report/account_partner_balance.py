...
class partner_balance(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type = None):
        ...
        self.cr.execute('SELECT a.id FROM account_account a LEFT JOIN account_account_type t ON (a.type = t.code) WHERE a.type IN %s AND a.active', (self.ACCOUNT_TYPE,))
    ...
    def _sum_debit(self):
        ...
        if not self.ids:
            return 0.0
        self.cr.execute('SELECT sum(debit) FROM account_move_line AS l JOIN account_move am ON (am.id = l.move_id)WHERE l.account_id IN %sAND am.state IN %sAND ' + self.query + '', (tuple(self.account_ids), tuple(move_state)))
    ...
    def _sum_credit(self):
        ...
        if not self.ids:
            return 0.0
        self.cr.execute('SELECT sum(credit) FROM account_move_line AS l JOIN account_move am ON (am.id = l.move_id)WHERE l.account_id IN %sAND am.state IN %sAND ' + self.query + '', (tuple(self.account_ids), tuple(move_state)))
        temp_res = float(self.cr.fetchone()[0] or 0.0)
        return temp_res
    ...
    def _sum_litige(self):
        ...
        if not self.ids:
            return 0.0
        self.cr.execute('SELECT sum(debit-credit) FROM account_move_line AS l JOIN account_move am ON (am.id = l.move_id)WHERE l.account_id IN %sAND am.state IN %sAND ' + self.query + ' AND l.blocked=TRUE ', (tuple(self.account_ids), tuple(move_state)))
        temp_res = float(self.cr.fetchone()[0] or 0.0)
        return temp_res

...
class account_analytic_cost_ledger(report_sxw.rml_parse):
    ...
    def _lines_a(self, general_account, account, date1, date2):
        self.cr.execute('SELECT aal.name AS name, aal.code AS code, aal.amount AS balance, aal.date AS date, aaj.code AS cj FROM account_analytic_line AS aal, account_analytic_journal AS aaj                 WHERE (aal.general_account_id=%s) AND (aal.account_id IN %s) AND (aal.date>=%s) AND (aal.date<=%s)                 AND (aal.journal_id=aaj.id)                 ORDER BY aal.date, aaj.code, aal.code', (general_account['id'],
         self._get_children([account]),
         date1,
         date2))
        res = self.cr.dictfetchall()
        for r in res:
    ...
    def _sum_debit(self, accounts, date1, date2):
        self.cr.execute('SELECT sum(amount) FROM account_analytic_line WHERE account_id IN %s AND date>=%s AND date<=%s AND amount>0', (self._get_children(accounts), date1, date2))

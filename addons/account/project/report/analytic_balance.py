...
class account_analytic_balance(report_sxw.rml_parse):
    ...
    def get_children(self, ids):
        read_data = self.pool.get('account.analytic.account').read(self.cr, self.uid, ids, ['child_ids',
         'code',
         'complete_name',
         'balance'])
        for data in read_data:
            if data['id'] not in self.acc_ids:
                inculde_empty = True
                if not self.empty_acc and data['balance'] == 0.0:
                    inculde_empty = False
                if inculde_empty:
    ...
    def _move_sum(self, account_id, date1, date2, option):
        ...
        if option == 'credit':
            self.cr.execute('SELECT COALESCE(-sum(amount),0.0) FROM account_analytic_line                     WHERE account_id IN %s AND date>=%s AND date<=%s AND amount<0', query_params)
        elif option == 'debit':
            self.cr.execute('SELECT COALESCE(sum(amount),0.0) FROM account_analytic_line                     WHERE account_id IN %s                        AND date>=%s AND date<=%s AND amount>0', query_params)
        elif option == 'quantity':
            self.cr.execute('SELECT COALESCE(sum(unit_amount),0.0) FROM account_analytic_line                 WHERE account_id IN %s                    AND date>=%s AND date<=%s', query_params)
...
report_sxw.report_sxw('report.account.analytic.account.balance', 'account.analytic.account', 'addons/account/project/report/analytic_balance.rml', parser=account_analytic_balance, header='internal')
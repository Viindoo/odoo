...
class product_uom_categ(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, translate=True, track_visibility='onchange')}
...
class product_uom(osv.osv):
    ...
    _order = 'name'
    _columns = {
         'name': fields.char('Unit of Measure', size=64, required=True, track_visibility='onchange'),
         'category_id': fields.many2one('product.uom.categ', 'Category', required=True, track_visibility='onchange', ondelete='cascade', help='Conversion between Units of Measure can only occur if they belong to the same category. The conversion will be made based on the ratios.'),
         'factor': fields.float('Ratio', required=True, track_visibility='onchange', digits=(12, 12), help='How much bigger or smaller this unit is compared to the reference Unit of Measure for this category:\n1 * (reference unit) = ratio * (this unit)'),
         'factor_inv': fields.function(_factor_inv, digits=(12, 12), fnct_inv=_factor_inv_write, track_visibility='onchange', string='Ratio', help='How many times this Unit of Measure is bigger than the reference Unit of Measure in this category:\n1 * (this unit) = ratio * (reference unit)', required=True),
         'rounding': fields.float('Rounding Precision', track_visibility='onchange', digits_compute=dp.get_precision('Product Unit of Measure'), required=True, help='The computed quantity will be a multiple of this value. Use 1.0 for a Unit of Measure that cannot be further split, such as a piece.'),
         'active': fields.boolean('Active', track_visibility='onchange', help='By unchecking the active field you can disable a unit of measure without deleting it.'),
         'uom_type': fields.selection([('bigger', 'Bigger than the reference Unit of Measure'), ('reference', 'Reference Unit of Measure for this category'), ('smaller', 'Smaller than the reference Unit of Measure')], 'Type', track_visibility='onchange', required=1)
         }
    _defaults = {
         'active': 1,
         'rounding': 0.01,
    ...
    _sql_constraints = [('factor_gt_zero', 'CHECK (factor!=0)', 'The conversion ratio for a unit of measure cannot be 0!')]
...
class product_ul(osv.osv):
    ...
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _columns = {
         'name': fields.char('Name', size=64, select=True, required=True, translate=True, track_visibility='onchange'),
         'type': fields.selection([('unit', 'Unit'),
                  ('pack', 'Pack'),
                  ('box', 'Box'),
                  ('pallet', 'Pallet')], 'Type', track_visibility='onchange', required=True)
                  }
...
class product_category(osv.osv):
    ...
    def name_get(self, cr, uid, ids, context = None):
        ...
        for record in reads:
            code = record['code']
            name = record['name']
            if record['parent_id'] and code:
                name = record['parent_id'][1] + ' / [' + code + ']' + name
            if record['parent_id'] and code == False:
                name = record['parent_id'][1] + ' / ' + name
            if not record['parent_id'] and code:
                name = '[' + code + ']' + name
            if not record['parent_id'] and not code:
                name = name
    ...
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _columns = {
         'name': fields.char('Name', size=64, required=True, select=True, track_visibility='onchange'),
         'complete_name': fields.function(_name_get_fnc, type='char', string='Name', track_visibility='onchange'),
         'parent_id': fields.many2one('product.category', 'Parent Category', select=True, ondelete='cascade', track_visibility='onchange'),
         'child_id': fields.one2many('product.category', 'parent_id', string='Child Categories', track_visibility='onchange'),
         'sequence': fields.integer('Sequence', select=True, help='Gives the sequence order when displaying a list of product categories.', track_visibility='onchange'),
         'type': fields.selection([('view', 'View'), ('normal', 'Normal')], 'Category Type', track_visibility='onchange', help='A category of the view type is a virtual category that can be used as the parent of another category to create a hierarchical structure.'),
         ...
         'check': fields.boolean('Đặt cố định', track_visibility='onchange'),
         'code': fields.char('Mã sản phẩm', size=500, required=True, track_visibility='onchange', select=True)
         }
    _defaults = {'type': lambda *a: 'normal'}
    ...
    def _check_code(self, cr, uid, ids, context = None):
        for categ in self.browse(cr, uid, ids, context=context):
            query = "select id from product_category where code = '" + str(categ.code) + "' and id!= " + str(categ.id)
            cr.execute(query)
            if cr.fetchall():
                return False
    ...
    _constraints = [(_check_recursion, 'Error ! You cannot create recursive categories.', ['parent_id']), (_check_code, 'Lỗi. Mã loại sản phẩm không được trùng nhau.', ['code'])]
...
class product_template(osv.osv):
    ...
    _columns = {
         ...
         'cost_method': fields.selection([('standard', 'Standard Price'), ('average', 'Average Price')], 'Costing Method', required=True, help='Standard Price: The cost price is manually updated at the end of a specific period (usually every year). \nAverage Price: The cost price is recomputed at each incoming shipment.'),
         ...
         'uos_id': fields.many2one('product.uom', 'Unit of Sale', help='Sepcify a unit of measure here if invoicing is made in another unit of measure than inventory. Keep empty to use the default unit of measure.'),
    ...
    def _check_uos(self, cursor, user, ids, context = None):
        for product in self.browse(cursor, user, ids, context=context):
            if product.uos_id and product.uos_id.category_id.id == product.uom_id.category_id.id:
                return False

        return True

    _constraints = [(_check_uom, 'Error: The default Unit of Measure and the purchase Unit of Measure must be in the same category.', ['uom_id'])]
...
class product_product(osv.osv):
    ...
    def _product_price(self, cr, uid, ids, name, arg, context = None):
        ...
        if pricelist:
            if isinstance(pricelist, basestring):
                pricelist_ids = self.pool.get('product.pricelist').name_search(cr, uid, pricelist, operator='=', context=context, limit=1)
    ...
    def _product_partner_ref(self, cr, uid, ids, name, arg, context = None):
        ...
        for p in self.browse(cr, uid, ids, context=context):
            ...
            if not data['name']:
                data['name'] = p.name
            res[p.id] = (data['code'] and '[' + data['code'] + '] ' or '') + (data['name'] or '') + (data['variants'] and ' - ' + data['variants'] or '')
    ...
    def _get_main_product_supplier(self, cr, uid, product, context = None):
        ...
        """
        sellers = [ (seller_info.sequence, seller_info) for seller_info in product.seller_ids or [] if seller_info and isinstance(seller_info.sequence, (int, long)) ]
    ...
    _columns = {
         ...
         'image_medium': fields.function(_get_image, fnct_inv=_set_image, string='Medium-sized image', type='binary', multi='_get_image', store={'product.product': (lambda self, cr, uid, ids, c = {}: ids, ['image'], 10)}, help='Medium-sized image of the product. It is automatically resized as a 128x128px image, with aspect ratio preserved, only when the image exceeds one of those sizes. Use this field in form views or some kanban views.'),
    ...
    def price_get(self, cr, uid, ids, ptype = 'list_price', context = None):
        ...
        for product in self.browse(cr, uid, ids, context=context):
            ...
            if 'currency_id' in context:
                res[product.id] = self.pool.get('res.currency').compute(cr, uid, price_type_currency_id, context['currency_id'], res[product.id], context=context)
...
class product_packaging(osv.osv):
    ...
    _columns = {
         ...
         'qty': fields.float('Quantity by Package', help='The total number of products you can put by pallet or box.'),
         ...
         'rows': fields.integer('Number of Layers', required=True, help='The number of layers on a pallet or box'),
         ...
         'weight': fields.float('Total Package Weight', help='The weight of a full package, pallet or box.'),
    ...
    def price_get(self, cr, uid, supplier_ids, product_id, product_qty = 1, context = None):
        ...
        for supplier in partner_pool.browse(cr, uid, supplier_ids, context=context):
            ...
            if supplier_info_ids:
                cr.execute('SELECT * FROM pricelist_partnerinfo WHERE suppinfo_id IN %sAND min_quantity <= %s ORDER BY min_quantity DESC LIMIT 1', (tuple(supplier_info_ids), product_qty))
...
class res_currency(osv.osv):
    ...
    _constraints = [(_check_main_currency_rounding, "Error! You cannot define a rounding factor for the company's main currency that is smaller than the decimal precision of 'Account'.", ['rounding'])]
...
class decimal_precision(osv.osv):
    ...
    _constraints = [(_check_main_currency_rounding, "Error! You cannot define the decimal precision of 'Account' as greater than the rounding factor of the company's main currency", ['digits'])]
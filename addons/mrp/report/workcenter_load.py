...
class report_custom(report_int):
    ...
    def _compute_dates(self, time_unit, start, stop):
        ...
        if time_unit == 'month':
            ...
            i = a
            while i <= z:
                dates[map(int, i.strftime('%Y%m%d').split())[0]] = {'name': i.strftime('%Y-%m-%d'),
    ...
    def create(self, cr, uid, ids, datas, context = None):
        if not len(ids):
            raise AssertionError('You should provide some ids!')
            ...
            tb.draw()
            ar = area.T(legend=legend.T(), x_grid_style=line_style.gray70_dash1, x_axis=axis.X(label='Periods', format='/a90/hC%s'), x_coord=category_coord.T(x_index, 0), y_axis=axis.Y(label=y_label), y_range=(0, None), size=(640, 480))
            bar_plot.fill_styles.reset()
            cr.execute('SELECT mw.id, rs.name FROM mrp_workcenter mw, resource_resource rs WHERE mw.id IN %s and mw.resource_id=rs.id ORDER BY mw.id', (tuple(ids),))
            workcenters = cr.dictfetchall()
            data = []
            for date in dates_list:
                vals = []
                for workcenter in workcenters:
                    cr.execute('SELECT SUM(mrp_production_workcenter_line.hour) AS hours, SUM(mrp_production_workcenter_line.cycle) AS cycles,                                 resource_resource.name AS name, mrp_workcenter.id AS id                             FROM mrp_production_workcenter_line, mrp_production, mrp_workcenter, resource_resource                             WHERE (mrp_production_workcenter_line.production_id=mrp_production.id)                                 AND (mrp_production_workcenter_line.workcenter_id=mrp_workcenter.id)                                 AND (mrp_workcenter.resource_id=resource_resource.id)                                 AND (mrp_workcenter.id=%s)                                 AND (mrp_production.date_planned BETWEEN %s AND %s)                             GROUP BY mrp_production_workcenter_line.workcenter_id, resource_resource.name, mrp_workcenter.id                             ORDER BY mrp_workcenter.id', (workcenter['id'], dates[date]['start'] + ' 00:00:00', dates[date]['stop'] + ' 23:59:59'))
                    res = cr.dictfetchall()
                    if not res:
                        vals.append(0.0)
                    elif datas['form']['measure_unit'] == 'cycles':
            ...
            ar = (not data or len(data[0]) <= 1) and self._empty_graph(time.strftime('%Y-%m-%d'))
        ar.draw(can)
        can.close()
    ...
    def _empty_graph(self, date):
        data = [[date, 0]]
        ar = area.T(x_coord=category_coord.T(data, 0), y_range=(0, None), x_axis=axis.X(label='Periods'), y_axis=axis.Y(label='Load'))

    ...
    def create_xml(self, cr, uid, ids, datas, context = None):
        ...
        def process_bom(bom, currency_id, factor = 1):
            ...
            if prod.seller_id:
                ...
                price = pricelist_pool.price_get(cr, uid, [pricelist.id], prod.id, number * prod_qtty or 1.0, prod.seller_id.id, {'uom': prod.uom_po_id.id,
                 'date': time.strftime('%Y-%m-%d')})[pricelist.id]
            ...
            for seller_id in prod.seller_ids:
                ...
                price = pricelist_pool.price_get(cr, uid, [pricelist.id], prod.id, number * prod_qtty or 1.0, seller_id.name.id, {'uom': prod.uom_po_id.id,
                 'date': time.strftime('%Y-%m-%d')})[pricelist.id]
        ...
        for product in product_pool.browse(cr, uid, ids, context=context):
            ...
            if not bom_id:
                ...
                total = 0
                total_strd = 0
                parent_bom = {'product_qty': bom.product_qty,

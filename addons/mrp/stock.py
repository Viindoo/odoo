...
class StockMove(osv.osv):
    _inherit = 'stock.move'
    _columns = {'production_id': fields.many2one('mrp.production', 'Production', select=True)}
    ...
    def _action_explode(self, cr, uid, move, context = None):
        ...
        if move.product_id.supply_method == 'produce':
            bis = bom_obj.search(cr, uid, [('product_id', '=', move.product_id.id), ('bom_id', '=', False), ('type', '=', 'phantom')])
            if bis:
                ...
                if move.state == 'assigned':
                    state = 'assigned'
                for line in res[0]:
                    valdef = {'picking_id': move.picking_id.id,
                    ...
                    proc_id = procurement_obj.create(cr, uid, {'name': move.picking_id.origin or '',
                ...
                move_obj.write(cr, uid, [move.id], {'location_dest_id': move.location_id.id,
                 'auto_validate': True,
                 'picking_id': False,
                 'state': 'confirmed'})

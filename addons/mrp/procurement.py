...
class procurement_order(osv.osv):
    ...
    _columns = {'bom_id': fields.many2one('mrp.bom', 'BoM', ondelete='cascade', select=True),
    ...
    def get_phantom_bom_id(self, cr, uid, ids, context = None):
        for procurement in self.browse(cr, uid, ids, context=context):
            if procurement.move_id and procurement.move_id.product_id.supply_method == 'produce' and procurement.move_id.product_id.procure_method == 'make_to_order':
                phantom_bom_id = self.pool.get('mrp.bom').search(cr, uid, [('product_id', '=', procurement.move_id.product_id.id), ('bom_id', '=', False), ('type', '=', 'phantom')])
    ...
    def make_mo(self, cr, uid, ids, context = None):
        ...
        for procurement in procurement_obj.browse(cr, uid, ids, context=context):
            ...
            produce_id = production_obj.create(cr, uid, {'origin': procurement.origin,

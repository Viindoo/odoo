...
class mrp_workcenter(osv.osv):
    ...
    _columns = {'note': fields.text('Description', help="Description of the Work Center. Explain here what's a cycle according to this Work Center."),
     ...
     'costs_hour_account_id': fields.many2one('account.analytic.account', 'Hour Account', domain=[('type', '!=', 'view')], help='Fill this only if you want automatic analytic accounting entries on production orders.'),
     'costs_cycle': fields.float('Cost per cycle', help='Specify Cost of Work Center per cycle.'),
     'costs_cycle_account_id': fields.many2one('account.analytic.account', 'Cycle Account', domain=[('type', '!=', 'view')], help='Fill this only if you want automatic analytic accounting entries on production orders.'),
    ...
    _defaults = {'capacity_per_cycle': 1.0,
...
class mrp_routing(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True),
     ...
     'location_id': fields.many2one('stock.location', 'Production Location', help='Keep empty if you produce at the location where the finished products are needed.Set a location if you produce at a fixed location. This can be a partner location if you subcontract the manufacturing operations.'),
...
class mrp_routing_workcenter(osv.osv):
    ...
    _order = 'sequence'
    _columns = {'workcenter_id': fields.many2one('mrp.workcenter', 'Work Center', required=True),
     ...
     'routing_id': fields.many2one('mrp.routing', 'Parent Routing', select=True, ondelete='cascade', help='Routing indicates all the Work Centers used, for how long and/or cycles.If Routing is indicated then,the third tab of a production order (Work Centers) will be automatically pre-completed.'),
     'note': fields.text('Description'),
    ...
    _defaults = {'cycle_nbr': lambda *a: 1.0,
...
class mrp_bom(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64),
     ...
     'type': fields.selection([('normal', 'Normal BoM'), ('phantom', 'Sets / Phantom')], 'BoM Type', required=True, help="If a by-product is used in several products, it can be useful to create its own BoM. Though if you don't want separated production orders for this by-product, select Set/Phantom as BoM type. If a Phantom BoM is used for a root product, it will be sold and shipped as a set of components, instead of being produced."),
     'method': fields.function(_compute_type, string='Method', type='selection', selection=[('', ''),
                ('stock', 'On Stock'),
                ('order', 'On Order'),
                ('set', 'Set / Pack')]),
    ...
    _constraints = [(_check_recursion, 'Error ! You cannot create recursive BoM.', ['parent_id']), (_check_product, 'BoM line product should not be same as BoM product.', ['product_id'])]
    ...
    def _bom_explode(self, cr, uid, bom, factor, properties = None, addthis = False, level = 0, routing_id = False):
        ...
        if not phantom:
            if addthis and not bom.bom_lines:
                result.append({'name': bom.product_id.name,
            ...
            if routing:
                for wc_use in routing.workcenter_lines:
                    ...
                    result2.append({'name': tools.ustr(wc_use.name) + ' - ' + tools.ustr(bom.product_id.name),
                     'workcenter_id': wc.id,
...
class mrp_production(osv.osv):
    ...
    _columns = {'name': fields.char('Reference', size=64, required=True, readonly=True, states={'draft': [('readonly', False)]}),
     'origin': fields.char('Source Document', size=64, readonly=True, states={'draft': [('readonly', False)]}, help='Reference of the document that generated this production order request.'),
     'priority': fields.selection([('0', 'Not urgent'),
                  ('1', 'Normal'),
                  ('2', 'Urgent'),
                  ('3', 'Very Urgent')], 'Priority', select=True, readonly=True, states=dict.fromkeys(['draft', 'confirmed'], [('readonly', False)])),
     ...
     'location_src_id': fields.many2one('stock.location', 'Raw Materials Location', required=True, readonly=True, states={'draft': [('readonly', False)]}, help='Location where the system will look for components.'),
     'location_dest_id': fields.many2one('stock.location', 'Finished Products Location', required=True, readonly=True, states={'draft': [('readonly', False)]}, help='Location where the system will stock the finished products.'),
     ...
     'bom_id': fields.many2one('mrp.bom', 'Bill of Material', domain=[('bom_id', '=', False)], readonly=True, states={'draft': [('readonly', False)]}, help='Bill of Materials allow you to define the list of required raw materials to make a finished product.'),
     ...
     'picking_id': fields.many2one('stock.picking', 'Picking List', readonly=True, ondelete='restrict', help='This is the Internal Picking List that brings the finished product to the production plan'),
     ...
     'move_lines2': fields.many2many('stock.move', 'mrp_production_move_ids', 'production_id', 'move_id', 'Consumed Products', domain=[('state', 'in', ('done', 'cancel'))], readonly=True, states={'draft': [('readonly', False)]}),
     'move_created_ids': fields.one2many('stock.move', 'production_id', 'Products to Produce', domain=[('state', 'not in', ('done', 'cancel'))], readonly=True, states={'draft': [('readonly', False)]}),
     'move_created_ids2': fields.one2many('stock.move', 'production_id', 'Produced Products', domain=[('state', 'in', ('done', 'cancel'))], readonly=True, states={'draft': [('readonly', False)]}),
     ...
     'workcenter_lines': fields.one2many('mrp.production.workcenter.line', 'production_id', 'Work Centers Utilisation', readonly=True, states={'draft': [('readonly', False)]}),
     'state': fields.selection([('draft', 'New'),
               ...
               ('done', 'Done')], string='Status', readonly=True, track_visibility='onchange', help="When the production order is created the status is set to 'Draft'.\n                If the order is confirmed the status is set to 'Waiting Goods'.\n                If any exceptions are there, the status is set to 'Picking Exception'.\n                If the stock is available then the status is set to 'Ready to Produce'.\n                When the production gets started then the status is set to 'In Production'.\n                When the production is over, the status is set to 'Done'."),
    ...
    _constraints = [(_check_qty, 'Order quantity cannot be negative or zero!', ['product_qty'])]
    ...
    def copy(self, cr, uid, id, default = None, context = None):
        if default is None:
            default = {}
        default.update({'name': self.pool.get('ir.sequence').get(cr, uid, 'mrp.production'),
         'move_lines': [],
         'move_lines2': [],
         'move_created_ids': [],
         'move_created_ids2': [],
    ...
    def product_id_change(self, cr, uid, ids, product_id, context = None):
        ...
        result = {'product_uom': product_uom_id,
         'bom_id': bom_id,
    ...
    def action_cancel(self, cr, uid, ids, context = None):
        ...
        for production in self.browse(cr, uid, ids, context=context):
            if production.state == 'confirmed' and production.picking_id.state not in ('draft', 'cancel'):
                raise osv.except_osv(_('Cannot cancel manufacturing order!'), _('You must first cancel related internal picking attached to this manufacturing order.'))
    ...
    def action_production_end(self, cr, uid, ids, context = None):
        ...
        write_res = self.write(cr, uid, ids, {'state': 'done',
        ...
        for wc_line in production.workcenter_lines:
            ...
            if wc.costs_journal_id and wc.costs_general_account_id:
                ...
                if value and account:
                    amount += value
                    analytic_line_obj.create(cr, uid, {'name': wc_line.name + ' (H)',
                     'amount': value,
                     'account_id': account,
                ...
                if value and account:
                    amount += value
                    analytic_line_obj.create(cr, uid, {'name': wc_line.name + ' (C)',
                     'amount': value,
                     'account_id': account,
                     'general_account_id': wc.costs_general_account_id.id,
                     'journal_id': wc.costs_journal_id.id,
    ...
    def action_in_production(self, cr, uid, ids, context = None):
        ...
        @return: True
        """
        return self.write(cr, uid, ids, {'state': 'in_production',
    ...
    def _make_production_line_procurement(self, cr, uid, production_line, shipment_move_id, context = None):
        ...
        procurement_id = procurement_order.create(cr, uid, {'name': procurement_name,
    ...
    def _make_production_internal_shipment_line(self, cr, uid, production_line, shipment_id, parent_move_id, destination_location_id = False, context = None):
        ...
        return stock_move.create(cr, uid, {'name': production.name,
    ...
    def _make_production_internal_shipment(self, cr, uid, production, context = None):
        ...
        picking_id = stock_picking.create(cr, uid, {'name': pick_name,
    ...
    def _make_production_consume_line(self, cr, uid, production_line, parent_move_id, source_location_id = False, context = None):
        ...
        if not source_location_id:
            source_location_id = production.location_src_id.id
        move_id = stock_move.create(cr, uid, {'name': production.name,
         'date': production.date_planned,
         'product_id': production_line.product_id.id,
         'product_qty': production_line.product_qty,
         'product_uom': production_line.product_uom.id,
...
class mrp_production_workcenter_line(osv.osv):
    ...
    _order = 'sequence'
    _inherit = ['mail.thread']
    _columns = {'name': fields.char('Work Order', size=64, required=True),
     ...
     'production_id': fields.many2one('mrp.production', 'Manufacturing Order', track_visibility='onchange', select=True, ondelete='cascade', required=True)}
...
class product_product(osv.osv):
    ...
    _columns = {'bom_ids': fields.one2many('mrp.bom', 'product_id', 'Bill of Materials')}
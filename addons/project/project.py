...
class project_task_type(osv.osv):
    ...
    _order = 'sequence'
    _columns = {'name': fields.char('Stage Name', required=True, size=64, translate=True),
     'description': fields.text('Description'),
     'sequence': fields.integer('Sequence'),
     'case_default': fields.boolean('Default for New Projects', help='If you check this field, this stage will be proposed by default on each new project. It will not assign this stage to existing projects.'),
     'project_ids': fields.many2many('project.project', 'project_task_type_rel', 'type_id', 'project_id', 'Projects'),
     'state': fields.selection(_TASK_STATE, 'Related Status', required=True, help="The status of your document is automatically changed regarding the selected stage. For example, if a stage is related to the status 'Close', when your document reaches this stage, it is automatically closed."),
     'fold': fields.boolean('Folded by Default', help='This stage is not visible, for example in status bar or kanban view, when there are no records in that stage to display.')}
...
class project(osv.osv):
    ...
    def _auto_init(self, cr, context = None):
        ...
        self.pool.get('mail.alias').migrate_to_alias(cr, self._name, self._table, super(project, self)._auto_init, self._columns['alias_id'], 'id', alias_prefix='project+', alias_defaults={'project_id': 'id'}, context=alias_context)
    ...
    def _get_visibility_selection(self, cr, uid, context = None):
        ...
        return [('public', 'All Users'), ('employees', 'Employees Only'), ('followers', 'Followers Only')]
    ...
    _columns = {'complete_name': fields.function(_complete_name, string='Project Name', type='char', size=250),
     ...
     'members': fields.many2many('res.users', 'project_user_rel', 'project_id', 'uid', 'Project Members', help="Project's members are users who can have an access to the tasks related to this project.", states={'close': [('readonly', True)],
                 'cancelled': [('readonly', True)]}),
     'tasks': fields.one2many('project.task', 'project_id', 'Task Activities'),
     'planned_hours': fields.function(_progress_rate, multi='progress', string='Planned Time', help='Sum of planned hours of all tasks related to this project and its child projects.', store={'project.project': (_get_project_and_parents, ['tasks', 'parent_id', 'child_ids'], 10),
                       'project.task': (_get_projects_from_tasks, ['planned_hours',
                                         'remaining_hours',
                                         'work_ids',
                                         'state'], 20)}),
     'effective_hours': fields.function(_progress_rate, multi='progress', string='Time Spent', help='Sum of spent hours of all tasks related to this project and its child projects.', store={'project.project': (_get_project_and_parents, ['tasks', 'parent_id', 'child_ids'], 10),
                         'project.task': (_get_projects_from_tasks, ['planned_hours',
                                           'remaining_hours',
                                           'work_ids',
                                           'state'], 20)}),
     'total_hours': fields.function(_progress_rate, multi='progress', string='Total Time', help='Sum of total hours of all tasks related to this project and its child projects.', store={'project.project': (_get_project_and_parents, ['tasks', 'parent_id', 'child_ids'], 10),
                     'project.task': (_get_projects_from_tasks, ['planned_hours',
                                       'remaining_hours',
                                       'work_ids',
                                       'state'], 20)}),
     'progress_rate': fields.function(_progress_rate, multi='progress', string='Progress', type='float', group_operator='avg', help='Percent of tasks closed according to the total of tasks todo.', store={'project.project': (_get_project_and_parents, ['tasks', 'parent_id', 'child_ids'], 10),
                       'project.task': (_get_projects_from_tasks, ['planned_hours',
                                         'remaining_hours',
                                         'work_ids',
                                         'state'], 20)}),
     'resource_calendar_id': fields.many2one('resource.calendar', 'Working Time', help='Timetable working hours to adjust the gantt diagram report', states={'close': [('readonly', True)]}),
     'type_ids': fields.many2many('project.task.type', 'project_task_type_rel', 'project_id', 'type_id', 'Tasks Stages', states={'close': [('readonly', True)],
                  'cancelled': [('readonly', True)]}),
     'task_count': fields.function(_task_count, type='integer', string='Open Tasks'),
     'color': fields.integer('Color Index'),
     'alias_id': fields.many2one('mail.alias', 'Alias', ondelete='restrict', required=True, help='Internal email associated with this project. Incoming emails are automatically synchronizedwith Tasks (or optionally Issues if the Issue Tracker module is installed).'),
     'alias_model': fields.selection(_alias_models, 'Alias Model', select=True, required=True, help="The kind of document created when an email is received on this project's email alias"),
    ...
    _constraints = [(_check_dates, 'Error! project start-date must be lower then project end-date.', ['date_start', 'date'])]
    ...
    def duplicate_template(self, cr, uid, ids, context = None):
        ...
        for proj in self.browse(cr, uid, ids, context=context):
            ...
            new_id = self.copy(cr, uid, proj.id, default={'name': _('%s (copy)') % proj.name,
             'state': 'open',
    ...
    def _schedule_project(self, cr, uid, project, context = None):
        ...
        result = "\n  def Project_%d():\n    start = '%s'\n    working_days = %s\n    resource = %s\n" % (project.id,
         project.date_start or time.strftime('%Y-%m-%d'),
         working_days,
    ...
    def schedule_tasks(self, cr, uid, ids, context = None):
        ...
        for project in projects:
            ...
            for task in project.tasks:
                ...
                self.pool.get('project.task').write(cr, uid, [task.id], {'date_start': p.start.strftime('%Y-%m-%d %H:%M:%S'),
                 'date_end': p.end.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
                if not task.user_id and p.booked_resource:
                    self.pool.get('project.task').write(cr, uid, [task.id], {'user_id': int(p.booked_resource[0].name[5:])}, context=context)
    ...
    def create(self, cr, uid, vals, context = None):
        ...
        if not vals.get('alias_id') and vals.get('name', False):
            ...
            alias_id = mail_alias.create_unique_alias(cr, uid, {'alias_name': 'project+' + short_name(vals['name'])}, model_name=vals.get('alias_model', 'project.task'), context=context)
...
class task(base_stage, osv.osv):
    ...
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _track = {'state': {'project.mt_task_new': lambda self, cr, uid, obj, ctx = None: obj['state'] in ('new', 'draft'),
     ...
     'stage_id': {'project.mt_task_stage': lambda self, cr, uid, obj, ctx = None: obj['state'] not in ('new', 'draft', 'done', 'open')},
     'kanban_state': {'project.mt_task_blocked': lambda self, cr, uid, obj, ctx = None: obj['kanban_state'] == 'blocked'}}
    ...
    def duplicate_task(self, cr, uid, map_ids, context = None):
        for new in map_ids.values():
            ...
            self.write(cr, uid, new, {'parent_ids': [(6, 0, set(parent_ids))],
             'child_ids': [(6, 0, set(child_ids))]})

    def copy_data(self, cr, uid, id, default = None, context = None):
        if default is None:
            default = {}
        default = default or {}
        default.update({'work_ids': [],
         'date_start': False,
         'date_end': False,
         'date_deadline': False})
    ...
    _columns = {'active': fields.function(_is_template, store=True, string='Not a Template Task', type='boolean', help="This field is computed automatically and have the same behavior than the boolean 'active' field: if the task is linked to a template or unactivated project, it will be hidden unless specifically asked."),
     ...
     'priority': fields.selection([('4', 'Very Low'),
                  ...
                  ('2', 'Medium'),
                  ('1', 'Important'),
                  ('0', 'Very important')], 'Priority', select=True),
     'sequence': fields.integer('Sequence', select=True, help='Gives the sequence order when displaying a list of tasks.'),
     'stage_id': fields.many2one('project.task.type', 'Stage', track_visibility='onchange', domain="['&', ('fold', '=', False), ('project_ids', '=', project_id)]"),
     'state': fields.related('stage_id', 'state', type='selection', store=True, selection=_TASK_STATE, string='Status', readonly=True, help="The status is set to 'Draft', when a case is created.                      If the case is in progress the status is set to 'Open'.                      When the case is over, the status is set to 'Done'.                      If the case needs to be reviewed then the status is                       set to 'Pending'."),
     'categ_ids': fields.many2many('project.category', string='Tags'),
     'kanban_state': fields.selection([('normal', 'Normal'), ('blocked', 'Blocked'), ('done', 'Ready for next stage')], 'Kanban State', track_visibility='onchange', help="A task's kanban state indicates special situations affecting it:\n * Normal is the default situation\n * Blocked indicates something is preventing the progress of this task\n * Ready for next stage indicates the task is ready to be pulled to the next stage", readonly=True, required=False),
     ...
     'effective_hours': fields.function(_hours_get, string='Hours Spent', multi='hours', help='Computed using the sum of the task work done.', store={'project.task': (lambda self, cr, uid, ids, c = {}: ids, ['work_ids', 'remaining_hours', 'planned_hours'], 10),
                         'project.task.work': (_get_task, ['hours'], 10)}),
     'remaining_hours': fields.float('Remaining Hours', digits=(16, 2), help='Total remaining time, can be re-estimated periodically by the assignee of the task.'),
     'total_hours': fields.function(_hours_get, string='Total', multi='hours', help='Computed as: Time Spent + Remaining Time.', store={'project.task': (lambda self, cr, uid, ids, c = {}: ids, ['work_ids', 'remaining_hours', 'planned_hours'], 10),
                     'project.task.work': (_get_task, ['hours'], 10)}),
     'progress': fields.function(_hours_get, string='Progress (%)', multi='hours', group_operator='avg', help="If the task has a progress of 99.99% you should close the task if it's finished or reevaluate the time", store={'project.task': (lambda self, cr, uid, ids, c = {}: ids, ['work_ids',
                                    'remaining_hours',
                                    'planned_hours',
                                    'state'], 10),
                  'project.task.work': (_get_task, ['hours'], 10)}),
     'delay_hours': fields.function(_hours_get, string='Delay Hours', multi='hours', help='Computed as difference between planned hours by the project manager and the total hours of the task.', store={'project.task': (lambda self, cr, uid, ids, c = {}: ids, ['work_ids', 'remaining_hours', 'planned_hours'], 10),
                     'project.task.work': (_get_task, ['hours'], 10)}),
    ...
    _defaults = {'stage_id': _get_default_stage_id,
    ...
    _constraints = [(_check_recursion, 'Error ! You cannot create recursive tasks.', ['parent_ids']), (_check_dates, 'Error ! Task end-date must be greater then task start-date', ['date_start', 'date_end'])]
    ...
    def do_delegate(self, cr, uid, ids, delegate_data = None, context = None):
        ...
        if delegate_data is None:
            delegate_data = {}
        raise delegate_data['user_id'] or AssertionError(_('Delegated User should be specified'))
        delegated_tasks = {}
        for task in self.browse(cr, uid, ids, context=context):
            delegated_task_id = self.copy(cr, uid, task.id, {'name': delegate_data['name'],
             ...
             'child_ids': [],
             'work_ids': []}, context=context)
            self._delegate_task_attachments(cr, uid, task.id, delegated_task_id, context=context)
            newname = delegate_data['prefix'] or ''
            task.write({'remaining_hours': delegate_data['planned_hours_me'],
             'planned_hours': delegate_data['planned_hours_me'] + (task.effective_hours or 0.0),
             'name': newname}, context=context)
    ...
    def _store_history(self, cr, uid, ids, context = None):
        for task in self.browse(cr, uid, ids, context=context):
            self.pool.get('project.task.history').create(cr, uid, {'task_id': task.id,
             'remaining_hours': task.remaining_hours,
             'planned_hours': task.planned_hours,
             'kanban_state': task.kanban_state,
             'type_id': task.stage_id.id,
             'state': task.state,
             'user_id': task.user_id.id}, context=context)
    ...
    def message_get_reply_to(self, cr, uid, ids, context = None):
        ...
        return [ (task.project_id.message_get_reply_to()[0] if task.project_id else False) for task in self.browse(cr, uid, ids, context=context) ]

    def message_new(self, cr, uid, msg, custom_values = None, context = None):
        """ Override to updates the document according to the email. """
        if custom_values is None:
            custom_values = {}
        defaults = {'name': msg.get('subject'),
         'planned_hours': 0.0}
    ...
    def message_update(self, cr, uid, ids, msg, update_vals = None, context = None):
        ...
        for line in msg['body'].split('\n'):
            ...
            if res:
                ...
                elif match.lower() == 'state' and res.group(2).lower() in ('cancel', 'close', 'draft', 'open', 'pending'):
...
class project_work(osv.osv):
    ...
    _columns = {'name': fields.char('Work summary', size=128),
...
class account_analytic_account(osv.osv):
    ...
    _columns = {'use_tasks': fields.boolean('Tasks', help='If checked, this contract will be available in the project menu and you will be able to manage tasks or track issues'),
    ...
    def _trigger_project_creation(self, cr, uid, vals, context = None):
        ...
        if context is None:
            context = {}
        return vals.get('use_tasks') and 'project_creation_in_progress' not in context
    ...
    def project_create(self, cr, uid, analytic_account_id, vals, context = None):
        ...
        if not project_id and self._trigger_project_creation(cr, uid, vals, context=context):
            project_values = {'name': vals.get('name'),
...
class project_task_history(osv.osv):
    ...
    _columns = {'task_id': fields.many2one('project.task', 'Task', ondelete='cascade', required=True, select=True),
     ...
     'end_date': fields.function(_get_date, string='End Date', type='date', store={'project.task.history': (_get_related_date, None, 20)}),
...
class project_task_history_cumulative(osv.osv):
    ...
    _auto = False
    _columns = {'end_date': fields.date('End Date'),

...
class hr_timesheet_sheet(osv.osv):
    ...
    def _total(self, cr, uid, ids, name, args, context = None):
        ...
        for sheet in self.browse(cr, uid, ids, context=context or {}):
            res.setdefault(sheet.id, {'total_attendance': 0.0,
             'total_timesheet': 0.0,
    ...
    _columns = {'name': fields.char('Note', size=64, select=1, states={'confirm': [('readonly', True)],
              'done': [('readonly', True)]}),
     ...
     'timesheet_ids': fields.one2many('hr.analytic.timesheet', 'sheet_id', 'Timesheet lines', readonly=True, states={'draft': [('readonly', False)],
                       'new': [('readonly', False)]}),
     'attendances_ids': fields.one2many('hr.attendance', 'sheet_id', 'Attendances'),
     'state': fields.selection([('new', 'New'),
               ('draft', 'Open'),
               ('confirm', 'Waiting Approval'),
               ('done', 'Approved')], 'Status', select=True, required=True, readonly=True, help=" * The 'Draft' status is used when a user is encoding a new and unconfirmed timesheet.                 \n* The 'Confirmed' status is used for to confirm the timesheet by user.                 \n* The 'Done' status is used when users timesheet is accepted by his/her senior."),
    ...
    _defaults = {'date_from': _default_date_from,
    ...
    def _sheet_date(self, cr, uid, ids, forced_user_id = False, context = None):
        for sheet in self.browse(cr, uid, ids, context=context):
            ...
            if new_user_id:
                cr.execute('SELECT id                     FROM hr_timesheet_sheet_sheet                     WHERE (date_from <= %s and %s <= date_to)                         AND user_id=%s                         AND id <> %s', (sheet.date_to,
                 sheet.date_from,
                 new_user_id,
                 sheet.id))
                if cr.fetchall():
    ...
    _constraints = [(_sheet_date, "You cannot have 2 timesheets that overlap!\nPlease use the menu 'My Current Timesheet' to avoid this problem.", ['date_from', 'date_to'])]
    ...
    def name_get(self, cr, uid, ids, context = None):
        ...
        if isinstance(ids, (long, int)):
            ids = [ids]
        return [ (r['id'], _('Week ') + datetime.strptime(r['date_from'], '%Y-%m-%d').strftime('%U')) for r in self.read(cr, uid, ids, ['date_from'], context=context, load='_classic_write') ]
    ...
    def unlink(self, cr, uid, ids, context = None):
        ...
        for sheet in sheets:
            ...
            elif sheet['total_attendance'] != 0.0:
...
class hr_timesheet_line(osv.osv):
    ...
    def _sheet(self, cursor, user, ids, name, args, context = None):
        ...
        for ts_line in self.browse(cursor, user, ids, context=context):
            sheet_ids = sheet_obj.search(cursor, user, [('date_to', '>=', ts_line.date), ('date_from', '<=', ts_line.date), ('employee_id.user_id', '=', ts_line.user_id.id)], context=context)
            if sheet_ids:
    ...
    _columns = {'sheet_id': fields.function(_sheet, string='Sheet', select='1', type='many2one', relation='hr_timesheet_sheet.sheet', ondelete='cascade', store={'hr_timesheet_sheet.sheet': (_get_hr_timesheet_sheet, ['employee_id', 'date_from', 'date_to'], 10),
    ...
    _constraints = [(_check_sheet_state, 'You cannot modify an entry in a Confirmed/Done timesheet !', ['state'])]
...
class hr_attendance(osv.osv):
    ...
    def _sheet(self, cursor, user, ids, name, args, context = None):
        ...
        for attendance in self.browse(cursor, user, ids, context=context):
            ...
            sheet_ids = sheet_obj.search(cursor, user, [('date_to', '>=', date_to), ('date_from', '<=', attendance.name), ('employee_id', '=', attendance.employee_id.id)], context=context)
            if sheet_ids:
                res[attendance.id] = sheet_obj.name_get(cursor, user, sheet_ids, context=context)[0]

        return res

    _columns = {'sheet_id': fields.function(_sheet, string='Sheet', type='many2one', relation='hr_timesheet_sheet.sheet', store={'hr_timesheet_sheet.sheet': (_get_hr_timesheet_sheet, ['employee_id', 'date_from', 'date_to'], 10),
                  'hr.attendance': (lambda self, cr, uid, ids, context = None: ids, ['employee_id', 'name', 'day'], 10)})}
    _defaults = {'name': _get_default_date}
    ...
    def create(self, cr, uid, vals, context = None):
        ...
        if 'sheet_id' in context:
            if context['sheet_id'] != self.browse(cr, uid, res, context=context).sheet_id.id:
                raise osv.except_osv(_('User Error!'), _('You cannot enter an attendance date outside the current timesheet dates.'))
        return res
...
class hr_timesheet_sheet_sheet_day(osv.osv):
    ...
    _auto = False
    _order = 'name'
    _columns = {'name': fields.date('Date', readonly=True),
...
class hr_timesheet_sheet_sheet_account(osv.osv):
    ...
    _auto = False
    _order = 'name'
    _columns = {'name': fields.many2one('account.analytic.account', 'Project / Analytic Account', readonly=True),
...
class res_company(osv.osv):
    ...
    _columns = {'timesheet_range': fields.selection([('day', 'Day'), ('week', 'Week'), ('month', 'Month')], 'Timesheet range', help='Periodicity on which you validate your timesheets.'),
     'timesheet_max_difference': fields.float('Timesheet allowed difference(Hours)', help='Allowed difference in hours between the sign in/out and the timesheet computation for one sheet. Set this to 0 if you do not want any control.')}
    _defaults = {'timesheet_range': lambda *args: 'week',

...
class stock_location_path(osv.osv):
    ...
    _columns = {'name': fields.char('Operation', size=64),
     ...
     'invoice_state': fields.selection([('invoiced', 'Invoiced'), ('2binvoiced', 'To Be Invoiced'), ('none', 'Not Applicable')], 'Invoice Status', required=True),
     ...
     'auto': fields.selection([('auto', 'Automatic Move'), ('manual', 'Manual Operation'), ('transparent', 'Automatic No Step Added')], 'Automatic Move', required=True, select=1, help="This is used to define paths the product has to follow within the location tree.\nThe 'Automatic Move' value will create a stock move after the current one that will be validated automatically. With 'Manual Operation', the stock move has to be validated by a worker. With 'Automatic No Step Added', the location is replaced in the original move.")}
    _defaults = {'auto': 'auto',
     'delay': 1,
     'invoice_state': 'none',
...
class product_pulled_flow(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, help='This field will fill the packing Origin and the name of its moves'),
     ...
     'invoice_state': fields.selection([('invoiced', 'Invoiced'), ('2binvoiced', 'To Be Invoiced'), ('none', 'Not Applicable')], 'Invoice Status', required=True)}
    _defaults = {'cancel_cascade': False,
...
class product_product(osv.osv):
    ...
    _columns = {'flow_pull_ids': fields.one2many('product.pulled.flow', 'product_id', 'Pulled Flows'),
     'path_ids': fields.one2many('stock.location.path', 'product_id', 'Pushed Flow', help='These rules set the right path of the product in the whole location tree.')}
...
class stock_move(osv.osv):
    _inherit = 'stock.move'
    _columns = {'cancel_cascade': fields.boolean('Cancel Cascade', help='If checked, when this move is cancelled, cancel the linked move too')}
        ...
        if product:
            for path in product.path_ids:
                if path.location_from_id.id == location.id:
                    return (path.location_dest_id,
                     path.auto,
                     path.delay,
                     path.journal_id and path.journal_id.id or False,
                     path.company_id and path.company_id.id or False,
                     path.picking_type,
                     path.invoice_state)

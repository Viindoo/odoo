...
class procurement_order(osv.osv):
    ...
    def action_move_create(self, cr, uid, ids, context = None):
        ...
        for proc in proc_obj.browse(cr, uid, ids, context=context):
            ...
            if not line:
                raise AssertionError('Line cannot be False if we are on this state of the workflow')
                origin = (proc.origin or proc.name or '').split(':')[0] + ':' + line.name
                picking_id = picking_obj.create(cr, uid, {'origin': origin,
                 ...
                 'note': _('Picking for pulled procurement coming from original location %s, pull rule %s, via original Procurement %s (#%d)') % (proc.location_id.name,
                          line.name,
                          proc.name,
                          proc.id),
                 'invoice_state': line.invoice_state})
                move_id = move_obj.create(cr, uid, {'name': line.name,
                 ...
                 'product_uos_qty': proc.product_uos and proc.product_uos_qty or proc.product_qty,
                 'product_uos': proc.product_uos and proc.product_uos.id or proc.product_uom.id,
                 'partner_id': line.partner_address_id.id,
                 'location_id': line.location_src_id.id,
                 'location_dest_id': line.location_id.id,
                 'move_dest_id': proc.move_id and proc.move_id.id or False,
                 'tracking_id': False,
                 'cancel_cascade': line.cancel_cascade,
                 'state': 'confirmed',
                 'note': _('Move for pulled procurement coming from original location %s, pull rule %s, via original Procurement %s (#%d)') % (proc.location_id.name,
                          line.name,
                          proc.name,
                          proc.id)})
                if proc.move_id and proc.move_id.state in 'confirmed':
                    move_obj.write(cr, uid, [proc.move_id.id], {'state': 'waiting'}, context=context)
                proc_id = proc_obj.create(cr, uid, {'name': line.name,
                 'origin': origin,
                 'note': _('Pulled procurement coming from original location %s, pull rule %s, via original Procurement %s (#%d)') % (proc.location_id.name,
                          line.name,
                          proc.name,
                          proc.id),
                 'company_id': line.company_id and line.company_id.id or False,
                ...
                proc.move_id and move_obj.write(cr, uid, [proc.move_id.id], {'location_id': proc.location_id.id})
            msg = _('Pulled from another location.')
            self.write(cr, uid, [proc.id], {'state': 'running',
             'message': msg})

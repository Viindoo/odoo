...
class project_issue(osv.osv):
    ...
    _columns = {'timesheet_ids': fields.one2many('hr.analytic.timesheet', 'issue_id', 'Timesheets'),
    ...
    def on_change_account_id(self, cr, uid, ids, account_id, context = None):
        ...
        if account and account.state == 'pending':
            result = {'warning': {'title': _('Analytic Account'),
                         'message': _('The Analytic Account is pending !')}}
        return result
...
class hr_analytic_issue(osv.osv):
    ...
    _columns = {'issue_id': fields.many2one('project.issue', 'Issue')}

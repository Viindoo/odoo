...
class share_wizard(osv.TransientModel):
    ...
    def create(self, cr, uid, values, context = None):
        if 'action_id' in values and 'name' not in values:
    ...
    _columns = {'action_id': fields.many2one('ir.actions.act_window', 'Action to share', required=True, help='The action that opens the screen containing the data you wish to share.'),
     ...
     'user_type': fields.selection(lambda s, *a, **k: s._user_type_selection(*a, **k), 'Sharing method', required=True, help='Select the type of user(s) you would like to share data with.'),
     ...
     'access_mode': fields.selection([('readonly', 'Can view'), ('readwrite', 'Can edit')], 'Access Mode', required=True, help='Access rights to be granted on the shared documents.'),
     'result_line_ids': fields.one2many('share.wizard.result.line', 'share_wizard_id', 'Summary', readonly=True),
     'share_root_url': fields.function(_share_root_url, string='Share Access URL', type='char', size=512, readonly=True, help='Main access page for users that are granted shared access'),
     ...
     'embed_code': fields.function(_embed_code, type='text', string='Code', help='Embed this code in your documents to provide a link to the shared document.'),
    ...
    def go_step_1(self, cr, uid, ids, context = None):
        ...
        if wizard_data.user_type == 'emails' and not self.has_email(cr, uid, context=context):
            raise osv.except_osv(_('No email address configured'), _('You must configure your email address in the user preferences before using the Share button.'))
    ...
    def _create_share_group(self, cr, uid, wizard_data, context = None):
        ...
        share_group_name = '%s: %s (%d-%s)' % ('Shared',
         wizard_data.name,
         uid,
         time.time())
        return group_obj.create(cr, UID_ROOT, {'name': share_group_name,
         'share': True}, {'noadmin': True})
    ...
    def _create_new_share_users(self, cr, uid, wizard_data, group_id, context = None):
        ...
        if wizard_data.user_type == 'emails':
            ...
            for new_user in new_users:
                ...
                user_id = user_obj.create(cr, UID_ROOT, {'login': new_user,
                 'password': new_pass,
                 'name': new_user,
                 'email': new_user,
                 ...
                 'company_ids': [(6, 0, [current_user.company_id.id])]}, context)
            ...
            new_pass = generate_random_pass()
            user_id = user_obj.create(cr, UID_ROOT, {'login': new_login,
             'password': new_pass,
             'name': new_login,
             'groups_id': [(6, 0, [group_id])],
             'share': True,
    ...
    def _shared_action_def(self, cr, uid, wizard_data, context = None):
        ...
        action_def = {'name': wizard_data.name,
        ...
        if copied_action.view_ids:
            action_def['view_ids'] = [ (0, 0, {'sequence': x.sequence,
              ...
              'view_id': x.view_id.id}) for x in copied_action.view_ids if wizard_data.access_mode != 'readonly' or x.view_mode == wizard_data.view_type ]
        return action_def
            ...
            if coltype in ttypes and colinfo.column._obj not in models:
                ...
                for parent in relation_osv._inherits:
                    if parent not in models:
                        ...
                        parent_model_browse = model_obj.browse(cr, UID_ROOT, model_obj.search(cr, UID_ROOT, [('model', '=', parent)]))[0]
                ...
                if relation_model_id != model.id and coltype in ('one2many', 'many2many'):
                    local_rel_fields += self._get_recursive_relations(cr, uid, relation_model_browse, [coltype], relation_fields + local_rel_fields, suffix=relation_field, context=context)
    ...
    def _get_relationship_classes(self, cr, uid, model, context = None):
        ...
        for parent in model_obj._inherits:
            parent_model_browse = ir_model_obj.browse(cr, UID_ROOT, ir_model_obj.search(cr, UID_ROOT, [('model', '=', parent)]))[0]
    ...
    def _get_access_map_for_groups_and_models(self, cr, uid, group_ids, model_ids, context = None):
        ...
        user_right_ids = model_access_obj.search(cr, uid, [('group_id', 'in', group_ids), ('model_id', 'in', model_ids)], context=context)
        user_access_matrix = {}
        if user_right_ids:
    ...
    def _add_access_rights_for_share_group(self, cr, uid, group_id, mode, fields_relations, context = None):
        ...
        current_user_access_map = self._get_access_map_for_groups_and_models(cr, uid, [ x.id for x in current_user.groups_id ], target_model_ids, context=context)
        group_access_map = self._get_access_map_for_groups_and_models(cr, uid, [group_id], target_model_ids, context=context)
        ...
        for dummy, model in fields_relations:
            if model.model in ('mail.message',):
                continue
            values = {'name': _('Copied access for sharing'),
             'group_id': group_id,
             'model_id': model.id}
            ...
            for perm in perms_to_add:
                if perm in current_user_access_line and perm not in existing_group_access_line:
                ...
                for rule in group.rule_groups:
                    ...
                    if rule.model_id.id == model.id:
                        if 'user.' in rule.domain_force:
                            rule_obj.copy(cr, UID_ROOT, rule.id, default={'name': '%s %s' % (rule.name, _('(Copy for sharing)')),
                             'groups': [(6, 0, [group_id])],
                            ...
                            rule_obj.write(cr, SUPERUSER_ID, [rule.id], {'groups': [(4, group_id)]})
    ...
    def _check_personal_rule_or_duplicate(self, cr, group_id, rule, context = None):
        ...
        if len(rule.groups) == 1:
            return rule
        rule_obj = self.pool.get('ir.rule')
        new_id = rule_obj.copy(cr, UID_ROOT, rule.id, default={'name': '%s %s' % (rule.name, _('(Duplicated for modified sharing permissions)')),
         'groups': [(6, 0, [group_id])],
    ...
    def _create_or_combine_sharing_rule(self, cr, current_user, wizard_data, group_id, model_id, domain, restrict = False, rule_name = None, context = None):
        ...
        if rule_name is None:
            rule_name = _('Sharing filter created by user %s (%s) for group %s') % (current_user.name, current_user.login, group_id)
        rule_obj = self.pool.get('ir.rule')
    ...
    def _create_indirect_sharing_rules(self, cr, current_user, wizard_data, group_id, fields_relations, context = None):
        rule_name = _('Indirect sharing filter created by user %s (%s) for group %s') % (current_user.name, current_user.login, group_id)
        try:
            ...
            if domain:
                for rel_field, model in fields_relations:
                    ...
                    self._create_or_combine_sharing_rule(cr, current_user, wizard_data, group_id, model_id=model.id, domain=str(related_domain), rule_name=rule_name, restrict=True, context=context)

        except Exception:
            _logger.exception('Failed to create share access')
            raise osv.except_osv(_('Sharing access cannot be created.'), _('Sorry, the current screen and filter you are trying to share are not supported at the moment.\nYou may want to try a simpler filter.'))

    def _check_preconditions(self, cr, uid, wizard_data, context = None):
        self._assert(wizard_data.action_id and wizard_data.access_mode, _('Action and Access Mode are required to create a shared access.'), context=context)
        self._assert(self.has_share(cr, uid, wizard_data, context=context), _('You must be a member of the Share/User group to use the share wizard.'), context=context)
        if wizard_data.user_type == 'emails':
            self._assert(wizard_data.new_users or wizard_data.email_1 or wizard_data.email_2 or wizard_data.email_3, _('Please indicate the emails of the persons to share with, one per line.'), context=context)
    ...
    def _create_share_users_group(self, cr, uid, wizard_data, context = None):
        ...
        if existing_ids:
            self.pool.get('res.users').write(cr, UID_ROOT, existing_ids, {'groups_id': [(4, group_id)]})
    ...
    def go_step_2(self, cr, uid, ids, context = None):
        ...
        self._create_or_combine_sharing_rule(cr, current_user, wizard_data, group_id, model_id=model.id, domain=main_domain, restrict=True, context=context)
        ...
        if not wizard_data.invite:
            ...
            return {'name': _('Shared access created!'),
             'view_type': 'form',
             'view_mode': 'form',
             'res_model': 'share.wizard',
             'view_id': False,
             'res_id': ids[0],
             'views': [(step2_form_view_id, 'form'),
                       (False, 'tree'),
                       (False, 'calendar'),
                       (False, 'graph')],
    ...
    def send_invite_email(self, cr, uid, wizard_data, context = None):
        ...
        for result_line in wizard_data.result_line_ids:
            ...
            notification_obj.create(cr, uid, {'user_id': result_line.user_id.id,
    ...
    def send_emails(self, cr, uid, wizard_data, context = None):
        ...
        for result_line in wizard_data.result_line_ids:
            ...
            body += '--\n'
            body += _('OpenERP is a powerful and user-friendly suite of Business Applications (CRM, Sales, HR, etc.)\nIt is open source and can be found on http://www.openerp.com.')
            mail_ids.append(mail_mail.create(cr, uid, {'email_from': user.email,
             'email_to': email_to,
             'subject': subject,
...
class share_result_line(osv.osv_memory):
    ...
    _columns = {'user_id': fields.many2one('res.users', required=True, readonly=True),

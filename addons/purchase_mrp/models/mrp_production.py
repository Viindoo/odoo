    ...
    def _compute_purchase_order_count(self):
        ...
        self.env.cr.execute("""
            ...
            FROM (
                ...
                SELECT sm.group_id, pol.order_id
                FROM stock_move sm
                JOIN stock_move_move_rel rel ON rel.move_dest_id = sm.id
                JOIN stock_move sm_orig ON sm_orig.id = rel.move_orig_id
                JOIN purchase_order_line pol ON pol.id = sm_orig.purchase_line_id
                WHERE sm.group_id IN %(groups)s
            ) AS combined
            GROUP BY group_id
        """, {

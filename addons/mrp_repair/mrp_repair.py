...
class mrp_repair(osv.osv):
    ...
    _columns = {'name': fields.char('Repair Reference', size=24, required=True, states={'confirmed': [('readonly', True)]}),
     ...
     'state': fields.selection([('draft', 'Quotation'),
               ...
               ('done', 'Repaired')], 'Status', readonly=True, track_visibility='onchange', help=" * The 'Draft' status is used when a user is encoding a new and unconfirmed repair order.             \n* The 'Confirmed' status is used when a user confirms the repair order.             \n* The 'Ready to Repair' status is used to start to repairing, user can start repairing only after repair order is confirmed.             \n* The 'To be Invoiced' status is used to generate the invoice before or after repairing done.             \n* The 'Done' status is set when repairing is completed.            \n* The 'Cancelled' status is used when user cancel repair order."),
     ...
     'invoice_method': fields.selection([('none', 'No Invoice'), ('b4repair', 'Before Repair'), ('after_repair', 'After Repair')], 'Invoice Method', select=True, required=True, states={'draft': [('readonly', False)]}, readonly=True, help="Selecting 'Before Repair' or 'After Repair' will allow you to generate invoice before or after the repair is done respectively. 'No invoice' means you don't want to generate invoice for this repair order."),
     ...
     'amount_untaxed': fields.function(_amount_untaxed, string='Untaxed Amount', store={'mrp.repair': (lambda self, cr, uid, ids, c = {}: ids, ['operations'], 10),
                        'mrp.repair.line': (_get_lines, ['price_unit',
                                             'price_subtotal',
                                             'product_id',
                                             'tax_id',
                                             'product_uom_qty',
                                             'product_uom'], 10)}),
     'amount_tax': fields.function(_amount_tax, string='Taxes', store={'mrp.repair': (lambda self, cr, uid, ids, c = {}: ids, ['operations'], 10),
                    'mrp.repair.line': (_get_lines, ['price_unit',
                                         'price_subtotal',
                                         'product_id',
                                         'tax_id',
                                         'product_uom_qty',
                                         'product_uom'], 10)}),
     'amount_total': fields.function(_amount_total, string='Total', store={'mrp.repair': (lambda self, cr, uid, ids, c = {}: ids, ['operations'], 10),
                      'mrp.repair.line': (_get_lines, ['price_unit',
                                           'price_subtotal',
                                           'product_id',
                                           'tax_id',
    ...
    def onchange_partner_id(self, cr, uid, ids, part, address_id):
        ...
        return {'value': {'address_id': addr['delivery'] or addr['default'],
        ...
        for repair in self.browse(cr, uid, ids, context=context):
            ...
            if repair.invoice_method != 'none':
                if group and repair.partner_invoice_id.id in invoices_group:
                    ...
                    invoice_vals = {'name': invoice.name + ', ' + repair.name,
                ...
                for operation in repair.operations:
                    if operation.to_invoice == True:
                        ...
                        invoice_line_id = inv_line_obj.create(cr, uid, {'invoice_id': inv_id,
                         'name': name,
                         'origin': repair.name,
                         'account_id': account_id,
                ...
                for fee in repair.fees_lines:
                    if fee.to_invoice == True:
                        ...
                        invoice_fee_id = inv_line_obj.create(cr, uid, {'invoice_id': inv_id,
                         'name': name,
                         'origin': repair.name,
                         'account_id': account_id,
                        ...
                        repair_fee_obj.write(cr, uid, [fee.id], {'invoiced': True,
    ...
    def action_repair_ready(self, cr, uid, ids, context = None):
        ...
        """
        for repair in self.browse(cr, uid, ids, context=context):
            self.pool.get('mrp.repair.line').write(cr, uid, [ l.id for l in repair.operations ], {'state': 'confirmed'}, context=context)
    ...
    def action_repair_start(self, cr, uid, ids, context = None):
        ...
        for repair in self.browse(cr, uid, ids, context=context):
            repair_line.write(cr, uid, [ l.id for l in repair.operations ], {'state': 'confirmed'}, context=context)
    ...
    def action_repair_done(self, cr, uid, ids, context = None):
        ...
        for repair in self.browse(cr, uid, ids, context=context):
            for move in repair.operations:
                move_id = move_obj.create(cr, uid, {'name': move.name,
            ...
            if repair.deliver_bool:
                ...
                picking = pick_obj.create(cr, uid, {'name': pick_name,
                 'origin': repair.name,
                 'state': 'draft',
                 'move_type': 'one',
                 'partner_id': repair.address_id and repair.address_id.id or False,
                 ...
                 'invoice_state': 'none',
                 'type': 'out'})
                move_id = move_obj.create(cr, uid, {'name': repair.name,
                 'picking_id': picking,
        ...
        if product:
            ...
            if not pricelist:
                warning = {'title': 'No Pricelist!',
                 'message': 'You have to select a pricelist in the Repair form !\nPlease set one before choosing a product.'}
            else:
                price = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist], product, product_uom_qty, partner_id, {'uom': uom})[pricelist]
                if price is False:
                    warning = {'title': 'No valid pricelist line found !',
                     'message': "Couldn't find a pricelist line matching this product and quantity.\nYou have to change either the product, the quantity or the pricelist."}
                else:
                    result.update({'price_unit': price,
                     'price_subtotal': price * product_uom_qty})
        return {'value': result,
         'warning': warning}
...
class mrp_repair_line(osv.osv, ProductChangeMixin):
    ...
    def copy_data(self, cr, uid, id, default = None, context = None):
        if not default:
            default = {}
        default.update({'invoice_line_id': False,
         'move_id': False,
         'invoiced': False,
         'state': 'draft'})
    ...
    _columns = {'name': fields.char('Description', size=64, required=True),
     ...
     'state': fields.selection([('draft', 'Draft'),
               ('confirmed', 'Confirmed'),
               ('done', 'Done'),
               ('cancel', 'Cancelled')], 'Status', required=True, readonly=True, help=" * The 'Draft' status is set automatically as draft when repair order in draft status.                         \n* The 'Confirmed' status is set automatically as confirm when repair order in confirm status.                         \n* The 'Done' status is set automatically when repair order is completed.                        \n* The 'Cancelled' status is set automatically when user cancel repair order.")}
    _defaults = {'state': lambda *a: 'draft',
...
class mrp_repair_fee(osv.osv, ProductChangeMixin):
    ...
    _columns = {'repair_id': fields.many2one('mrp.repair', 'Repair Order Reference', required=True, ondelete='cascade', select=True),
     ...
     'to_invoice': fields.boolean('To Invoice'),
     'invoiced': fields.boolean('Invoiced', readonly=True)}
    _defaults = {'to_invoice': lambda *a: True}

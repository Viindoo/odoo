...
class project_project(osv.osv):
    ...
    def open_timesheets(self, cr, uid, ids, context = None):
        ...
        view_context = {'search_default_account_id': [project.analytic_account_id.id],
...
class project_work(osv.osv):
    ...
    def get_user_related_details(self, cr, uid, user_id):
        ...
        if not emp_id:
            ...
            raise osv.except_osv(_('Bad Configuration!'), _('Please define employee for user "%s". You must create one.') % (user_name,))
        emp = emp_obj.browse(cr, uid, emp_id[0])
        if not emp.product_id:
            raise osv.except_osv(_('Bad Configuration!'), _('Please define product and product category property account on the related employee.\nFill in the HR Settings tab of the employee form.'))
        if not emp.journal_id:
            raise osv.except_osv(_('Bad Configuration!'), _('Please define journal on the related employee.\nFill in the timesheet tab of the employee form.'))
    ...
    def create(self, cr, uid, vals, *args, **kwargs):
        ...
        if not context.get('no_analytic_entry', False):
            ...
            if acc_id:
                ...
                amount_unit = timesheet_obj.on_change_unit_amount(cr, uid, timeline_id, prod_id, amount, False, unit, vals_line['journal_id'], context=context)
    ...
    def write(self, cr, uid, ids, vals, context = None):
        ...
        for task in self.browse(cr, uid, ids, context=context):
            ...
            if 'hours' in vals:
                vals_line['unit_amount'] = vals['hours']
                prod_id = vals_line.get('product_id', line_id.product_id.id)
                ...
                amount_unit = timesheet_obj.on_change_unit_amount(cr, uid, line_id.id, prod_id=prod_id, company_id=False, unit_amount=vals_line['unit_amount'], unit=False, journal_id=vals_line['journal_id'], context=context)
                if amount_unit and 'amount' in amount_unit.get('value', {}):
    ...
    _columns = {'hr_analytic_timesheet_id': fields.many2one('hr.analytic.timesheet', 'Related Timeline Id', ondelete='set null')}
...
class res_partner(osv.osv):
    ...
    def unlink(self, cursor, user, ids, context = None):
        ...
        return super(res_partner, self).unlink(cursor, user, ids, context=context)

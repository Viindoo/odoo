...
class hr_expense_expense(osv.osv):
    ...
    _order = 'id desc'
    _track = {'state': {'hr_expense.mt_expense_approved': lambda self, cr, uid, obj, ctx = None: obj['state'] == 'accepted',
    ...
    _columns = {'name': fields.char('Description', size=128, required=True, readonly=True, states={'draft': [('readonly', False)],
     ...
     'state': fields.selection([('draft', 'New'),
               ...
               ('paid', 'Paid')], 'Status', readonly=True, track_visibility='onchange', help="When the expense request is created the status is 'Draft'.\n It is confirmed by the user and request is sent to admin, the status is 'Waiting Confirmation'.            \nIf the admin accepts it, the status is 'Accepted'.\n If the accounting entries are made for the expense request, the status is 'Waiting Payment'.")}
    _defaults = {'company_id': lambda s, cr, uid, c: s.pool.get('res.company')._company_default_get(cr, uid, 'hr.employee', context=c),
    ...
    def copy(self, cr, uid, id, default = None, context = None):
        if default is None:
            default = {}
        default.update(account_move_id=False, voucher_id=False, date_confirm=False, date_valid=False, user_valid=False)
    ...
    def expense_accept(self, cr, uid, ids, context = None):
        return self.write(cr, uid, ids, {'state': 'accepted',
         ...
         'user_valid': uid}, context=context)
    ...
    def line_get_convert(self, cr, uid, x, part, date, context = None):
        ...
        return {'date_maturity': x.get('date_maturity', False),
    ...
    def compute_expense_totals(self, cr, uid, exp, company_currency, ref, account_move_lines, context = None):
        ...
        for i in account_move_lines:
            if exp.currency_id.id != company_currency:
                ...
                i['price'] = cur_obj.compute(cr, uid, exp.currency_id.id, company_currency, i['price'], context=context)
            else:
    ...
    def action_receipt_create(self, cr, uid, ids, context = None):
        ...
        for exp in self.browse(cr, uid, ids, context=context):
            ...
            self.write(cr, uid, ids, {'account_move_id': move_id,
    ...
    def move_line_get(self, cr, uid, expense_id, context = None):
        ...
        for line in exp.line_ids:
            ...
            if not taxes:
                continue
            for tax in tax_obj.compute_all(cr, uid, taxes, line.unit_amount, line.unit_quantity, line.product_id, exp.user_id.partner_id)['taxes']:
...
class product_product(osv.osv):
    ...
    _columns = {'hr_expense_ok': fields.boolean('Can be Expensed', help='Specify if the product can be selected in an HR expense line.')}
...
class hr_expense_line(osv.osv):
    ...
    _columns = {'name': fields.char('Expense Note', size=128, required=True),

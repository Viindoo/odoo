...
CRM_CLAIM_PENDING_STATES = (crm.AVAILABLE_STATES[2][0], crm.AVAILABLE_STATES[3][0], crm.AVAILABLE_STATES[4][0])
...
class crm_claim_stage(osv.osv):
    ...
    _columns = {'name': fields.char('Stage Name', size=64, required=True, translate=True),
     ...
     'section_ids': fields.many2many('crm.case.section', 'section_claim_stage_rel', 'stage_id', 'section_id', string='Sections', help='Link between stages and sales teams. When set, this limitate the current stage to the selected sales teams.'),
     ...
     'case_refused': fields.boolean('Refused stage', help='Refused stages are specific stages for done.'),
...
class crm_claim(base_stage, osv.osv):
    ...
    _columns = {'id': fields.integer('ID', readonly=True),
     ...
     'stage_id': fields.many2one('crm.claim.stage', 'Stage', track_visibility='onchange', domain="['&',('fold', '=', False),'|', ('section_ids', '=', section_id), ('case_default', '=', True)]"),
    ...
    _defaults = {'user_id': lambda s, cr, uid, c: s._get_default_user(cr, uid, c),
...
class res_partner(osv.osv):
    ...
    _columns = {'claims_ids': fields.one2many('crm.claim', 'partner_id', 'Claims')}
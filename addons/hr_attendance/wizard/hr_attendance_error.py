...
class hr_attendance_error(osv.osv_memory):
    ...
    _columns = {'init_date': fields.date('Starting Date', required=True),
     'end_date': fields.date('Ending Date', required=True),
     'max_delay': fields.integer('Max. Delay (Min)', required=True)}
    _defaults = {'init_date': lambda *a: time.strftime('%Y-%m-%d'),
    ...
    def print_report(self, cr, uid, ids, context = None):
        ...
        cr.execute("SELECT id FROM hr_attendance WHERE employee_id IN %s AND to_char(name,'YYYY-mm-dd')<=%s AND to_char(name,'YYYY-mm-dd')>=%s AND action IN %s ORDER BY name", (tuple(context['active_ids']),
         date_to,
         date_from,
         tuple(['sign_in', 'sign_out'])))

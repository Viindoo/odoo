...
class hr_action_reason(osv.osv):
    ...
    _columns = {'name': fields.char('Reason', size=64, required=True, help='Specifies the reason for Signing In/Signing Out.'),
     'action_type': fields.selection([('sign_in', 'Sign in'), ('sign_out', 'Sign out')], 'Action Type')}
    _defaults = {'action_type': 'sign_in'}
...
class hr_attendance(osv.osv):
    ...
    _columns = {'name': fields.datetime('Date', required=True, select=1),
...
class hr_employee(osv.osv):
    ...
    def _state(self, cr, uid, ids, name, args, context = None):
        ...
        cr.execute("SELECT hr_attendance.action, hr_attendance.employee_id                 FROM (                     SELECT MAX(name) AS name, employee_id                     FROM hr_attendance                     WHERE action in ('sign_in', 'sign_out')                     GROUP BY employee_id                 ) AS foo                 LEFT JOIN hr_attendance                     ON (hr_attendance.employee_id = foo.employee_id                         AND hr_attendance.name = foo.name)                 WHERE hr_attendance.employee_id IN %s", (tuple(ids),))
    ...
    _columns = {'state': fields.function(_state, type='selection', selection=[('absent', 'Absent'), ('present', 'Present')], string='Attendance'),
    ...
    def attendance_action_change(self, cr, uid, ids, context = None):
        ...
        warning_sign = {'sign_in': _('Sign In'),
         'sign_out': _('Sign Out')}

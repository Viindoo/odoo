...
class sale_order(osv.Model):
    ...
    _columns = {'portal_payment_options': fields.function(_payment_block_proxy, type='html', string='Portal Payment Options')}
    ...
    def _portal_payment_block(self, cr, uid, ids, fieldname, arg, context = None):
        ...
        for this in self.browse(cr, uid, ids, context=context):
            if this.state not in ('draft', 'cancel') and not this.invoiced:
                result[this.id] = payment_acquirer.render_payment_block(cr, uid, this, this.name, this.pricelist_id.currency_id, this.amount_total, context=context)
    ...
    def action_button_confirm(self, cr, uid, ids, context = None):
        if not len(ids) == 1:
            ...
            partner.id not in document.message_follower_ids and self.message_subscribe(cr, uid, ids, [partner.id], context=context)
    ...
    def _portal_payment_block(self, cr, uid, ids, fieldname, arg, context = None):
        ...
        for this in self.browse(cr, uid, ids, context=context):
            if this.type == 'out_invoice' and this.state not in ('draft', 'done') and not this.reconciled:
                result[this.id] = payment_acquirer.render_payment_block(cr, uid, this, this.number, this.currency_id, this.residual, context=context)

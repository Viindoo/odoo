...
DATE_RANGE_FUNCTION = {'minutes': lambda interval: timedelta(minutes=interval),
...
class base_action_rule(osv.osv):
    ...
    _columns = {'name': fields.char('Rule Name', size=64, required=True),
     ...
     'active': fields.boolean('Active', help='When unchecked, the rule is hidden and will not be executed.'),
     'sequence': fields.integer('Sequence', help='Gives the sequence order when displaying a list of rules.'),
     'trg_date_id': fields.many2one('ir.model.fields', string='Trigger Date', help='When should the condition be triggered. If present, will be checked by the scheduler. If empty, will be checked at creation and update.', domain="[('model_id', '=', model_id), ('ttype', 'in', ('date', 'datetime'))]"),
     'trg_date_range': fields.integer('Delay after trigger date', help='Delay after the trigger date.You can put a negative number if you need a delay before thetrigger date, like sending a reminder 15 minutes before a meeting.'),
     'trg_date_range_type': fields.selection([('minutes', 'Minutes'),
                             ('hour', 'Hours'),
                             ('day', 'Days'),
                             ('month', 'Months')], 'Delay type'),
     ...
     'filter_id': fields.many2one('ir.filters', string='After Update Filter', ondelete='restrict', domain="[('model_id', '=', model_id.model)]", help='If present, this condition must be satisfied after the update of the record.'),
    ...
    def _filter(self, cr, uid, action, action_filter, record_ids, context = None):
        ...
        if not (record_ids and action_filter and action.model == action_filter.model_id):
            raise AssertionError('Filter model different from action rule model')

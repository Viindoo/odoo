...
class delivery_carrier(osv.osv):
    ...
    _columns = {'name': fields.char('Delivery Method', size=64, required=True),
    ...
    def create_grid_lines(self, cr, uid, ids, vals, context = None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if record.free_if_more_than:
                line_data = {'grid_id': grid_id and grid_id[0],
...
class delivery_grid(osv.osv):
    ...
    _columns = {'name': fields.char('Grid Name', size=64, required=True),
...
class delivery_grid_line(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True),
     ...
     'type': fields.selection([('weight', 'Weight'),
              ('volume', 'Volume'),
              ('wv', 'Weight * Volume'),
              ('price', 'Price')], 'Variable', required=True),
     ...
     'variable_factor': fields.selection([('weight', 'Weight'),
                         ('volume', 'Volume'),
                         ('wv', 'Weight * Volume'),
                         ('price', 'Price')], 'Variable Factor', required=True),
    ...
    _defaults = {'type': lambda *args: 'weight',

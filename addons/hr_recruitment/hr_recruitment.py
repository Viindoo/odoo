...
class hr_recruitment_source(osv.osv):
    ...
    _columns = {'name': fields.char('Source Name', size=64, required=True, translate=True)}
...
class hr_applicant(base_stage, osv.Model):
    ...
    _track = {'state': {'hr_recruitment.mt_applicant_hired': lambda self, cr, uid, obj, ctx = None: obj['state'] == 'done',
               'hr_recruitment.mt_applicant_refused': lambda self, cr, uid, obj, ctx = None: obj['state'] == 'cancel'},
     'stage_id': {'hr_recruitment.mt_stage_changed': lambda self, cr, uid, obj, ctx = None: obj['state'] not in ('done', 'cancel')}}
    ...
    _columns = {'name': fields.char('Subject', size=128, required=True),
     ...
     'stage_id': fields.many2one('hr.recruitment.stage', 'Stage', track_visibility='onchange', domain="['&', ('fold', '=', False), '|', ('department_id', '=', department_id), ('department_id', '=', False)]"),
     'state': fields.related('stage_id', 'state', type='selection', store=True, selection=AVAILABLE_STATES, string='Status', readonly=True, help="The status is set to 'Draft', when a case is created.                      If the case is in progress the status is set to 'Open'.                      When the case is over, the status is set to 'Done'.                      If the case needs to be reviewed then the status is                       set to 'Pending'."),
    ...
    def action_makeMeeting(self, cr, uid, ids, context = None):
        ...
        res['context'] = {'default_partner_ids': applicant.partner_id and [applicant.partner_id.id] or False,
         'default_user_id': uid,
    ...
    def action_print_survey(self, cr, uid, ids, context = None):
        ...
        context.update({'survey_id': record.survey.id,
    ...
    def message_update(self, cr, uid, ids, msg, update_vals = None, context = None):
        ...
        if update_vals is None:
            update_vals = {}
        update_vals.update({'email_from': msg.get('from'),
         'email_cc': msg.get('cc')})
        if msg.get('priority'):
...
class hr_job(osv.osv):
    ...
    _columns = {'survey_id': fields.many2one('survey', 'Interview Form', help='Choose an interview form for this job position and you will be able to print/answer this interview from all applicants who apply for this job'),
     'alias_id': fields.many2one('mail.alias', 'Alias', ondelete='restrict', required=True, help='Email alias for this job position. New emails will automatically create new applicants for this job position.')}
    _defaults = {'alias_domain': False}
    ...
    def _auto_init(self, cr, context = None):
        ...
        self.pool.get('mail.alias').migrate_to_alias(cr, self._name, self._table, super(hr_job, self)._auto_init, self._columns['alias_id'], 'name', alias_prefix='job+', alias_defaults={'job_id': 'id'}, context=alias_context)
        return
    ...
    def create(self, cr, uid, vals, context = None):
        ...
        if not vals.get('alias_id'):
            ...
            alias_id = mail_alias.create_unique_alias(cr, uid, {'alias_name': 'jobs+' + vals['name']}, model_name='hr.applicant', context=context)

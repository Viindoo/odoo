...
class hr_recruitment_partner_create(osv.osv_memory):
    ...
    _columns = {'close': fields.boolean('Close job request')}
    ...
    def view_init(self, cr, uid, fields_list, context = None):
        ...
        for case in case_obj.browse(cr, uid, context['active_ids'], context=context):
            if case.partner_id:
                raise osv.except_osv(_('Error!'), _('A contact is already defined on this job request.'))
    ...
    def make_order(self, cr, uid, ids, context = None):
        ...
        for case in case_obj.browse(cr, uid, context['active_ids'], context=context):
            ...
            partner_id = partner_obj.create(cr, uid, {'name': case.partner_name or case.name,
             'user_id': case.user_id.id,
             'comment': case.description,
             'phone': case.partner_phone,
             'mobile': case.partner_mobile,
             'email': case.email_from}, context=context)
            case_obj.write(cr, uid, [case.id], {'partner_id': partner_id}, context=context)

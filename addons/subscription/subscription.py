...
class subscription_document(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=60, required=True),
...
class subscription_document_fields(osv.osv):
    ...
    _rec_name = 'field'
    _columns = {'field': fields.many2one('ir.model.fields', 'Field', domain="[('model_id', '=', parent.model)]", required=True),
...
class subscription_subscription(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=60, required=True),
     ...
     'cron_id': fields.many2one('ir.cron', 'Cron Job', help='Scheduler which runs on subscription', states={'running': [('readonly', True)],
                 'done': [('readonly', True)]}),
     'note': fields.text('Notes', help='Description or Summary of Subscription')}
    _defaults = {'date_init': lambda *a: time.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    def model_copy(self, cr, uid, ids, context = None):
        for row in self.read(cr, uid, ids, context=context):
            ...
            self.pool.get('subscription.subscription.history').create(cr, uid, {'subscription_id': row['id'],
...
class subscription_subscription_history(osv.osv):
    ...
    _rec_name = 'date'
    _columns = {'date': fields.datetime('Date'),

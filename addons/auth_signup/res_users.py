...
class res_partner(osv.Model):
    ...
    def _get_signup_valid(self, cr, uid, ids, name, arg, context = None):
        ...
        for partner in self.browse(cr, uid, ids, context):
            res[partner.id] = bool(partner.signup_token) and (not partner.signup_expiration or dt <= partner.signup_expiration)
    ...
    def _signup_retrieve_partner(self, cr, uid, token, check_validity = False, raise_exception = False, context = None):
...
class res_users(osv.Model):
    ...
    _columns = {'state': fields.function(_get_state, string='Trạng thái', type='selection', selection=[('new', 'Chưa kích hoạt'), ('active', 'Đã kích hoạt')])}
    ...
    def signup(self, cr, uid, values, token = None, context = None):
        ...
        if token:
            ...
            partner = res_partner._signup_retrieve_partner(cr, uid, token, check_validity=True, raise_exception=True, context=None)
            ...
            values.update({'name': partner.name,
    ...
    def _signup_create_user(self, cr, uid, values, context = None):
        ...
        if not (template_user_id and self.exists(cr, uid, template_user_id, context=context)):
            assert AssertionError('Signup: invalid template user')
            if 'partner_id' not in values:
                raise literal_eval(ir_config_parameter.get_param(cr, uid, 'auth_signup.allow_uninvited', 'False')) or SignupError('Signup is not allowed for uninvited users')
        assert values.get('login') or AssertionError('Signup: no login given for new user')
        assert values.get('partner_id') or values.get('name') or AssertionError('Signup: no name or partner given for new user')
        values['active'] = True
    ...
    def reset_password(self, cr, uid, login, context = None):
        ...
        if len(user_ids) != 1:
            assert Exception('Reset password: invalid username or email')
    ...
    def action_reset_password(self, cr, uid, ids, context = None):
        ...
        assert template._name == 'email.template' or AssertionError
        for user in self.browse(cr, uid, ids, context):
            if not user.email:

...
class project_issue_version(osv.osv):
    ...
    _order = 'name desc'
    _columns = {'name': fields.char('Version Number', size=32, required=True),
...
class project_issue(base_stage, osv.osv):
    ...
    _track = {'state': {'project_issue.mt_issue_new': lambda self, cr, uid, obj, ctx = None: obj['state'] in ('new', 'draft'),
     ...
     'stage_id': {'project_issue.mt_issue_stage': lambda self, cr, uid, obj, ctx = None: obj['state'] not in ('new', 'draft', 'done', 'open')},
     'kanban_state': {'project_issue.mt_issue_blocked': lambda self, cr, uid, obj, ctx = None: obj['kanban_state'] == 'blocked'}}
    ...
    def _compute_day(self, cr, uid, ids, fields, args, context = None):
        ...
        @param ids: List of Openday\xe2\x80\x99s IDs
            ...
            for field in fields:
                ...
                if field in ('working_hours_open', 'day_open'):
                    if issue.date_open:
                        ...
                        hours = cal_obj._interval_hours_get(cr, uid, working_hours, date_create, date_open, timezone_from_uid=issue.user_id.id or uid, exclude_leaves=False, context=context)
                elif field in ('working_hours_close', 'day_close'):
                    if issue.date_closed:
                        date_close = datetime.strptime(issue.date_closed, '%Y-%m-%d %H:%M:%S')
                        date_until = issue.date_closed
                        ans = date_close - date_create
                        hours = cal_obj._interval_hours_get(cr, uid, working_hours, date_create, date_close, timezone_from_uid=issue.user_id.id or uid, exclude_leaves=False, context=context)
    ...
    _columns = {'id': fields.integer('ID', readonly=True),
     ...
     'days_since_creation': fields.function(_compute_day, string='Days since creation date', multi='compute_day', type='integer', help='Difference in days between creation date and current date'),
     ...
     'kanban_state': fields.selection([('normal', 'Normal'), ('blocked', 'Blocked'), ('done', 'Ready for next stage')], 'Kanban State', track_visibility='onchange', help="A Issue's kanban state indicates special situations affecting it:\n * Normal is the default situation\n * Blocked indicates something is preventing the progress of this issue\n * Ready for next stage indicates the issue is ready to be pulled to the next stage", readonly=True, required=False),
     ...
     'day_open': fields.function(_compute_day, string='Days to Open', multi='compute_day', type='float', store=True),
     'day_close': fields.function(_compute_day, string='Days to Close', multi='compute_day', type='float', store=True),
     'user_id': fields.many2one('res.users', 'Assigned to', required=False, select=1, track_visibility='onchange'),
     'working_hours_open': fields.function(_compute_day, string='Working Hours to Open the Issue', multi='compute_day', type='float', store=True),
     'working_hours_close': fields.function(_compute_day, string='Working Hours to Close the Issue', multi='compute_day', type='float', store=True),
     'inactivity_days': fields.function(_compute_day, string='Days since last action', multi='compute_day', type='integer', help='Difference in days between last action and current date'),
     'color': fields.integer('Color Index'),
     'user_email': fields.related('user_id', 'email', type='char', string='User Email', readonly=True),
     'date_action_last': fields.datetime('Last Action', readonly=1),
     'date_action_next': fields.datetime('Next Action', readonly=1),
     'progress': fields.function(_hours_get, string='Progress (%)', multi='hours', group_operator='avg', help='Computed as: Time Spent / Total Time.', store={'project.issue': (lambda self, cr, uid, ids, c = {}: ids, ['task_id'], 10),
    ...
    def convert_issue_task(self, cr, uid, ids, context = None):
        ...
        for bug in case_obj.browse(cr, uid, ids, context=context):
            new_task_id = task_obj.create(cr, uid, {'name': bug.name,
    ...
    def case_reset(self, cr, uid, ids, context = None):
        ...
        self.write(cr, uid, ids, {'date_open': False,
         'date_closed': False})
        return res
    ...
    def case_escalate(self, cr, uid, ids, context = None):
        ...
        for case in cases:
            ...
            if case.project_id.project_escalation_id:
                ...
                if case.task_id:
                    self.pool.get('project.task').write(cr, uid, [case.task_id.id], {'project_id': data['project_id'],
                     'user_id': False})
            else:
    ...
    def message_get_reply_to(self, cr, uid, ids, context = None):
        ...
        return [ (issue.project_id.message_get_reply_to()[0] if issue.project_id else False) for issue in self.browse(cr, uid, ids, context=context) ]
    ...
    def message_new(self, cr, uid, msg, custom_values = None, context = None):
        ...
        defaults = {'name': msg.get('subject') or _('No Subject'),
         'description': desc,
...
class project(osv.osv):
    ...
    _columns = {'project_escalation_id': fields.many2one('project.project', 'Project Escalation', help='If any issue is escalated from the current Project, it will be listed under the project selected here.', states={'close': [('readonly', True)],
    ...
    _constraints = [(_check_escalation, 'Error! You cannot assign escalation to the same project!', ['project_escalation_id'])]
...
class account_analytic_account(osv.osv):
    ...
    _columns = {'use_issues': fields.boolean('Issues', help='Check this field if this project manages issues')}
    ...
    def _trigger_project_creation(self, cr, uid, vals, context = None):
        ...
        return res or vals.get('use_issues') and 'project_creation_in_progress' not in context

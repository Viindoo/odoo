...
class account_budget_post(osv.osv):
    ...
    _columns = {'code': fields.char('Code', size=64, required=True),
     'name': fields.char('Name', size=256, required=True),
     'account_ids': fields.many2many('account.account', 'account_budget_rel', 'budget_id', 'account_id', 'Accounts'),
     'crossovered_budget_line': fields.one2many('crossovered.budget.lines', 'general_budget_id', 'Budget Lines'),
     'company_id': fields.many2one('res.company', 'Company', required=True)}
    _defaults = {'company_id': lambda self, cr, uid, c: self.pool.get('res.company')._company_default_get(cr, uid, 'account.budget.post', context=c)}
    _order = 'name'
...
class crossovered_budget(osv.osv):
    ...
    _description = 'Budget'
    _columns = {'name': fields.char('Name', size=64, required=True, states={'done': [('readonly', True)]}),
...
class crossovered_budget_lines(osv.osv):
    ...
    def _prac_amt(self, cr, uid, ids, context = None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if line.analytic_account_id.id:
                cr.execute("SELECT SUM(amount) FROM account_analytic_line WHERE account_id=%s AND (date between to_date(%s,'yyyy-mm-dd') AND to_date(%s,'yyyy-mm-dd')) AND general_account_id=ANY(%s)", (line.analytic_account_id.id,
                 date_from,
                 date_to,
                 acc_ids))
    ...
    _columns = {'crossovered_budget_id': fields.many2one('crossovered.budget', 'Budget', ondelete='cascade', select=True, required=True),
...
class account_analytic_account(osv.osv):
    ...
    _columns = {'crossovered_budget_line': fields.one2many('crossovered.budget.lines', 'analytic_account_id', 'Budget Lines')}

...
class document_page_create_menu(osv.osv_memory):
    ...
    _columns = {'menu_name': fields.char('Menu Name', size=256, required=True),
    ...
    def document_page_menu_create(self, cr, uid, ids, context = None):
        ...
        if not data:
            ...
            menu_id = obj_menu.create(cr, uid, {'name': data.menu_name,
             'parent_id': data.menu_parent_id.id,
             'icon': 'STOCK_DIALOG_QUESTION',
             'action': 'ir.actions.act_window,' + str(action_id)}, context)

...
class resource_calendar(osv.osv):
    ...
    _defaults = {'company_id': lambda self, cr, uid, context: self.pool.get('res.company')._company_default_get(cr, uid, 'resource.calendar', context=context)
        }
    ...
    def _get_leaves(self, cr, uid, id, resource):
        ...
        dt_leave = []
        resource_leave_ids = resource_cal_leaves.search(cr, uid, [('calendar_id', '=', id),
         '|',
         ('resource_id', '=', False),
    ...
    def _interval_hours_get(self, cr, uid, id, dt_from, dt_to, resource_id = False, timezone_from_uid = None, exclude_leaves = True, context = None):
        ...
        for day in rrule.rrule(rrule.DAILY, dtstart=interval_start, until=interval_end + timedelta(days=1), byweekday=hours_range_per_weekday.keys()):
...
class resource_calendar_attendance(osv.osv):
    ...
    _columns = {
         ...
         'dayofweek': fields.selection([('0', 'Monday'),
                       ...
                       ('4', 'Friday'),
                       ('5', 'Saturday'),
                       ('6', 'Sunday')], 'Day of Week', required=True, select=True),
...
class resource_resource(osv.osv):
    ...
    def generate_resources(self, cr, uid, user_ids, calendar_id, context = None):
        ...
        for user in user_pool.browse(cr, uid, user_ids, context=context):
            resource_objs[user.id] = {'name': user.name,
             'vacation': [],
             'efficiency': 1.0}
    ...
    def compute_vacation(self, cr, uid, calendar_id, resource_id = False, resource_calendar = False, context = None):
        ...
        leave_list = []
        if resource_id:
            leave_ids = resource_calendar_leaves_pool.search(cr, uid, ['|',
             ('calendar_id', '=', calendar_id),
             ('calendar_id', '=', resource_calendar),
             ('resource_id', '=', resource_id)], context=context)
        else:
            leave_ids = resource_calendar_leaves_pool.search(cr, uid, [('calendar_id', '=', calendar_id), ('resource_id', '=', False)], context=context)
...
class resource_calendar_leaves(osv.osv):
    ...
    _constraints = [(check_dates, 'Error! leave start-date must be lower then leave end-date.', ['date_from', 'date_to'])]

...
DEFAULT_WORKING_TIMES = ((480, 720), (780, 1020))
        ...
        if is_start:
            if is_free:
                ...
                if not work_count:
                    ...
                    last = date
                work_count += 1
        elif not (is_free and free_count > 0):
            raise AssertionError
            free_count -= 1
            if not free_count and not work_count:
                sequence.append((last, date, True))
        elif not work_count > 0:
            raise AssertionError
            work_count -= 1
            if not work_count:
                sequence.append((last, date, False))
            last = free_count and date
...
def to_timedelta(src, cal = None, is_duration = False):
    ...
    for s in split:
        ...
        if not mo:
            raise ValueError(src + ' is not a valid duration: valid units are: d w m y M H')
        unit = mo.group(3)
...
def timedelta_to_str(delta, format, cal = None, is_duration = False):
    ...
    def rebase(d_r, cond1, cond2, letter, divisor):
        ...
        if cond2:
            ...
            result = result.replace('%' + letter, locale.format('%.2f', float(days) / divisor))
        return (days, result)
    ...
    if has_hours:
        ...
        result = result.replace('%d', '%.2f' % (days + float(minutes) / (d_w_h * 60)))
    if has_hours:
        if has_minutes:
            val = minutes / 60
            if not val:
...
def strftime(dt, format):
    ...
    if iso[0] != dt.year:
        ...
        format = format.replace('%IB', iso_date.strftime('%B')).replace('%ib', iso_date.strftime('%b')).replace('%im', iso_date.strftime('%m'))
    else:
        format = format.replace('%IB', '%B').replace('%ib', '%b').replace('%im', '%m')
    format = format.replace('%IW', str(iso[1])).replace('%IY', str(iso[0]))
...
class _Minutes(_CalendarItem):
    ...
    def strftime(self, format = None, is_duration = False):
        ...
        return timedelta_to_str(td, format or self.STR_FORMAT, self.calendar, is_duration)
...
class Calendar(object):
    ...
    def from_datetime(self, value):
        if not isinstance(value, datetime.datetime):
            ...
            if pos >= 0:
                ...
                if value < end:
                    delta = nstart < nend and value - start
    ...
    def to_starttime(self, value):
        ...
        return self.EPOCH + datetime.timedelta(weeks=weeks, days=days, minutes=minutes)
    ...
    def _build_mapping(self):
        ...
        for start, end, is_free in self.time_spans:
            ...
            dt_num_can.append((start,
             end,
             nstart,
             nend,
             cend))
            num_dt_can.append((nstart,
             nend,
             start,
             end,
             cend))

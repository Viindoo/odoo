...
class payment_mode(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, help='Mode of Payment'),
    ...
    _defaults = {'company_id': lambda self, cr, uid, c: self.pool.get('res.users').browse(cr, uid, uid, c).company_id.id}
...
class payment_order(osv.osv):
    ...
    _columns = {'date_scheduled': fields.date('Scheduled Date', states={'done': [('readonly', True)]}, help='Select a date if you have chosen Preferred Date to be fixed.'),
     'reference': fields.char('Reference', size=128, required=1, states={'done': [('readonly', True)]}),
     'mode': fields.many2one('payment.mode', 'Payment Mode', select=True, required=1, states={'done': [('readonly', True)]}, help='Select the Payment Mode to be applied.'),
     'state': fields.selection([('draft', 'Draft'),
               ('cancel', 'Cancelled'),
               ('open', 'Confirmed'),
               ('done', 'Done')], 'Status', select=True, help="When an order is placed the status is 'Draft'.\n Once the bank is confirmed the status is set to 'Confirmed'.\n Then the order is paid the status is 'Done'."),
...
class payment_line(osv.osv):
    ...
    def select_by_name(self, cr, uid, ids, name, args, context = None):
        ...
        if name == 'partner_id':
            partner_name = {}
            for p_id, p_name in partner_obj.name_get(cr, uid, filter(lambda x: x and x != 0, res.values()), context=context):
    ...
    def _amount(self, cursor, user, ids, name, args, context = None):
        if not ids:
            ...
            for line in self.browse(cursor, user, ids, context=context):
                ...
                res[line.id] = currency_obj.compute(cursor, user, line.currency.id, line.company_currency.id, line.amount_currency, context=ctx)
    ...
    _columns = {'name': fields.char('Your Reference', size=64, required=True),
     ...
     'amount': fields.function(_amount, string='Amount in Company Currency', type='float', help='Payment amount in the company currency'),
    ...
    _defaults = {'name': lambda obj, cursor, user, context: obj.pool.get('ir.sequence').get(cursor, user, 'payment.line'),
     'state': 'normal',
     'currency': _get_currency,
     'company_currency': _get_currency,
     'date': _get_date}
    _sql_constraints = [('name_uniq', 'UNIQUE(name)', 'The payment line name must be unique!')]

...
class event_type(osv.osv):
    ...
    _columns = {'name': fields.char('Event Type', size=64, required=True),
    ...
    _defaults = {'default_registration_min': 0,
...
class event_event(osv.osv):
    ...
    def confirm_event(self, cr, uid, ids, context = None):
        ...
        if self.event.email_confirmation_id:
            reg_ids = register_pool.search(cr, uid, [('event_id', '=', self.event.id), ('state', 'not in', ['draft', 'cancel'])], context=context)
    ...
    _columns = {'name': fields.char('Name', size=64, required=True, translate=True, readonly=False, states={'done': [('readonly', True)]}),
     ...
     'state': fields.selection([('draft', 'Unconfirmed'),
               ('cancel', 'Cancelled'),
               ('confirm', 'Confirmed'),
               ('done', 'Done')], 'Status', readonly=True, required=True, track_visibility='onchange', help="If event is created, the status is 'Draft'.If event is confirmed for the particular dates the status is set to 'Confirmed'. If the event is over, the status is set to 'Done'.If event is cancelled the status is set to 'Cancelled'."),
     ...
     'country_id': fields.related('address_id', 'country_id', type='many2one', relation='res.country', string='Country', readonly=False, states={'done': [('readonly', True)]}),
    ...
    def subscribe_to_event(self, cr, uid, ids, context = None):
        ...
        if not curr_reg_ids:
            curr_reg_ids = [register_pool.create(cr, SUPERUSER_ID, {'event_id': ids[0],
              'email': user.email,
              'name': user.name,
              'user_id': user.id,
    ...
    _constraints = [(_check_closing_date, 'Error ! Closing Date cannot be set before Beginning Date.', ['date_end'])]

    def onchange_event_type(self, cr, uid, ids, type_event, context = None):
        if type_event:
            type_info = self.pool.get('event.type').browse(cr, uid, type_event, context)
            dic = {'reply_to': type_info.default_reply_to,
...
class event_registration(osv.osv):
    ...
    _columns = {'id': fields.integer('ID'),
     ...
     'state': fields.selection([('draft', 'Unconfirmed'),
               ('cancel', 'Cancelled'),
               ('open', 'Confirmed'),
               ('done', 'Attended')], 'Status', track_visibility='onchange', size=16, readonly=True),
     'email': fields.char('Email', size=64),
     'phone': fields.char('Phone', size=64),
     'name': fields.char('Name', size=128, select=True)}
    _defaults = {'nb_register': 1,
     'state': 'draft'}
    ...
    def onchange_contact_id(self, cr, uid, ids, contact, partner, context = None):
        ...
        return {'value': {'email': contact_id.email,

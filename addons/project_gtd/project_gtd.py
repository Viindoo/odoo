...
class project_gtd_context(osv.osv):
    ...
    _columns = {'name': fields.char('Context', size=64, required=True, select=1, translate=1),
...
class project_gtd_timebox(osv.osv):
    ...
    _order = 'sequence'
    _columns = {'name': fields.char('Timebox', size=64, required=True, select=1, translate=1),
...
class project_task(osv.osv):
    ...
    _columns = {'timebox_id': fields.many2one('project.gtd.timebox', 'Timebox', help='Time-laps during which task has to be treated'),
    ...
    _defaults = {'context_id': _get_context}

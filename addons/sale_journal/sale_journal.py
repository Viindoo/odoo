...
class sale_journal_invoice_type(osv.osv):
    ...
    _columns = {'name': fields.char('Invoice Type', size=64, required=True),
...
class res_partner(osv.osv):
    ...
    _columns = {'property_invoice_type': fields.property('sale_journal.invoice.type', type='many2one', relation='sale_journal.invoice.type', string='Invoicing Type', view_load=True, group_name='Accounting Properties', help='This invoicing type will be used, by default, to invoice the current partner.')}
...
class picking(osv.osv):
    ...
    _columns = {'invoice_type_id': fields.many2one('sale_journal.invoice.type', 'Invoice Type', readonly=True)}
...
class sale(osv.osv):
    _inherit = 'sale.order'
    _columns = {'invoice_type_id': fields.many2one('sale_journal.invoice.type', 'Invoice Type', help='Generate invoice based on the selected option.')}

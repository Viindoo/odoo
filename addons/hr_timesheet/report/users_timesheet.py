...
def emp_create_xml(cr, id, som, eom, emp):
    cr.execute('select line.date, (unit_amount / unit.factor) as amount from account_analytic_line as line, hr_analytic_timesheet as hr, product_uom as unit where hr.line_id=line.id and product_uom_id = unit.id and line.user_id=%s and line.date >= %s and line.date < %s order by line.date', (id, som.strftime('%Y-%m-%d'), eom.strftime('%Y-%m-%d')))
    month = {}

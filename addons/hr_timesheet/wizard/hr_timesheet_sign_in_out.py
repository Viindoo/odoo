...
class hr_so_project(osv.osv_memory):
    ...
    _columns = {'account_id': fields.many2one('account.analytic.account', 'Project / Analytic Account', domain=[('type', '=', 'normal')]),
    ...
    def _write(self, cr, uid, data, emp_id, context = None):
        ...
        if context is None:
            context = {}
        hour = (time.mktime(time.strptime(data['date'] or time.strftime('%Y-%m-%d %H:%M:%S'), '%Y-%m-%d %H:%M:%S')) - time.mktime(time.strptime(data['date_start'], '%Y-%m-%d %H:%M:%S'))) / 3600.0
    ...
    def sign_out_result_end(self, cr, uid, ids, context = None):
        ...
        for data in self.browse(cr, uid, ids, context=context):
            ...
            emp_obj.attendance_action_change(cr, uid, [emp_id], {'action': 'sign_out',
    ...
    def sign_out_result(self, cr, uid, ids, context = None):
        ...
        for data in self.browse(cr, uid, ids, context=context):
            emp_id = data.emp_id.id
            emp_obj.attendance_action_change(cr, uid, [emp_id], {'action': 'action',
             'action_date': data.date})
            self._write(cr, uid, data, emp_id, context=context)
...
class hr_si_project(osv.osv_memory):
    ...
    _columns = {'name': fields.char('Employees name', size=32, readonly=True),
     'state': fields.related('emp_id', 'state', string='Current Status', type='char', required=True, readonly=True),
    ...
    def check_state(self, cr, uid, ids, context = None):
        ...
        return {'name': _('Sign in / Sign out'),
         'view_type': 'form',
         'view_mode': 'tree,form',
    ...
    def sign_in_result(self, cr, uid, ids, context = None):
        ...
        for data in self.browse(cr, uid, ids, context=context):
            emp_id = data.emp_id.id
            emp_obj.attendance_action_change(cr, uid, [emp_id], {'action': 'sign_in',
             'action_date': data.date})

...
class hr_employee(osv.osv):
    ...
    _columns = {'product_id': fields.many2one('product.product', 'Product', help="Specifies employee's designation as a product with type 'service'."),
    ...
    _defaults = {'journal_id': _getAnalyticJournal,
...
class hr_analytic_timesheet(osv.osv):
    ...
    _order = 'id desc'
    _columns = {'line_id': fields.many2one('account.analytic.line', 'Analytic Line', ondelete='cascade', required=True),
    ...
    _defaults = {'product_uom_id': _getEmployeeUnit,
     'product_id': _getEmployeeProduct,
        ...
        if ids:
            ...
            if date != new_date:
                warning = {'title': 'User Alert!',
                 'message': 'Changing the date will let this entry appear in the timesheet of the new date.'}
                return {'value': {},
                 'warning': warning}
        return {'value': {}}
    ...
    def on_change_user_id(self, cr, uid, ids, user_id):
        ...
        return {'value': {'product_id': self._getEmployeeProduct(cr, uid, context),
...
class account_analytic_account(osv.osv):
    ...
    _columns = {'use_timesheets': fields.boolean('Timesheets', help='Check this field if this project manages timesheets')}

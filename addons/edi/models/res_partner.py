...
RES_PARTNER_EDI_STRUCT = {'name': True,
 'ref': True,
 'lang': True,
 'website': True,
 'email': True,
...
class res_partner(osv.osv, EDIMixin):
    ...
    def edi_export(self, cr, uid, records, edi_struct = None, context = None):
        return super(res_partner, self).edi_export(cr, uid, records, edi_struct or dict(RES_PARTNER_EDI_STRUCT), context=context)
    ...
    def _get_bank_type(self, cr, uid, context = None):
        ...
        if not bank_code_ids:
            _logger.info('Normal bank account type is missing, creating a generic bank account type for EDI.')
    ...
    def edi_import(self, cr, uid, edi_document, context = None):
        ...
        if edi_bank_ids:
            ...
            import_ctx = dict(context or {}, default_partner_id=contact.id, default_state=self._get_bank_type(cr, uid, context))
            for ext_bank_id, bank_name in edi_bank_ids:
                try:
                    self.edi_import_relation(cr, uid, 'res.partner.bank', bank_name, ext_bank_id, context=import_ctx)
                except osv.except_osv:
                    _logger.warning('Failed to import bank account usingbank type: %s, ignoring', import_ctx['default_state'], exc_info=True)

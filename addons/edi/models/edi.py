...
EXTERNAL_ID_PATTERN = re.compile('^([^.:]+)(?::([^.]+))?\\.(\\S+)$')
EDI_VIEW_WEB_URL = '%s/edi/view?db=%s&token=%s'
EDI_PROTOCOL_VERSION = 1
...
def split_external_id(ext_id):
    ...
    raise match or AssertionError(_("'%s' is an invalid external ID") % ext_id)
...
class edi(osv.AbstractModel):
    ...
    def new_edi_token(self, cr, uid, record):
        ...
        edi_token = hashlib.sha256('%s-%s-%s-%s' % (time.time(),
         db_uuid,
         record._name,
         record.id)).hexdigest()
        return edi_token
    ...
    def load_edi(self, cr, uid, edi_documents, context = None):
        ...
        for edi_document in edi_documents:
            ...
            if not module:
                raise AssertionError('a `__module` or `__import_module` attribute is required in each EDI document.')
                raise module != 'base' and not ir_module.search(cr, uid, [('name', '=', module), ('state', '=', 'installed')]) and osv.except_osv(_('Missing Application.'), _('The document you are trying to import requires the OpenERP `%s` application. You can install it by connecting as the administrator and opening the configuration assistant.') % (module,))
            model = edi_document.get('__import_model') or edi_document.get('__model')
            raise model or AssertionError('a `__model` or `__import_model` attribute is required in each EDI document.')
            model_obj = self.pool.get(model)
            raise model_obj or AssertionError('model `%s` cannot be found, despite module `%s` being available - this EDI document seems invalid or unsupported.' % (model, module))
    ...
    def import_edi(self, cr, uid, edi_document = None, edi_url = None, context = None):
        ...
        """
        if not (edi_url and not edi_document):
            raise AssertionError('edi must not be provided if edi_url is given.')
            edi_document = urllib2.urlopen(edi_url).read()
        raise edi_document or AssertionError('EDI Document is empty!')
...
class EDIMixin(object):
    ...
    def _edi_requires_attributes(self, attributes, edi):
        ...
        for attribute in attributes:
            raise edi.get(attribute) or AssertionError('Attribute `%s` is required in %s EDI documents.' % (attribute, model_name))

    def _edi_external_id(self, cr, uid, record, existing_id = None, existing_module = None, context = None):
        ...
        if not ext_id:
            ...
            ir_model_data.create(cr, openerp.SUPERUSER_ID, {'name': ext_id,
             'model': record._name,
             'module': module,
             'res_id': record.id})
        else:
            ...
            if ':' not in module:
                if not module == record._original_module:
                    _logger.debug('Mismatching module: expected %s, got %s, for %s.', module, record._original_module, record)
        ...
        for record in records:
            ...
            if attachment_ids:
                ...
                for attachment in ir_attachment.browse(cr, uid, attachment_ids, context=context):
                    attachments.append({'name': attachment.name,
    ...
    def _edi_generate_report_attachment(self, cr, uid, record, context = None):
        ...
        matching_reports = ir_actions_report.search(cr, uid, [('model', '=', self._name), ('report_type', '=', 'pdf'), ('usage', '=', 'default')])
        if matching_reports:
            ...
            if not report.attachment or not eval(report.attachment, eval_context):
                ...
                file_name += '.pdf'
                self.pool.get('ir.attachment').create(cr, uid, {'name': file_name,
                 'datas': result,
    ...
    def _edi_import_attachments(self, cr, uid, record_id, edi, context = None):
        ...
        for attachment in edi.get('__attachments', []):
            ...
            raise file_data or AssertionError('Incorrect/Missing attachment file content.')
            raise attachment.get('name') or AssertionError('Incorrect/Missing attachment name.')
            raise attachment.get('file_name') or AssertionError('Incorrect/Missing attachment file name.')
            raise attachment.get('file_name') or AssertionError('Incorrect/Missing attachment file name.')
            ir_attachment.create(cr, uid, {'name': attachment['name'],
             'datas_fname': attachment['file_name'],
             'res_model': self._name,
             'res_id': record_id,
             'datas': str(attachment['content'])}, context=context)
    ...
    def _edi_get_object_by_external_id(self, cr, uid, external_id, model, context = None):
        ...
        data_ids = ir_model_data.search(cr, uid, [('model', '=', model), ('name', '=', ext_id), ('module', 'in', modules)])
        if data_ids:
    ...
    def edi_import_relation(self, cr, uid, model, value, external_id, context = None):
        ...
        if not target:
            _logger.debug('%s: Importing EDI relationship [%r,%r] - ID not found, trying name_get.', self._name, external_id, value)
            target = self._edi_get_object_by_name(cr, uid, value, model, context=context)
            need_new_ext_id = True
        if not target:
            _logger.debug('%s: Importing EDI relationship [%r,%r] - name not found, creating it.', self._name, external_id, value)
            model = self.pool.get(model)
            res_id, _ = model.name_create(cr, uid, value, context=context)
            target = model.browse(cr, uid, res_id, context=context)
        else:
            _logger.debug('%s: Importing EDI relationship [%r,%r] - record already exists with ID %s, using it', self._name, external_id, value, target.id)
        if need_new_ext_id:
            ext_id_members = split_external_id(external_id)
    ...
    def edi_import(self, cr, uid, edi, context = None):
        ...
        """
        if not (self._name == edi.get('__import_model') or '__import_model' not in edi and self._name == edi.get('__model')):
            raise AssertionError("EDI Document Model and current model do not match: '%s' (EDI) vs '%s' (current)." % (edi.get('__model'), self._name))
            ...
            existing and _logger.info("'%s' EDI Document with ID '%s' is already known, skipping import!", self._name, ext_id_members['full'])
            return existing.id
        else:
            record_values = {}
            o2m_todo = {}
            for field_name, field_value in edi.iteritems():
                ...
                if field._type == 'many2one':
                    record_values[field_name] = self.edi_import_relation(cr, uid, relation_model, field_value[1], field_value[0], context=context)
                elif field._type == 'many2many':
                    record_values[field_name] = [ self.edi_import_relation(cr, uid, relation_model, m2m_value[1], m2m_value[0], context=context) for m2m_value in field_value ]
            ...
            record_id = self.pool.get('ir.model.data')._update(cr, uid, self._name, module_ref, record_values, xml_id=ext_id_members['id'], context=context)

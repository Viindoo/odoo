# Embedded file name: /opt/lamthao/server/openerp/addons/project_long_term/project_long_term.py
...
class project_phase(osv.osv):
    ...
    _columns = {'name': fields.char('Name', size=64, required=True),
     'date_start': fields.datetime('Start Date', select=True, help="It's computed by the scheduler according the project date or the end date of the previous phase.", states={'done': [('readonly', True)],
                    'cancelled': [('readonly', True)]}),
     'date_end': fields.datetime('End Date', help=" It's computed by the scheduler according to the start date and the duration.", states={'done': [('readonly', True)],
                  'cancelled': [('readonly', True)]}),
     'constraint_date_start': fields.datetime('Minimum Start Date', help='force the phase to start after this date', states={'done': [('readonly', True)],
                               'cancelled': [('readonly', True)]}),
     'constraint_date_end': fields.datetime('Deadline', help='force the phase to finish before this date', states={'done': [('readonly', True)],
                             'cancelled': [('readonly', True)]}),
     ...
     'duration': fields.float('Duration', required=True, help='By default in days', states={'done': [('readonly', True)],
                  'cancelled': [('readonly', True)]}),
     'product_uom': fields.many2one('product.uom', 'Duration Unit of Measure', required=True, help='Unit of Measure (Unit of Measure) is the unit of measurement for Duration', states={'done': [('readonly', True)],
                     'cancelled': [('readonly', True)]}),
     'task_ids': fields.one2many('project.task', 'phase_id', 'Project Tasks', states={'done': [('readonly', True)],
                  'cancelled': [('readonly', True)]}),
     'user_force_ids': fields.many2many('res.users', string='Force Assigned Users'),
     'user_ids': fields.one2many('project.user.allocation', 'phase_id', 'Assigned Users', states={'done': [('readonly', True)],
                  'cancelled': [('readonly', True)]}, help='The resources on the project can be computed automatically by the scheduler.'),
    ...
    _constraints = [(_check_recursion, 'Loops in phases not allowed', ['next_phase_ids', 'previous_phase_ids']), (_check_dates, 'Phase start-date must be lower than phase end-date.', ['date_start', 'date_end'])]
    ...
    def generate_phase(self, cr, uid, phases, context = None):
        ...
        for phase in phases:
            ...
            duration_uom = {'day(s)': 'd',
             ...
             'hours': 'H',
             'hour': 'H',
             'h': 'H'}.get(phase.product_uom.name.lower(), 'H')
...
class project_user_allocation(osv.osv):
    ...
    _rec_name = 'user_id'
    _columns = {'user_id': fields.many2one('res.users', 'User', required=True),
...
class project(osv.osv):
    ...
    _columns = {'phase_ids': fields.one2many('project.phase', 'project_id', 'Project Phases'),
    ...
    def schedule_phases(self, cr, uid, ids, context = None):
        ...
        for project in projects:
            ...
            for phase in project.phase_ids:
                ...
                self.pool.get('project.user.allocation').unlink(cr, uid, [ x.id for x in phase.user_ids ], context=context)
                for r in p.booked_resource:
                    self.pool.get('project.user.allocation').create(cr, uid, {'user_id': int(r.name[5:]),
                     'phase_id': phase.id,
                     'date_start': p.start.strftime('%Y-%m-%d %H:%M:%S'),
                     'date_end': p.end.strftime('%Y-%m-%d %H:%M:%S')}, context=context)

                self.pool.get('project.phase').write(cr, uid, [phase.id], {'date_start': p.start.strftime('%Y-%m-%d %H:%M:%S'),
                 'date_end': p.end.strftime('%Y-%m-%d %H:%M:%S')}, context=context)
...
class account_analytic_account(osv.osv):
    ...
    _columns = {'use_phases': fields.boolean('Phases', help='Check this field if you plan to use phase-based scheduling')}
...
class project_task(osv.osv):
    ...
    _columns = {'phase_id': fields.many2one('project.phase', 'Project Phase', domain="[('project_id', '=', project_id)]")}
